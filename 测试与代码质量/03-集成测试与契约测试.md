...
> **档位选择原则**：能用 `@WebMvcTest`/`@DataJpaTest` 就别用全 `@SpringBootTest`——启动快一个数量级。`@SpringBootTest` 仅用于确实需要完整上下文（如事务传播跨多 Bean）。
...
**Testcontainers 常用模块清单：**

| 模块 | 用途 |
|------|------|
| `mysql`/`postgres`/`oracle` | 关系型 DB |
| `redis`/`mongodb` | KV / 文档库 |
| `kafka`/`rabbitmq` | 消息队列 |
| `localstack` | AWS 本地模拟（S3/SQS） |
| `elasticsearch` | 搜索引擎 |
...
WireMock 还能**注入延迟/故障**，是制造错误场景的利器（详见 08 篇 Mock Server）。

### 1.5 集成测试反模式

| 反模式 | 危害 |
|--------|------|
| 全部 `@SpringBootTest` | 套件慢到没人跑 |
| 集成依赖共享脏库 | Flaky、顺序相关 |
| 真调第三方 API | 慢、烧钱、不稳定 |
| 测完不清理数据 | 污染下游、累积失败 |
| H2 替生产 MySQL | 方言差异漏测 |
...
```mermaid
graph LR
    A[消费者改需求] --> B[更新 Pact 测试]
    B --> C[跑测试生成 pact]
    C --> D[发布 pact 到 Broker]
    D --> E[提供者 CI 拉 pact 验证]
    E --> F{通过?}
    F -->|是| G[提供者可发布]
    F -->|否| H[阻断提供者发布+通知消费者]
    G --> I[can-i-deploy 校验全链路契约绿]
```

> 注意：CDC 不是银弹。它管"接口契约不破"，但**不验证业务流程正确性**（那是 E2E 的活）、**不验证性能**。三者互补。
...
1. **集成测试为何必要？** → 单测 Mock DB，SQL/事务/映射/序列化漏测。
2. **Spring Boot Test 各档位？** → @DataJpaTest/@WebMvcTest 比全 @SpringBootTest 快。
3. **Testcontainers 价值？** → 真依赖又隔离，免方言坑。
4. **CI 没 Docker 怎么办？** → Testcontainers Cloud / 用真实测试库。
5. **WireMock 用途？** → 仿真外部 HTTP + 验证请求契约 + 故障注入。
6. **为何不真调第三方？** → 慢、费钱、不稳定。
7. **CDC 解决什么？** → 微服务接口悄悄 breaking。
8. **CDC 流程？** → 消费者定契约→Broker 存→提供者验证。
9. **Pact 消费者端做什么？** → 写测试生成 pact 文件。
10. **Pact 提供者端做什么？** → 拉 pact 验证自己满足。
11. **can-i-deploy 作用？** → 发布前确认依赖契约全绿。
12. **契约测试能替代 E2E 吗？** → 不能，各管一段。
13. **契约兼容性规则？** → 加字段兼容、删/改 breaking。
14. **H2 替生产 DB 问题？** → 方言差异漏测。
15. **集成测试数据怎么隔离？** → 每用例独立、事务回滚、容器重置。
16. **@SpringBootTest 何时用？** → 需完整上下文时，否则用窄档位。
17. **集成测试反模式？** → 全量启动、共享脏库、真调三方。
18. **契约测试局限？** → 不验业务流程、不验性能。
19. **Mock 与 Fake 在集成层怎么选？** → 外部 HTTP 用 WireMock，DB 用真/Testcontainers。
20. **如何让集成测试快又真？** → Testcontainers 真依赖 + 窄档位 + 数据隔离。
