...
// 时间 O(n)，空间 O(1)。有序数组 → 对撞，和偏小移左、偏大移右。
```
...
func maxArea(h []int) int {
    ...
    for l < r {
        ...
        if h[l] < h[r] { l++ } else { r-- } // 移矮侧（高侧面积已受限）
    }
    return ans
}
// 时间 O(n)，空间 O(1)。移高侧面积必不增，故移矮侧。
```
    ...
    for i := 0; i < len(nums)-2; i++ {
        ...
        for l < r {
            ...
            if s < 0 { l++ } else if s > 0 { r-- } else {
                ...
                for l < r && nums[l] == nums[l+1] { l++ } // 左去重
                for l < r && nums[r] == nums[r-1] { r-- } // 右去重
                l++; r--
            }
        }
    }
...
// 时间 O(n²)，空间 O(1)。排序 + 对撞 + 两侧去重。

func trap(h []int) int {
    l, r, leftMax, rightMax, ans := 0, len(h)-1, 0, 0, 0
    for l < r {
        leftMax = max(leftMax, h[l]); rightMax = max(rightMax, h[r])
        if leftMax < rightMax { ans += leftMax - h[l]; l++ } else { ans += rightMax - h[r]; r-- }
    }
    return ans
}
// 时间 O(n)，空间 O(1)。移动较小 max 一侧（瓶颈已定）。
```

### 10.4 颜色分类（75）/ 快乐数（202）/ 相交链表（160）
...
func sortColors(nums []int) {
    ...
    for i <= p2 {
        ...
        else if nums[i] == 2 { nums[i], nums[p2] = nums[p2], nums[i]; p2-- } // i 不动
        else { i++ }
    }
}
// 时间 O(n)，空间 O(1)。荷兰国旗：p0 管 0 右界、p2 管 2 左界。
```
...
flowchart TD
    A[双指针] --> B{指针方向?}
    B -->|同向速度不同| C[快慢: 环/中点/移除]
    B -->|相向对撞| D[有序: 两/三/四数之和, 盛水]
    B -->|同向维护区间| E[滑动窗口]
    C --> F[链表场景]
    D --> G[数组排序后]
```
...
1. 对撞前提先**排序**；不排序对撞无效。
2. 去重位置：**移动之后**再 `while` 跳过相同，否则丢解或重复。
3. 接雨水移动**较小 max** 一侧；移动错侧答案偏小。
4. 颜色分类换 2 时 `i` **不增**（来自 p2 的可能仍是 2）。
5. 二分上界 `mid` 必须 `+1`（见 [二分查找](二分查找.md)），否则死循环。
6. 快/慢指针循环条件 `f != nil && f.Next != nil`，防空指针。

---

## 十三、速记口诀与小结

> **口诀**：同向快慢找环与中点，相向对撞求和盛水；三数之和先排序、两侧去重莫忘；接雨水移矮侧，颜色分类 i 换 2 不动；窗口滑动同向走，二分边界防死循环。

- 面试三连：**指针方向（同向/相向/窗口）→ 前提（排序/链表）→ 收缩与去重**。
- 与 [链表](链表.md)（快慢/相交）、[滑动窗口](滑动窗口.md)（同向窗口）、[二分查找](二分查找.md)（对撞二分）同构。
...
> 自测标准：**对撞先排序、去重位置、接雨水移矮侧、颜色分类 i 不动**零失误。
...
- 窗口同向：见 [滑动窗口](滑动窗口.md)
- 对撞二分：见 [二分查找](二分查找.md)
- 盛水/接雨水进阶：见 [单调栈与单调队列](单调栈与单调队列.md)（栈法接雨水）
