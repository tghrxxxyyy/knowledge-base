...
### 3.1 A2A 协议要点（2025-2026 跨组织协作）

- **Agent Card**：JSON 文档描述能力/接口/鉴权，放 `.well-known/agent.json` 端点，供其他 agent **发现**；
- **核心原语**：`task`（提交任务并跟踪状态）、`message`（结构化消息）、`artifact`（产物流）；任务状态含 `submitted → working → input-required → completed/failed`，长任务可**推送事件**；
- **与 MCP 的分工已写入业界共识**：A2A 对「跨组织 agent 协作」、MCP 对「agent 内部接工具」——微软（Agent Framework）、OpenAI、Google 2025-12 联合发文明确互补关系（Agentic Commerce 场景试点）；
- **落地注意**：A2A 侧重互操作标准，**身份与授权仍是企业自己的事**（OAuth 2.1 + 网关，见工程落地 05 篇 2.3）；小团队内部多 agent 用 Handoff 就够了，A2A 是「要对外开放/跨公司协作」才需要的重量级标准。
...
## 七、多智能体通信拓扑（Mermaid 深化）

把第三节的四种拓扑画成完整数据流，便于选型：
...
| 拓扑 | 数据流 | 何时选 | 注意 |
| --- | --- | --- | --- |
| 流水线 | 顺序传递、可加 gate | 步骤固定 | 一步慢全链慢 |
| 监督者 | 中央派活+回收 | 任务结构运行时才清 | 中心成瓶颈 |
| 群体 | 对等自由讨论 | 头脑风暴 | 易跑题/无主责 |
| 层级 | 父派子树 | 大规模分层治理 | 路由逻辑复杂 |

**回传纪律（代码）**：子 agent 必须只回传摘要，否则主窗口被淹。

```python
def subagent_run(task):
    full = do_work(task)                       # 在子 agent 干净窗口里跑
    return llm.summarize(full, max_tokens=1500)  # 只回传摘要
```

> 💡 选型心法：**先穷尽单 agent**；要并行/隔离/专业化才拆。拆了就守住「隔离窗口 + 摘要回传」纪律，否则不如不拆。
