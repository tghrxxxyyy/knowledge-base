...
## 七、无框架派：裸 Agent Loop（2025-2026 生产趋势）

生产社区的一个显著趋势是「**框架去重**」——不少团队砍掉框架，回归**几十行裸循环**：

```python
def agent_loop(goal, tools, llm, max_steps=30):
    messages = [{"role": "system", "content": SYSTEM}]
    for _ in range(max_steps):
        msg = llm(messages, tools=tools)          # 模型决策
        if msg.tool_calls:
            for tc in msg.tool_calls:
                result = execute(tc)              # 你自己控制的执行
...
| 对比 | 框架派 | 裸循环派 |
| --- | --- | --- |
| 上手 | 快（内置记忆/重试/调试） | 慢（全自己写） |
| 可定制性 | 被抽象约束 | 完全可控 |
| 依赖风险 | 框架版本迁移/抽象泄漏 | 只有模型 API |
| 调试 | 时间旅行等利器 | 自己打 log |
| 适用 | 复杂状态机/持久化/多 agent | 单 agent 工具循环、核心能力已稳定 |

**裸循环派适合**：单 agent + 工具调用、已稳定可固化、团队要极致可控（成本/行为/审计）；
**框架派适合**：需要持久化状态机、HITL 断点、子代理调度、团队要快速交付多 agent 系统。

> 一句话：**2026 年的成熟姿势 = 「裸循环为主干 + 按需引入框架能力（持久化/HITL 用 LangGraph 或自研）」——框架是库不是枷锁。**（示例实现见 02 篇六节模板与 03 篇错误恢复）
