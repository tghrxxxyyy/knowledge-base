...
> RAG 结合 NL2SQL：检索表结构/SQL 样例，生成可执行查询。核心是语义到结构。
...
```python
ctx = retrieve(table_docs, question)
sql = llm.generate("根据上下文写SQL: "+ctx+question)
```
...
RAG与Text2SQL实战 的核心是"结构化拆解、明确边界、可观测兜底"：在正确性与扩展性之间取平衡，用工程化手段把不确定性降到最低，并随规模持续演进。
