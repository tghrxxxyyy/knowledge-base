...
### 2.1 长上下文时代，RAG 会死吗？（2025-2026 必须答的题）

上下文窗口冲到 1M~10M token 后，「直接把整库塞进去」成为可能——但业界结论**不是 RAG 死亡，而是分工更明确**：

| 维度 | 长上下文硬塞 | RAG 检索 |
| --- | --- | --- |
| 成本 | token 随全库线性涨（10M 输入每次都在烧钱） | 只付 top-k 的成本 |
| 延迟 | 首 token 随上下文线性涨（prefill 变慢） | 检索百毫秒级 |
| 质量 | 「lost in the middle」：模型对长上下文中间段遗忘严重（Needle-in-a-Haystack 测试） | 只喂相关片段，噪声小 |
| 知识更新 | 全量重发上下文 | 增量改库即可 |
| 查证/引用 | 来源追踪难 | 天然带 chunk 来源 |

- **2025 实测共识**：长上下文在「简单事实查找」（单针测试）表现好，但**多跳、跨段聚合、细粒度推理**仍明显掉点（MAPE/Needle 多针测试掉 30%+）；
- **正确姿势是「组合拳」**：检索（top-k 定位）→ 把相关片段 + **必要时更多上下文**一起喂给长窗口模型（RAG 决定「看哪」，长窗口决定「看多细」）；
- **长上下文 RAG 变体**：GraphRAG（结构化跨段聚合，见 02 篇）、Late Chunking（见下）——都是为「长文 + 检索」矛盾服务的。

> 面试一句话：**长上下文是 RAG 的「受益者」而非「替代者」：检索负责选，窗口负责看，成本与质量两头都赢。**

### 2.2 Late Chunking（晚切分，2025 检索热点）

传统切分问题：**先切块再编码**会让块丢失上下文（块首尾句语义残缺）；Late Chunking 反其道：

```
整篇文档 → 一次编码（保留全文注意力上下文） → 再按边界把向量「切开」存储
```

- 做法：用**长上下文嵌入模型**整文编码，然后在模型内部按 chunk 边界取平均池化，得到「每个块都见过全文」的向量；
- 效果：语义相似度比传统切分高（多份研究显示命中率/NDCG 显著提升），且**几乎零额外成本**（只多一次长文编码）；
- 落地：`llama-index` 的 `LateChunking` 模块 / `Jina` 的 late chunking API 已内置；前提是嵌入模型支持长上下文（≥4K~8K）。
...
| `BAAI/bge-m3` | BAAI（2024-2025 主流） | 开源标杆：1024 维、多语言、支持 **dense + sparse + multi-vector 三路检索**，中文 RAG 首选 |
| `text-embedding-3-large` | OpenAI | 闭源综合强，3072 维可降维 |
| `Qwen3-Embedding-0.6B/4B/8B` | 阿里（2025-2026） | 开源新秀，MTEB 中文+英文领先，长文本（8K）直接编码 |

> 嵌入选型三问：**要不要中文 → bge-m3 / Qwen3-Embedding；要不要稀疏词项召回 → bge-m3 的 sparse 输出（省一套 BM25）；多大规模 → 千亿级私域用开源可自部署（bge/Qwen 系），预算足上闭源。**
...
> 企业落地的工程侧（权限隔离、脱敏、增量管道、评测看板）见「[工程落地/03-RAG-工程化](../工程落地/03-RAG-工程化.md)」；检索质量如何评见「[03-评估与最佳实践](./03-评估与最佳实践.md)」。
