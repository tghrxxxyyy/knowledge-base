...
>
> 💡 实测对比（社区共识，非权威基准）：英文场景 Cohere 常略优（效果好、按 token 计费、需外发数据）；中文/合规私有化优先 `bge-reranker-v2-m3`（开源、自托管算力自有）；`jina-reranker-v2` 长上下文友好；`ms-marco-MiniLM-L-12-v2` 轻量快但精度一般。2025 新选项：Qwen3-Reranker、gpt-oss-rerank。具体以你自己的评测集为准。
...
### 9.1 GraphRAG 工程化要点（避免「建了个寂寞」）

- **两条建图路线**：微软 GraphRAG（LLM 抽取实体/关系 + 社区检测，成本高但全局性强）vs LightRAG（轻量双级检索，2025 年热度高、成本低一个量级）；
- **建图成本**：全量文档 LLM 抽取很贵（每千 token 实体抽取 3~5× 文本量）——**先抽样估算成本再全量**；增量更新（新文档 → 增量抽取 → 合并子图）是必须项；
- **混合才是常态**：生产上「向量检索 + 图检索 + BM25」三路召回再 RRF 融合（社区做法），纯图问答少；
- **评测重点**：GraphRAG 的价值在「多跳聚合」类问题（如「A 公司与 B 公司有哪些共同供应商」）——评测集必须含这类题，否则看不出投入回报；
- **适用边界**：实体关系明确的领域（金融关系、供应链、科研文献）收益大；自由文本/叙事文档（对话、散文）图谱稀疏，收益小。

### 9.2 长上下文 RAG 变体（1M 窗口时代的再平衡）

- **LightRAG-Lite / GRAG（图增强）**：用图增强「长文全局聚合」的检索召回；
- **分层摘要 + 检索**：文档树（每层摘要）→ 顶层检索定位子树 → 底层取细节（LangGraph 的 Hierarchical RAG 模式），比「全量塞窗口」省 10~100× token；
- **窗口内结构注入**：长文档整体入窗时，用「段落标题索引 + 行号引用」替代整篇正文（Anthropic 的长上下文结构化技巧）——检索负责粗选，结构负责细读。
- 长上下文与 RAG 的完整关系与取舍见「[RAG/01 2.1 长上下文时代，RAG 会死吗](./01-概述与基础管线.md)」。

> 💡 实测结论（社区共识，非权威基准）：英文场景 Cohere 常略优；中文/合规私有化优先 `bge-reranker-v2-m3`。无论哪个，**重排只在粗召回 top-20~50 上跑**，成本可控、收益明显。具体以你自己的评测集为准。

## 十、Agentic RAG（检索智能体化，2025-2026 主线）

基础 RAG 是「查一次就回答」；Agentic RAG 让模型**自主决定何时检索、检索几次、要不要换库/换工具**——是「RAG + Agent Loop」的融合，也是 Deep Research / Agentic Search 类产品的底层范式。

```
查询 → Agent 规划(要不要检索? 拆几个子问题?) → 检索(多轮/多路)
     → 自我评估(答案够吗? 引用可靠吗?) → 不满意再检(CRAG 修正) → 生成
```
...
| 机制 | 核心思路 | 代表工作 |
| --- | --- | --- |
| **Adaptive-RAG** | 根据查询难度路由：简单→直接答，中等→检索一次，复杂→多轮检索 | Adaptive-RAG（2024） |
| **Self-RAG** | 检索后自我评估（是否需要检索、检索片段是否相关、回答是否被支持），决定继续或停止 | Self-RAG（ICLR 2024） |
| **CRAG（Corrective RAG）** | 检索质量差时主动修正：丢弃低置信片段 → 换检索源（Web/知识库）重试 | Corrective-RAG（2024） |
...
| 规划 | 无 | 拆解子问题、选检索源 |
| 评估 | 无 | 自我评估 + 修正闭环 |
| 延迟/成本 | 低 | 高（多轮 LLM 调用，通常配推理模型） |
| 适用 | 问答、单跳知识查询 | 多跳研究、实时信息、复杂分析 |

### 3. 工程要点

- **用推理模型做规划/评估**（思考 token 换准确性），检索与生成仍可用快模型；
- **设检索次数上限与超时**（防 Agent 死循环烧钱），失败降级为单轮 RAG；
- 配套引用验证：生成后校验引用片段真实性（RAG 防幻觉闭环，见「03-评估与最佳实践」）。

> 一句话：**Agentic RAG = RAG 加「规划-检索-评估-修正」闭环，Deep Research 类产品的地基；代价是多轮延迟，工程上必须设上限与降级。**
