...
### 4.1 2025-2026 训练范式趋势（把「训什么」讲清楚）

1. **预训练「卷不动了」，后训练成主战场**：Base 模型开源趋同（Llama/Qwen/DeepSeek 系），差距转移到「**后训练配方**」——SFT 数据质量、DPO/RL 阶段、推理 RL（见推理 01 篇）成了团队差异点；
2. **合成数据主导**：后训练数据 90%+ 由强模型合成 + 过滤（见 03 篇 1.1），人类标注退守「少量高质量锚点 + 评测」；
3. **RL 后训练进入推理阶段**：从「对齐偏好」扩展到「训练推理能力」（RLVR/GRPO，o3/R1 的复现路径），训练重心从「学知识」转向「学会思考」；
4. **MoE + 长上下文成为底座标配**：Qwen3-MoE、DeepSeek-V3 证明「稀疏激活省训练算力 + 长上下文（128K~1M）是卖点」，训练时直接训练长序列（见 02 篇长上下文）；
5. **蒸馏流行化**：用小模型蒸馏大模型（或反向：大模型蒸馏出小模型）在推理场景成主流路线（见推理 01 篇 3.3）。
