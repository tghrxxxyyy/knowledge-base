...
| **RLOO / REINFORCE++** | 去价值网络 + 组采样基线（轻量 RL） | 2025 推理对齐常用轻量配方 |

> GRPO 因 DeepSeek-R1 而闻名：用「组内相对优势」替代价值网络，大幅简化 RL 循环，成为推理模型训练的关键技术。

### 6.1 2025-2026 对齐工程新现实

- **对齐 ≠ 单一技术**：生产线上「SFT 学格式 → DPO 学偏好 → RL（GRPO/RLVR）学推理与安全」三段式已成标配，且各段数据**互相独立**（不同标注/合成管道）；
- **偏好数据稀缺化**：合成偏好对（强模型生成 + judge 打分）占比逐年上升，但「**长尾/领域偏好仍需人工**」——合成偏好同质化，导致模型「看似对齐实则单一」；
- **推理模型的对齐特殊性**：长 CoT 模型不能简单 DPO（会压缩思考、破坏推理），2025 年的做法是**过程监督**（对思考步骤打分，见推理 01 篇 3.2）+ 轻量 GRPO 微调；
- **对齐评测回归**：对齐效果不是「一次训完」——线上持续监控偏好指标（点赞率/拒答率/越狱成功率），见 07 篇安全基准。
