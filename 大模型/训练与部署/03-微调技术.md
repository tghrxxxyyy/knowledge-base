...
### 1.1 SFT 数据工程：2025 年 90% 以上数据是合成的

高质量 SFT 数据的核心事实：**OpenAI 系、DeepSeek、Qwen 等 2025 年开源 SFT 数据集绝大多数来自「合成 + 过滤」流水线**，而非人工标注。标准流程：

```
强模型采样 → 过滤/去重 → 质量打分(LM-as-Judge/规则) → 难度增强 → 配比合成
```

| 技术 | 做法 | 用途 |
| --- | --- | --- |
| **Self-Instruct / Evol-Instruct** | 种子指令让模型自生成变体，逐步提升难度 | 指令多样性 |
| **Rejection Sampling（拒绝采样）** | 采样多条回答只留最优（如 RFT，用强 judge 打分） | 回答质量 |
| **蒸馏生成偏好对** | 强模型对同一 prompt 生成好/坏两答（或「选+拒」） | DPO/RL 阶段用 |
| **合成文档 + 问答** | 先让模型生成领域文档，再从文档抽问答 | 领域知识注入 |

**质量红线**：
- **去重与近重复**（simhash/embedding 聚类）：重复样本让模型「背题」且过拟合；
- **难度分布**：全简单样本学不到推理，全难题训不动——按「简单 60% / 中等 30% / 难 10%」配比（经验值）；
- **模型崩塌风险**（Model Collapse）：纯合成数据喂回模型会退化（采样分布收窄、多样性枯竭）——保持 10~30% 人类/真实数据锚点；
- **版权与合规**：合成数据若来自受保护语料，需合规审查（2025-2026 诉讼热点）。

> 一句话：**SFT 数据 = 合成流水线（生成→过滤→打分→配比）+ 真实验证集兜底；质量与配比决定微调上限，数据工程 > 训练技巧。**
...
**LoRA 家族深化（2025 年生产选型）**：
- **rsLoRA / LoRA+**：调整缩放系数 / 分层学习率，收敛更快，几乎零成本升级（直接替换默认 LoRA）；
- **AdaLoRA**：按重要性自适应分配秩，理论更优但实现重，生产少用；
- **多 LoRA 合并 vs 挂载**：多个适配器可 **merge 回底座**（省显存、零推理开销，但不可再拆）或**动态挂载**（vLLM 多 LoRA 服务，切换开销极小）——「长期固化用 merge，多租户变体用挂载」；
- **DoRA**：把 ΔW 分解为方向 + 幅度，效果略优于 LoRA，与 QLoRA 可叠加。

**推理模型微调注意**（R1/o3 类）：直接对长 CoT 数据做 SFT 易致**推理能力退化**（模型学会「照抄格式」而非「真思考」）——需要拒绝采样 + RL 阶段配合，详见「[推理与思考模型/01-核心原理与训练机制](../推理与思考模型/01-核心原理与训练机制.md)」。
...
## 五、分布式并行训练：单卡装不下怎么办（DP/TP/PP/EP/ZeRO）

> 2025 年训练一个 70B~400B 的模型/微调必然涉及多卡甚至多机，四种并行范式 + ZeRO 分片是必答考点。

| 并行范式 | 切什么 | 解决的问题 | 通信开销 |
| --- | --- | --- | --- |
| **数据并行 DP/FSDP** | 每个 GPU 一份完整模型，喂不同 batch | 吞吐低，简单直接 | 每步同步梯度（重） |
| **张量并行 TP** | 一层内的矩阵按行/列切开，多卡算同一个 layer | 单层放不下（显存） | **极高**（每层每步都通信） |
| **流水线并行 PP** | 层按先后分段到不同卡，像流水线 | 模型层数多（纵向放不下） | 中（仅切边界通信） |
| **专家并行 EP** | MoE 的专家层分片到多卡，token 按路由分发 | MoE 模型（DeepSeek-V3/Qwen3-MoE） | 中（All-to-All） |

**ZeRO（零冗余优化器）**：数据并行的「显存救星」——把优化器状态/梯度/参数分片到各卡（Stage1/2/3），Stage3 可让 8B 模型在 4×4090 上全参训练；FSDP 是 PyTorch 对 ZeRO-3 的官方实现。

**集群与网络**：
- 千卡以上必须 **IB/RDMA**（GCP 的 A3、Azure ND 系列、自建 InfiniBand），TCP 在 All-to-All 通信下吞吐损失 5~10 倍；
- 典型配比经验：**TP ≤ 8**（超过则通信吞掉收益）、**PP = 机架内层数**（跨机架 PP 慢）、**DP/EP 跨机扩展**；
- checkpoint 每 N 步保存一次（灾备 + 断点续训），MoE 检查点体积可达数百 GB，需增量/异步保存。

> 一句话：**显存不够先上 ZeRO/FSDP；单层放不下用 TP；模型太长用 PP；MoE 用 EP——先算清「显存瓶颈在权重还是激活」，再选并行范式。**

## 六、一个最小 QLoRA 配置（Axolotl 风格）
