...
### 3.1 手算一个 KV Cache（面试送分题）

以 **Qwen3-14B（GQA，32 Q 头 / 8 KV 头，head_dim=128，32 层）**，FP16（2 字节/token 元素）为例：

```
单 token 单层 KV 字节 = 2(份) × 8(KV头) × 128(维度) × 2(字节) = 4096 B
单 token 全部 32 层    = 4096 × 32 = 128 KB
并发 100 个请求 × 每个 4K token 上下文：
  128 KB × 4096 × 100 ≈ 50 GB 显存
```

| 模型 | KV 头数 | 4K 上下文 × 100 并发 KV 显存（估算） |
| --- | --- | --- |
| 同规模 MHA（32 KV 头） | 32 | ~200 GB（放不下单卡） |
| 同规模 GQA（8 KV 头） | 8 | ~50 GB |
| 同规模 MLA（共享压缩） | 极低 | ~10-20 GB |

> 结论：**GQA/MLA 不是「提速技巧」，是「并发经济学」——KV 砍 4 倍 ≈ 同卡并发翻 4 倍 ≈ 单 token 成本除以 4。** 这也是为什么 2024 后开源模型全员 GQA/MLA。动态分配（vLLM PagedAttention、SGLang RadixAttention）在此基础上再省 2~5 倍。
