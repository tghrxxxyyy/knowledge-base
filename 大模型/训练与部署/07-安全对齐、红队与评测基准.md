...
> 通用评测方法论、LLM-as-Judge 与评测流水线详见「[模型评测与基准](../模型评测与基准/01-LLM-Eval方法论.md)」与「[02-主流基准与榜单](../模型评测与基准/02-主流基准与榜单.md)」——本篇只保留「安全与鲁棒专项」视角，通用基准表不在此重复罗列。

### 4.0 安全与鲁棒专项基准

| 基准 | 测什么 | 说明 |
| --- | --- | --- |
| **AdvBench** | 有害指令越狱成功率 | 安全对齐核心指标 |
| **ToxiGen / RealToxicityPrompts** | 毒性内容 | 生成安全 |
| **HaluEval / TruthfulQA** | 幻觉与事实性 | 对抗性事实测试 |
| **JailbreakBench / HarmBench** | 越狱攻击面 | 红队迭代回归用 |
| **WMDP** | 危险知识泄露 | 生物/化学/网络武器知识 |

> 这些基准与「4.1-4.4」的通用基准不同：通用基准测「能力上限」，安全专项测「拒绝与防护下限」——**上线前两个都要看**。
