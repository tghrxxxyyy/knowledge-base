...
## 八、Agentic RAG 工程化（把 RAG 交给 agent 管）

2025 年后生产 RAG 大多不是「单轮检索→生成」，而是 **agent 编排 RAG**。从工程视角看有三种演进档位：

| 档位 | 形态 | 何时够用 |
| --- | --- | --- |
| **Pipeline RAG** | 固定链路：检索→重排→生成 | 问题形态单一、质量已达标 |
| **Adaptive RAG（条件路由）** | 分类器判断「要不要检索/检索多深」，简单问题直答 | 混合问题流量，省 token |
| **Agentic RAG（完全编排）** | agent 自主决策：多轮检索、改写查询、选工具、验证答案 | 复杂多跳、多数据源 |

**工程落地的 Agentic RAG 套路（已验证的组合）**：

```
query → (改写/分解子问题) → 多路检索(向量+BM25+知识图谱) → 自评「够不够」
     → 不够则换关键词/换源/追问 → 够了 → 重排 → 生成 → groundedness 自检 → 不达标重试
```

- **自评（self-critique）**：让 agent 对检索结果打分（相关性/覆盖度），低分触发再检索——检索质量靠「循环」兜底，而非一次命中；
- **查询改写**：长问句先改写为检索友好的关键词（HyDE 或 LLM 改写），多跳问题**分解成子查询逐个检索再聚合**；
- **工具化**：RAG 组件（检索/重排/表格问答/代码解释器）都注册为工具，agent 按需调用（复用智能体 03 篇的 MCP 套路）；
- **成本控制**：每次「再检索」都烧 token——设最大检索轮数与预算，超限给「尽力回答 + 标注不确定」；
- **评测升级**：Agentic RAG 的评测从「检索指标」升级为「任务成功率 + 轮次成本」（见评测 02 篇 3.4 Agentic Eval）。

> 一句话：**别一上来就 Agentic RAG——先把 Pipeline 跑通、评测达标；只有多跳/多源/不确定问题占比高时再升级编排，否则是为复杂度而复杂度。**
