...
GoF 原话：*Compose objects into tree structures to represent part-whole hierarchies. Composite lets clients treat individual objects and compositions of objects uniformly.*
...
class FileNode implements Node { public void accept(Visitor v){ v.visit(this); } }
class DirNode implements Node {
    List<Node> children;
    public void accept(Visitor v){ v.visit(this); children.forEach(c->c.accept(v)); }
}
interface Visitor { void visit(FileNode f); void visit(DirNode d); }
// 新增「导出 JSON / 统计行数 / 权限校验」等操作，无需改节点类
```

> 当「树结构稳定、操作常变」时，组合 + 访问者是最佳搭档；反之若节点类型常变，则访问者会频繁改接口。
...
- 与**访问者**：组合定义结构，访问者定义作用于结构的操作，二者常配合。
...
- **深树爆栈防护**：限制最大深度，或用显式栈替代递归；内容安全场景要防恶意构造超深树。
...
```java
// 反模式：透明式下调用方不知道叶子的 add 会抛异常
class FileNode implements Node {
    public void add(Node n) { throw new UnsupportedOperationException(); }
}
client.getSelected().add(newNode); // 若选中的是文件，运行时才炸
```

**修正**：安全式更稳妥——`add` 只在容器暴露，客户端用 `instanceof` 判断；或在 add 返回 boolean 表明是否支持。

### 案例：组合树 toString 无限递归

```java
class DirNode {
    List<Node> children;
    public String toString() { return "Dir" + children; } // List.toString 又调子节点 toString → 循环引用爆栈
}
```

**修正**：toString 避免序列化整个树；父引用用 `transient` 或在序列化时忽略。
...
21. Q：组合模式如何持久化？ A：邻接表（id/parent_id）或嵌套集（Nested Set）存库，加载时重建树。
22. Q：组合树能懒加载吗？ A：能，容器节点的 children 按需从 DB 拉取，避免一次性加载整棵树。
23. Q：组合模式在 UI 框架的应用？ A：Swing/JavaFX 的容器-组件树，layout 递归测量/布局子节点。
24. Q：组合模式怎么计算树深度？ A：递归 `1 + max(children深度)`，注意剪枝与深度上限。
25. Q：组合模式支持 undo 吗？ A：用备忘录/命令模式记录 add/remove 操作，便于回滚整棵子树。
26. Q：组合与享元如何结合省内存？ A：重复叶子（如相同样式文本）用享元共享，树只存引用。
27. Q：组合模式能用于规则引擎吗？ A：能，AND/OR/NOT 节点组合成布尔表达式树，eval 递归求值。
28. Q：组合模式与策略怎么配合？ A：每个节点可持有一个策略决定如何聚合子结果（求和/取最大/平均）。
29. Q：组合树的事件冒泡怎么实现？ A：子节点触发事件后沿 parent 链向上传递，类似 DOM 事件冒泡。
30. Q：组合模式最大的风险？ A：过度统一导致叶子被迫实现无关方法（透明式），或类型判断散落（安全式）。
...
- 结构型：[装饰器模式](装饰器模式.md) · [享元模式](享元模式.md)
- 行为型：[访问者模式](../行为型/访问者模式.md) · [迭代器模式](../行为型/迭代器模式.md)
- 源码：MyBatis `SqlNode` 动态 SQL 组合
