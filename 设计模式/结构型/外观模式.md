...
5. **异常归一**：门面应把子系统五花八门的异常收敛为业务可读的统一异常，避免调用方捕获十种异常。
...
class OrderFacade {
    public void placeOrder(Order o) {
        if (o.getAmount() > 10000) { /* 风控规则写进门面 */ }
        inventory.lock(o); payment.charge(o);
    }
}
```

**修正**：风控是领域规则，应下沉到领域服务；门面只编排 `riskService.check(o)`。
...
20. Q：门面在整洁架构的位置？ A：属「接口适配器层」，把内层用例暴露给外层（UI/API）。
21. Q：门面与事务边界的关系？ A：门面方法常标 @Transactional，但不要把长耗时 IO 包进同一事务。
22. Q：门面如何做异常归一？ A：捕获子系统多种异常，转换为统一的业务异常抛给调用方。
23. Q：门面与 MVC 的 Controller 区别？ A：Controller 接收 HTTP 请求、做参数校验；门面是业务用例编排，可被多个 Controller 复用。
24. Q：门面会造成性能瓶颈吗？ A：不会，门面本身只是转发；瓶颈在被编排的子系统。
25. Q：什么时候不该用门面？ A：子系统很简单、调用方只需调一个方法时，门面是多余间接层。
26. Q：门面与命令模式配合？ A：门面方法内部可把每步操作封装成命令，支持撤销/重试编排。
27. Q：前端也有门面吗？ A：有，如把多个 backend API 聚合成一个 BFF 接口，本质就是「前端门面」。
28. Q：门面如何避免循环依赖？ A：严格单向：门面→子系统，子系统不引用门面；用包结构约束。
29. Q：门面能统一异步编排吗？ A：能，门面内部用 CompletableFuture 编排多个子系统异步调用，对外仍同步/异步统一。
30. Q：门面拆分粒度怎么定？ A：按「业务用例 + 调用方角色」拆分，一个角色一个门面，而非一个子系统一个门面。
...
- 结构型：[适配器模式](适配器模式.md) · [中介者模式](../行为型/中介者模式.md) · [代理模式](代理模式.md)
- 架构实践：DDD 应用服务（Application Service）、API 网关、BFF
