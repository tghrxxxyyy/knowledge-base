...
GoF 原话：*Use sharing to support large numbers of fine-grained objects efficiently.*
...
```java
// 并发安全的享元工厂（用 ConcurrentHashMap + computeIfAbsent）
class TreeFactory {
    private static final ConcurrentMap<String, TreeType> POOL = new ConcurrentHashMap<>();
    static TreeType get(String name, String color) {
        return POOL.computeIfAbsent(name + "@" + color, k -> new TreeType(name, color));
    }
    // 容量治理：防止 key 无限增长
    static void evictIfNeeded(int max) {
        if (POOL.size() > max) PODL.clear(); // 或接入弱引用 / LRU
    }
}
```

> 享元工厂的 Map 若无限增长会是内存泄漏源。高动态 key 场景应用 `WeakHashMap` 或加容量上限 + 淘汰策略。
...
- **Kafka**：`BufferPool` 固定 16KB 内存块池化复用，避免高频分配导致 GC。
...
- **工厂容量治理**：key 空间大时用 `WeakHashMap` 或加 LRU 淘汰，防内存泄漏。
...
```java
class TreeType { int x, y; }   // 错误：坐标本应是外部状态
// 两个绘制调用会互相覆盖 x/y，导致所有树画到同一位置
```

**修正**：坐标作为 `draw(int x, int y)` 参数传入，享元只保留 name/color 等内部状态且 final。

### 案例：享元工厂无限增长

```java
// 反模式：key 由用户任意字符串拼接，永不清理
POOL.computeIfAbsent(userInput + color, k -> new ExpensiveType(k));
// 恶意/高频不同 key 导致 POOL 无限膨胀 → OOM
```

**修正**：用 `WeakHashMap`、设上限 + LRU 淘汰，或对 key 做白名单校验。
...
17. Q：享元会导致内存泄漏吗？ A：工厂 Map 若无限增长会，需设上限/弱引用。
...
21. Q：享元工厂需要并发安全吗？ A：需要，用 ConcurrentHashMap + computeIfAbsent 保证只建一个。
22. Q：WeakHashMap 在享元的作用？ A：当 key 失去外部强引用时自动回收对应享元，防泄漏。
23. Q：享元与组合模式怎么配合？ A：树节点可用享元共享重复子节点（如相同样式文本段）。
24. Q：享元能用于 DTO 吗？ A：若大量 DTO 字段相同，可把公共部分抽成享元，但 DTO 通常可变，需谨慎。
25. Q：游戏里享元的典型用法？ A：子弹/粒子按类型共享（贴图/配置），位置/速度作为外部状态。
26. Q：享元与克隆（原型）对比？ A：原型每次克隆新对象（占内存）；享元复用已有（省内存）。
27. Q：享元模式会增加代码复杂度吗？ A：会，需显式拆分内外状态，小对象场景得不偿失。
28. Q：如何判断某对象能否享元化？ A：问「去掉随实例变化的字段后，剩余字段能否不可变且被大量复用」。
29. Q：Spring 单例 Bean 是享元吗？ A：不完全是——它更像单例；但无状态 Bean 被所有请求共享，思想接近。
30. Q：享元在 ORM 框架的应用？ A：Hibernate 的不可变值类型（如 Enum、Embedded）可享元缓存，减少实体数。
...
- 结构型：[单例模式](../创建型/单例模式.md) · [原型模式](../创建型/原型模式.md)
- 工程实践：对象池（连接池/线程池）、Netty `PooledByteBufAllocator`、Kafka `BufferPool`
