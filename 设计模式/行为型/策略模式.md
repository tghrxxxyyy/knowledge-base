...
> **一句话定义**：定义一系列**可互换的算法**，把它们封装起来，使算法可独立于使用它的客户端变化——「消灭 if/else 分支」的标准答案。
...
class FullReduction implements DiscountStrategy {
    private double threshold, minus;
    public FullReduction(double t, double m){ threshold=t; minus=m; }
    public double calc(double p){ return p >= threshold ? p - minus : p; }
}
class Cart {
...
| 模式 | 谁决定行为 | 关系 | 典型 |
|------|-----------|------|------|
| 策略 | 客户端主动选择算法 | 组合（has-a） | 促销折扣、序列化格式 |
| 状态 | 对象内部状态自动切换 | 组合（has-a） | 订单状态机 |
| 桥接 | 编译期解耦两维度 | 组合（has-a） | JDBC |
| 责任链 | 请求沿链传递命中即止 | 链（next） | FilterChain |

## 适用场景

- 一个行为有多种可互换算法（排序、压缩、校验、促销、序列化、路由）。
- 避免大量 if/else 或 switch 分支，且算法可能独立演进。
...
- **JDK**：`Comparator` 即排序策略；`LayoutManager`（布局策略）；`ZipOutputStream` 的压缩策略；`RejectedExecutionHandler`（线程池拒绝策略）。
- **Spring**：`Resource`（classpath/file/url 资源策略）、`HandlerMapping`/`HandlerAdapter`、`RetryPolicy`（重试策略）。
...
16. Q：策略模式与桥接的区别？ A：桥接解耦的是「抽象与实现两个维度」，策略只替换行为算法。
17. Q：策略注册表怎么防空指针？ A：get 时给默认策略（如 NoDiscount），或不存在时抛明确业务异常。
18. Q：策略能在运行时动态加载吗？ A：能，SPI/插件机制加载策略实现类，注册表热更新。
19. Q：函数式策略有什么坑？ A：lambda 无类型名、难调试、难在注册表按语义查找，复杂策略仍建议显式类。
20. Q：策略与责任链怎么配合？ A：链上每环可用策略处理不同逻辑，节点选择策略、链决定顺序。

## 速记口诀

> **「一系列算法封装好，客户端随选随换；注册表加 DI，if-else 全不见。」**
> 策略（主动选算法）≠ 状态（自切换）。
