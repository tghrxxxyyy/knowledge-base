...
## 三·五、深挖：GitOps 与传统部署的范式对比

| 维度 | 传统（kubectl apply / 脚本） | GitOps（Argo CD / Flux） |
|------|------------------------------|--------------------------|
| 真相源 | 人脑 / 分散脚本 | **Git 仓库（唯一真相源）** |
| 变更入口 | 命令行 / 平台点击 | PR（评审 + 审计 + 回滚天然成立） |
| 漂移处理 | 无人管，手动发现 | **持续 reconcile 自动纠偏** |
| 审计 | 难 | 每次变更 = 一次 commit |
| 回滚 | 手工找旧版本 | `git revert` 即回滚 |
| 多集群 | 各自为政 | 同一套 Git 声明灌多个集群 |

> 心法：GitOps 把「部署」从**操作**变成**状态管理**——你只负责改 Git，集群永远向 Git 收敛。这背后是 K8s 声明式 API 的哲学：不是「执行步骤」，而是「描述期望，等待收敛」。

## 三·六、深挖：eBPF 为什么是基础设施的未来

```
传统: 数据包 → 内核协议栈 → iptables/用户态代理(拷贝多次) → 应用
eBPF: 数据包 → 内核钩子点运行注入的字节码(虚拟机校验+JIT) → 旁路处理
```

| 应用 | 解决的问题 | 代表项目 |
|------|-----------|----------|
| 网络 | 高性能转发，替代 kube-proxy/iptables | Cilium |
| 安全 | 内核态行为监控、运行时安全 | Falco、Tetragon |
| 可观测 | 低开销 trace/指标采集 | Pixie、Cilium Hubble |
| 性能 | 内核路径定制（丢包采样、限速） | BCC、bpftrace |

- eBPF 的价值：**不重启内核、不改代码，往内核里安全注入程序**——让"内核可编程"成为可能；
- 面试点：eBPF 程序运行前要经过**验证器**（保证终止性与内存安全），失败则拒绝加载。

## 三·七、面试高频追问

1. Q：K8s 为什么弃用 Docker？ A：不是弃用，是弃用 dockershim——容器运行时标准化为 CRI，containerd 更轻且没有 Docker 的上层工具链。
2. Q：声明式 API 和命令式区别？ A：命令式「告诉我怎么做」，声明式「告诉我想要什么」；声明式可收敛、可重放、幂等。
3. Q：Helm 解决什么问题？ A：一套应用多环境部署的模板化 + 版本化发布（release 可升级回滚）。
4. Q：Prometheus 拉模型和推模型区别？ A：拉模型目标发现动态（K8s 友好）、存储集中可控；推模型适合短生命周期任务（用 Pushgateway 兜底）。
5. Q：Terraform 的 state 丢了会怎样？ A：资源漂移无法感知，可能重复创建/误删——state 必须远程存储 + 锁。
