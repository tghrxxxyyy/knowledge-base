...
### 20.1.1 Expand-Contract 迁移模式

| 阶段 | 操作 | 代码兼容 | 数据库状态 | 回滚难度 |
|------|------|---------|-----------|---------|
| Expand | 新增字段/表 | 新旧代码兼容 | 双写 | 低 |
| Migrate | 数据迁移 | 新旧代码兼容 | 双写+校验 | 中 |
| Contract | 删除旧字段 | 新代码独占 | 新结构 | 高 |
...
-- 数据迁移（后台任务）
UPDATE users 
SET email_verified = TRUE, verification_token = 'legacy'
WHERE verification_token IS NULL;

-- Contract 阶段：删除旧字段（新代码上线后）
ALTER TABLE users DROP COLUMN legacy_verification;
```
...
### 20.3.1 Pod Disruption Budget 配置
...
spec:
  ...
  minReadySeconds: 10    # Pod就绪后等待10秒
  progressDeadlineSeconds: 600
  template:
    spec:
      terminationGracePeriodSeconds: 60
...
| 窗口类型 | 时间段 | 适用场景 | 审批要求 | 回滚SLA |
|---------|--------|---------|---------|---------|
| 常规窗口 | 工作日 10:00-16:00 | 低风险变更 | 自动审批 | 30分钟 |
| 扩展窗口 | 工作日 08:00-20:00 | 中风险变更 | TL审批 | 15分钟 |
| 紧急窗口 | 任意时间 | 紧急修复 | CTO审批 | 5分钟 |
| 冻结期 | 无 | 节假日/大促 | 禁止发布 | N/A |
...
| 演练项目 | 演练频率 | 执行步骤 | 成功标准 | 负责人 |
|---------|---------|---------|---------|--------|
| 蓝绿切回 | 每周 | 切换DNS/验证 | 30秒内完成 | SRE |
| 金丝雀置0 | 每周 | 停止金丝雀/验证 | 15秒内完成 | SRE |
| 滚动undo | 每周 | 回滚Deployment | 60秒内完成 | 开发 |
| 数据库回滚 | 每月 | 执行回滚脚本 | 5分钟内完成 | DBA |
| 全量回滚 | 每季度 | 完整回滚流程 | 10分钟内完成 | 团队 |
...
rollback_sop:
  ...
  blue_green:
    steps:
      - name: "切换DNS"
        command: "aws route53 change-resource-record-sets --hosted-zone-id Z123 --change-batch file://rollback.json"
        timeout: 30s
      - name: "验证健康检查"
        command: "curl -f http://blue.example.com/health"
        timeout: 10s
      - name: "清理资源"
  ...
  canary:
    steps:
      - name: "停止金丝雀"
        command: "kubectl patch deployment my-app -p '{\"spec\":{\"strategy\":{\"rollingUpdate\":{\"maxSurge\":0}}}}'"
        timeout: 30s
      - name: "回滚到稳定版"
...
```mermaid
graph TB
    A[新版本准备部署] --> B{是否有数据库变更?}
    B -->|是| C{是否破坏性变更?}
    B -->|否| D{是否高风险?}
    C -->|是| E[Expand-Contract + 蓝绿]
    C -->|否| F[滚动更新 + PDB]
    D -->|是| G[金丝雀部署]
    D -->|否| H{是否需要验证?}
    H -->|是| I[影子流量]
...
| 阶段 | 时间占比 | 优化目标 | 具体措施 |
|------|---------|---------|---------|
| 检测时间 | 30% | < 1分钟 | 监控告警、自动检测 |
| 响应时间 | 20% | < 2分钟 | 自动化流程、SOP |
| 诊断时间 | 30% | < 3分钟 | 日志聚合、链路追踪 |
| 修复时间 | 20% | < 4分钟 | 自动回滚、快速切换 |
...
|------|---------|--------|---------|
| 部署频率 | 部署次数/天 | > 10次 | < 1次 |
| 变更前置时间 | 代码提交到部署时间 | < 1小时 | > 4小时 |
| 变更失败率 | 失败部署/总部署 | < 5% | > 10% |
| MTTR | 故障恢复时间 | < 30分钟 | > 1小时 |
| 回滚率 | 回滚次数/总部署 | < 3% | > 5% |
...
dashboard:
  ...
  panels:
    ...
    - title: "变更失败率"
      type: "gauge"
      query: "rate(deployments_failed_total[24h]) / rate(deployments_total[24h])"
      thresholds:
        - value: 0
          color: "green"
        - value: 0.05
    ...
    - title: "MTTR"
      type: "stat"
      query: "avg(time_to_remediate_seconds)"
      unit: "s"
      thresholds:
        - value: 0
          color: "green"
...
| 检查项 | 检查方式 | 阻断级别 | 处理建议 |
|--------|---------|---------|---------|
| 代码质量 | SonarQube | 阻断 | 修复后重新提交 |
| 单元测试 | Jest/Pytest | 阻断 | 修复后重新提交 |
| 集成测试 | Cypress | 阻断 | 修复后重新提交 |
| 安全扫描 | Snyk/Trivy | 阻断 | 修复后重新提交 |
| 性能测试 | JMeter | 警告 | 评估后决定 |
| 依赖检查 | Dependabot | 警告 | 更新依赖 |
| 配置检查 | OPA/Kyverno | 阻断 | 修正配置 |
...
environment_consistency:
  ...
  # 容器镜像
  container:
    base_image: "company/base-image:2.0"
...
graph TB
    A[环境一致性检查] --> B{基础设施差异?}
    B -->|是| C[Terraform apply]
    B -->|否| D{配置差异?}
    D -->|是| E[Ansible playbook]
    D -->|否| F{镜像差异?}
    F -->|是| G[重新构建部署]
    F -->|否| H[环境一致]
    C --> I[验证配置]
