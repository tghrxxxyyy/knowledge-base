...
| 指标类型 | 指标名称 | 计算方式 | 目标值 |
|----------|----------|----------|--------|
| 效率 | 构建成功率 | 成功构建/总构建 | >95% |
| 效率 | 平均构建时间 | 总构建时间/构建次数 | <10分钟 |
| 效率 | 平均部署时间 | 总部署时间/部署次数 | <5分钟 |
| 质量 | 变更失败率 | 失败部署/总部署 | <5% |
| 质量 | 回滚率 | 回滚次数/总部署 | <2% |
| 稳定 | MTTR | 平均恢复时间 | <30分钟 |
| 稳定 | 可用性 | 正常时间/总时间 | >99.9% |
...
| 方案 | 描述 | 优点 | 缺点 |
|------|------|------|------|
| 文件版本化 | .gitlab-ci.yml跟随代码 | 简单 | 无历史对比 |
| 模板化 | 公共模板独立管理 | 复用性强 | 维护成本高 |
| 工具化 | 流水线即代码（Pulumi） | 灵活 | 学习曲线陡 |
| 平台化 | CI/CD平台内置版本 | 集成度高 | 厂商锁定 |

### 26.2 流水线模板化实践

```yaml
# 公共模板：.gitlab/ci-templates/build.yml
.build-template:
  stage: build
  script:
...
| 扫描类型 | 工具 | 适用对象 | 阻断级别 |
|----------|------|----------|----------|
| SAST | Semgrep/Checkmarx | 源代码 | CRITICAL |
| SCA | Trivy/Snyk | 依赖包 | HIGH |
| DAST | OWASP ZAP | 运行时 | HIGH |
| Container | Trivy/Grype | 镜像 | CRITICAL |
| IaC | Checkov/Terraform | 配置文件 | MEDIUM |
...
| 指标 | 数据源 | 告警阈值 |
|------|--------|----------|
| 构建时长 | Jenkins/GitLab CI | > 30 分钟 |
| 成功率 | Jenkins/GitLab CI | < 90% |
| 测试覆盖率 | SonarQube | < 80% |
| 漏洞数量 | Trivy/Snyk | > 0 CRITICAL |
| 部署频率 | ArgoCD | < 每周 1 次 |
...
| 策略 | 方法 | 效果 |
|------|------|------|
| 缓存优化 | 依赖缓存、构建缓存 | -50% 构建时间 |
| 并行执行 | 测试并行、构建并行 | -30% 总时间 |
| 资源调度 | 低峰构建、Spot 实例 | -60% 成本 |
| 清理策略 | 及时清理构建产物 | -40% 存储 |
...
metrics:
  - name: pipeline_cost
    ...
    value: $CI_JOB_DURATION * $CI_PIPELINE_COST_PER_SECOND
```
...
安全左移：
  阶段        安全措施              工具
  ─────────────────────────────────────────
  编码阶段   代码规范检查          ESLint/Checkstyle
  提交阶段   预提交钩子           Husky/Prettier
  构建阶段   SAST扫描             SonarQube/Snyk
  测试阶段   单元测试安全         OWASP测试
  部署阶段   镜像扫描             Trivy/Clair
  运行阶段   DAST扫描             OWASP ZAP
```
...
安全扫描:
  ...
  variables:
    SAST_DISABLED: "false"
    DEPENDENCY扫描_ENABLED: "true"
    LICENSE扫描_ENABLED: "true"
  script:
    - sast --include-sast-sast.yml
    - dependency扫描 --include-dependency-scanning.yml
    - license扫描 --include-license-scanning.yml
  artifacts:
    reports:
...
| 支柱 | 数据 | 工具 | 用途 |
|------|------|------|------|
| 指标 | 数值 | Prometheus/Grafana | 趋势分析 |
| 日志 | 文本 | ELK/Loki | 问题排查 |
| 链路 | 追踪 | Jaeger/Tempo | 调用链分析 |
...
| 策略 | 方法 | 效果 | 实施难度 |
|------|------|------|---------|
| 缓存优化 | 依赖缓存、构建缓存 | -50%构建时间 | 低 |
| 并行执行 | 测试并行、构建并行 | -30%总时间 | 中 |
| 资源调度 | 低峰构建、Spot实例 | -60%成本 | 高 |
| 清理策略 | 及时清理构建产物 | -40%存储 | 低 |
| 资源限制 | CPU/内存限制 | -30%成本 | 中 |
...
metrics:
  ...
  - name: pipeline_success_rate
    ...
    labels:
      project: $CI_PROJECT_NAME
    value: $CI_PIPELINE_SUCCESS / $CI_PIPELINE_TOTAL
```
...
test:
  stage: test
  script:
    - pytest --forked -n auto  # pytest-xdist并行
  parallel: 4  # GitLab并行任务
  tags:
    - docker
...
test:
  strategy:
    matrix:
      shard: [1, 2, 3, 4]
  steps:
    - run: pytest --shard-id=$shard --num-shards=4
```
...
| 实践 | 说明 | 收益 | 优先级 |
|------|------|------|--------|
| 模板化 | Golden Pipeline模板 | 统一标准 | P0 |
| 质量门 | 分层拦截 | 质量保证 | P0 |
| 安全门 | SAST/SCA/DAST | 安全保障 | P0 |
| 自动化 | 全流程自动化 | 效率提升 | P0 |
| 可观测 | DORA指标 | 持续改进 | P1 |
| 成本优化 | 缓存/并行/资源调度 | 成本降低 | P1 |
| 环境隔离 | 命名空间/网络/资源 | 风险隔离 | P0 |
| 制品晋级 | snapshot→rc→prod | 发布安全 | P0 |

## Pipeline as Code 最佳实践
...
test:integration:
  stage: test
  script:
    - mvn verify -Dtest.parallel=true
  parallel: 2
...
deploy:blue-green:
  ...
  script:
    # 当前是蓝色环境，部署到绿色环境
    - kubectl set image deployment/myapp-green myapp=$DOCKER_IMAGE:$CI_COMMIT_SHA
    - kubectl rollout status deployment/myapp-green
    
    # 验证绿色环境
    - curl -f http://myapp-green.default.svc.cluster.local/health
...
deploy:canary:
  ...
  script:
    ...
    # 监控金丝雀指标
    - ./monitor-canary.sh --duration=300 --threshold=0.01
...
| 隔离维度 | 实现方式 | 说明 |
|----------|----------|------|
| 命名空间 | Kubernetes Namespace | 逻辑隔离 |
| 网络 | NetworkPolicy | 网络隔离 |
| 资源 | ResourceQuota | 资源隔离 |
| 权限 | RBAC | 权限隔离 |
...
metrics:
  deployment_frequency:
    description: "部署频率"
    query: "count(deployments{env='production'})"
  
  lead_time:
    description: "变更前置时间"
    query: "avg(deployment_duration)"
  
  change_failure_rate:
    description: "变更失败率"
    query: "failed_deployments / total_deployments"
  
  time_to_restore:
    description: "恢复时间"
    query: "avg(incident_recovery_time)"
```

```text
可观测性三支柱：
  Metrics：部署频率、前置时间、失败率、恢复时间
  Logging：构建日志、测试日志、部署日志
  Tracing：流水线执行链路、慢步骤分析
```
...
dast:
  stage: security
  script:
    - zap-api-scan.py -t https://api.example.com -r zap-report.html
```

| 安全实践 | 工具 | 说明 |
|----------|------|------|
| SAST | Semgrep/SonarQube | 静态代码分析 |
| SCA | Snyk/Trivy | 依赖安全扫描 |
| DAST | ZAP/Burp | 动态安全测试 |
| 容器扫描 | Trivy/Grype | 镜像安全扫描 |
| 密钥检测 | GitLeaks/TruffleHog | 密钥泄露检测 |

## 流水线最佳实践 Checklist

- [ ] Pipeline as Code（Git 管理）
- [ ] 模板化（Golden Pipeline）
- [ ] 质量门（单元测试覆盖率>80%）
- [ ] 安全门（SAST/SCA通过）
- [ ] 制品晋级（snapshot→rc→prod）
- [ ] 环境隔离（命名空间/网络）
- [ ] 蓝绿/金丝雀发布
- [ ] 自动回滚机制
- [ ] DORA指标监控
