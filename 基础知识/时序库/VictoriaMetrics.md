...
## VictoriaMetrics 深度实战补充

### Cluster 部署架构详解

```text
VictoriaMetrics Cluster 部署架构：

vmstorage（数据存储层）：
  ├── 无状态协调，靠一致性哈希分片
  ├── 每个实例存储 1/N 的数据
  ├── 支持水平扩展（添加节点自动分片）
  ├── 数据存储：WAL + Block（类似 LSM-Tree）
  └── 副本：每个数据块存储多副本

vminsert（写入代理层）：
  ├── 无状态，接收 remote_write 请求
  ├── 按指标名哈希路由到对应 vmstorage
  ├── 支持水平扩展（无状态，随便加）
  ├── 协议转换：Prometheus/InfluxDB/Graphite/OpenTSDB
  └── 负载均衡：一致性哈希分片
...
1. 批量写入：
   ├── remote_write 批量发送（max_samples_per_send=10000）
   ├── 减少网络往返（RTT）
   └── 降低 vmstorage 写入压力
...
2. 写入缓冲：
   ...
   ├── 内存缓冲：批量刷盘（减少磁盘 IO）
   └── 异步写入：写入 WAL 即返回（不等刷盘）

3. 压缩优化：
   ├── Gorilla 编码：时间戳和值差值压缩
   ├── 列式存储：按列压缩（相似数据一起压缩）
   └── 压缩比：典型 10:1（vs Prometheus 2:1）
...
1. PromQL 执行：
   ├── 解析 PromQL 表达式
   ├── 执行查询计划（并行扫描各 vmstorage）
   └── 合并结果（去重、聚合）

2. 查询缓存：
   ├── 结果缓存：相同查询结果缓存（可配置 TTL）
   ├── 块缓存：热数据块在内存中
   └── 索引缓存：热数据索引在内存中
...
1. 去重时机：
   ├── 写入时去重：vminsert 层去重（可选）
   └── 查询时去重：vmselect 层去重（默认）

2. 去重规则：
   ├── 相同 label 组合 + 时间戳差 < minScrapeInterval
   ├── 保留第一个数据点（先到先得）
   └── 去重发生在查询时，不影响存储

3. 配置参数：
   ├── -dedup.minScrapeInterval=30s（默认）
   ├── 设置太小：去重效果差（重复数据多）
   └── 设置太大：可能丢数据（高频采集）

4. 去重效果：
   ├── 多 Prometheus 写入同一目标：去重
   ├── 同一目标多次抓取：去重
   └── 不同目标抓取同一指标：不去重（标签不同）
```
...
4. 降采样效果：
   ├── 5m 降采样：存储节省 ~50%，精度损失低
   ├── 1h 降采样：存储节省 ~95%，精度损失中
   └── 1d 降采样：存储节省 ~99%，精度损失高
```
...
1. 存储容量：
   每样本大小 ≈ 1-2 字节（压缩后）
   存储 = 样本数 × 每样本大小 × 保留天数 × 副本数
   
   示例：
     100 万时间序列 × 每秒 1 样本 × 86400 秒 × 30 天
     = 2.59 万亿样本 × 2 字节
     ≈ 5.2 TB（原始数据）
     ÷ 10 压缩比
     ≈ 520 GB（压缩后）
...
1. 指标暴露：
   ...
   关键指标：
     ...
     vm_slow_queries_total：慢查询数
     vm_active_merges：活跃合并数
     vm_parts：数据块数量
     vm_memory_size_bytes：内存使用

2. 告警规则：
   写入延迟高：
     - vm_rows_inserted_total rate > 阈值
   
   查询延迟高：
     - vm_slow_queries_total rate > 阈值
   
   磁盘空间不足：
     - vm_disk_space_bytes / vm_disk_space_total < 0.8
