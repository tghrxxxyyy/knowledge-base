...
| 工具 | 层级 | 数据类型 | 开销 | 适用场景 |
|------|------|----------|------|----------|
| perf | 内核级 | 硬件计数器 | 极低 | CPU微架构分析 |
| strace | 系统调用 | 系统调用跟踪 | 中 | 系统调用分析 |
| ltrace | 库调用 | 库函数调用 | 中 | 动态库调用分析 |
| gdb | 调试级 | 断点/单步 | 高 | 程序调试 |
| valgrind | 模拟级 | 内存/缓存 | 极高 | 内存问题检测 |

### perf 工具使用

```bash
# CPU性能分析
perf stat -p <pid> sleep 10  # 统计性能计数器
perf record -g -p <pid> sleep 10  # 采样调用栈
perf report  # 分析报告
...
## 二十二、进程 vs 线程 vs 协程（资源消耗 / 调度方式 / 适用场景）

| 维度 | 进程 | 线程 | 协程 |
|------|------|------|------|
| 资源分配 | 独立地址空间（~数 MB 页表） | 共享进程地址空间（仅栈独立） | 用户态栈（~数 KB） |
| 调度方式 | OS 内核调度（上下文切换 ~1-10μs） | OS 内核调度（切换 ~0.1-1μs） | 用户态 runtime 调度（切换 ~10-100ns） |
| 创建开销 | 高（fork/COW 页表复制） | 中（clone 共享地址空间） | 极低（分配小栈即可） |
| 通信方式 | IPC（管道/共享内存/消息队列） | 共享内存（需锁/原子操作） | 共享变量（单线程内无锁） |
| 适用场景 | 重量级隔离（浏览器多进程） | 通用并发（Java/C++ 多线程） | 高并发 I/O（Go goroutine/Kotlin 协程） |
| 典型限制 | 数百个 | 数千个 | 数十万~百万个 |

```text
资源消耗对比（以 10000 并发连接为例）：
  进程模型：10000 × (栈 8MB + 页表 1MB) ≈ 90GB RAM → 不现实
  线程模型：10000 × (栈 1MB) ≈ 10GB RAM → 可行但压力大
  协程模型：10000 × (栈 8KB) ≈ 80MB RAM → 轻松承载
```

> **工程建议**：I/O 密集型（Web 服务器/网关）用协程；CPU 密集型计算用线程；需要强隔离（多租户/安全沙箱）用进程。

## 二十三、虚拟内存（页表 / TLB / 缺页中断 / Copy-on-Write）

### 页表结构

```text
x86-64 四级页表：
  虚拟地址（48位）→ PML4(9b) → PDPT(9b) → PD(9b) → PT(9b) → Offset(12b)

  每级 512 个条目，每个条目 8 字节
  一级页表覆盖 512GB，四级覆盖 256TB 虚拟地址空间
  CR3 寄存器 → 当前进程 PML4 基地址（进程切换时加载）
```
...
| TLB 层级 | 容量 | 命中延迟 | 说明 |
|----------|------|----------|------|
| L1 dTLB | 64 项 | 1 周期 | 数据 TLB |
| L1 iTLB | 128 项 | 1 周期 | 指令 TLB |
| L2 sTLB | 1024-2048 项 | 7 周期 | 统一 TLB |

```text
TLB Miss 处理：
  1. 检查页表缓存（PTE Cache）→ 命中则更新 TLB
  2. 未命中 → 硬件 Page Walk 遍历四级页表
  3. 遍历成功 → 更新 TLB + PTE Cache
  4. 页面不在内存 → 触发缺页中断
```
...
fork() 时父子进程共享物理页（标记为只读）
  → 任一写入时触发缺页中断
  → 内核复制该页，更新页表为可写
  → 优化：fork 后立即 exec 不会复制整个地址空间

Linux 大页 THP（Transparent Huge Pages）：
  普通页 4KB → THP 2MB/1GB
  减少 TLB Miss，但可能引起延迟抖动
  数据库/Redis 推荐关闭 THP（用静态 HugePages）
```

## 二十四、CPU 缓存（多级缓存 / 伪共享 / 缓存行对齐）

### 多级缓存架构

```text
CPU 缓存层次：
  L1d（32-64KB/核，~1ns）→ L2（256KB-1MB/核，~3-5ns）→ L3（共享 4-64MB，~10-20ns）→ 主存（~100ns）

  缓存行（Cache Line）：64 字节（x86）/ 128 字节（Apple Silicon）
  读取单位：按 Cache Line 整行从内存加载
```

### 伪共享（False Sharing）

```text
问题：两个核心修改同一 Cache Line 的不同变量
  → MESI 协议频繁 invalidate → 性能下降 10-100 倍

解决：Cache Line 填充对齐
  @Contended（Java 8+）：自动填充至 64 字节边界
  手动填充：long padding[7]; // 填充 56 字节（64-8=56）
```

```java
// 伪共享示例
class Counter {
    volatile long a;  // Core 0 修改
    volatile long b;  // Core 1 修改
    // a 和 b 在同一 Cache Line → 伪共享
}
...
| 数据结构 | 对齐策略 | 效果 |
|----------|----------|------|
| 热点计数器 | @Contended 或手动填充 | 消除伪共享 |
| 消息队列环形缓冲 | 缓存行对齐的 head/tail | 无锁高效 |
| 哈希表桶 | 按 Cache Line 对齐分配 | 减少冲突 |
| 数组遍历 | 行优先 + 对齐首地址 | 最大化空间局部性 |

## 二十五、内存对齐（结构体 / 陷阱 / 性能影响）
...
struct Bad {           // 16 字节
    char a;            // 1 字节 + 3 填充
    int b;             // 4 字节
    char c;            // 1 字节 + 3 填充（对齐到 8）
    long d;            // 8 字节
};

struct Good {          // 16 字节（无填充浪费）
    long d;            // 8 字节
    int b;             // 4 字节
    char a;            // 1 字节
    char c;            // 1 字节 + 2 填充（对齐到 8）
};
```
...
对齐规则：
  ...
  3. 未对齐访问 → 部分 CPU 触发异常（ARM）或性能惩罚（x86）

性能影响：
  未对齐读取：x86 硬件自动处理但有 ~2x 延迟
  跨 Cache Line 访问：两次 Cache Line 加载 → ~2x 延迟
  优化手段：按大小降序排列成员，避免不必要的填充
```

## 二十六、I/O 模型（阻塞 / 非阻塞 / 多路复用 / 事件驱动）

| 模型 | 特点 | 系统调用 | 适用场景 |
|------|------|----------|----------|
| 阻塞 I/O | 等待数据就绪，线程挂起 | read/write | 简单场景 |
| 非阻塞 I/O | 立即返回 EAGAIN，轮询检查 | read(O_NONBLOCK) | 低并发 |
| I/O 多路复用 | select/poll/epoll 监听多个 fd | epoll_wait | 高并发网络 |
| 信号驱动 I/O | 内核就绪时发 SIGIO 信号 | signal | 特殊场景 |
| 异步 I/O（AIO） | 内核完成数据拷贝后通知 | io_submit/io_getevents | 高性能批量 |

```text
epoll 三系统调用：
  epoll_create：创建 epoll 实例（红黑树 + 就绪链表）
  epoll_ctl：注册/修改/删除监听 fd
  epoll_wait：等待事件（阻塞或超时）

epoll vs select/poll：
  select：fd 上限 1024，每次调用拷贝全部 fd 集合 → O(n)
  poll：无 fd 上限，但仍需遍历 → O(n)
  epoll：事件驱动，内核维护红黑树 → O(1) 就绪通知

ET（Edge Trigger）：状态变化触发，高效但需一次读完
LT（Level Trigger）：只要就绪就触发，简单但可能重复通知
```

## 二十七、并发原语（原子操作 / CAS / 锁 / 信号量 / 屏障）

| 原语 | 原理 | 性能 | 适用 |
|------|------|------|------|
| 原子操作 | CPU 指令级不可分割（CAS/LL-SC） | 极高（~ns） | 计数器/标志位 |
| 自旋锁 | 忙等（CAS 循环） | 无上下文切换，但占 CPU | 极短临界区 |
| 互斥锁 | 内核态等待（futex） | 有上下文切换 | 通用临界区 |
| 读写锁 | 读共享/写互斥 | 读多写少场景高效 | 缓存/配置 |
| 信号量 | 计数器 + 等待队列 | 控制并发度 | 资源池/限流 |
| 屏障（Barrier） | 等待所有线程到达 | 同步点 | 并行计算 |

```java
// CAS（Compare-And-Swap）原理
// 原子地比较并交换：if (value == expected) value = newValue;
AtomicInteger counter = new AtomicInteger(0);
counter.compareAndSet(0, 1);  // 原子操作，失败则重试

// ABA 问题：值从 A→B→A，CAS 误判未修改
// 解决：AtomicStampedRef（加版本号）
...
## 二十八、编译优化（内联 / 常量折叠 / 循环展开 / 分支预测）

| 优化技术 | 原理 | 效果 | 风险 |
|----------|------|------|------|
| 内联（Inline） | 函数体替换调用 | 消除调用开销 | 代码膨胀 |
| 常量折叠 | 编译期计算常量表达式 | 消除运行时计算 | 无 |
| 循环展开 | 减少循环控制开销 | 提高指令级并行 | 代码膨胀 |
| 分支预测提示 | likely/unlikely 注解 | 优化分支预测 | 预测错有惩罚 |
| PGO | 基于 profile 数据优化 | 10-30% 性能提升 | 需要测试数据 |
...
// 常量折叠
int x = 3 * 4 + 2;  // 编译期直接计算为 14
```

## 二十九、安全基础（ASLR / DEP / 栈溢出 / Spectre / Meltdown）

| 安全机制 | 原理 | 防护目标 |
|----------|------|----------|
| ASLR | 地址空间随机化（堆/栈/库基址随机） | 缓冲区溢出 |
| DEP/NX | 数据页不可执行 | 代码注入攻击 |
| Stack Canary | 栈帧末尾放置随机值 | 栈溢出攻击 |
| PIE | 可执行文件地址随机 | ROP 攻击 |
| Spectre | 分支预测 + 推测执行侧信道 | 跨进程信息泄露 |
| Meltdown | 乱序执行越权读内核内存 | 内核数据泄露 |

```text
Spectre/Meltdown 应对：
  内核补丁：KPTI（Kernel Page Table Isolation）→ 用户态/内核态页表分离
  性能影响：系统调用密集型应用 ~5-30% 性能下降
  编译器：LFENCE（限制推测执行范围）
  硬件：新 CPU 修复推测执行漏洞
```

## 三十、性能分析（CPU / 内存 / I/O / 缓存命中率）

| 工具 | 用途 | 关键指标 |
|------|------|----------|
| perf stat | CPU 硬件计数器 | 指令数/CPI/缓存命中率 |
| perf record/report | CPU 热点分析 | 函数调用栈/热点指令 |
| valgrind --tool=memcheck | 内存泄漏检测 | 未初始化/越界/泄漏 |
| valgrind --tool=cachegrind | 缓存命中率分析 | L1/L2/L3 命中率 |
| iostat | 磁盘 I/O 分析 | IOPS/吞吐/延迟/队列深度 |
| vmstat | 系统级概览 | CPU/内存/IO/b 上下文切换 |
...
# cachegrind 缓存模拟
valgrind --tool=cachegrind --cachegrind-out-file=cg.out ./myapp
cg_annotate cg.out
...
## 三十一、计算机组成（CPU / 内存 / 磁盘 / 网络 / 总线）

```mermaid
flowchart TD
    subgraph CPU
        CORE0[Core 0] --- L1_0[L1 Cache]
        CORE1[Core 1] --- L1_1[L1 Cache]
        L1_0 --- L2_0[L2 Cache]
        L1_1 --- L2_1[L2 Cache]
        L2_0 --- L3[L3 Cache 共享]
        L2_1 --- L3
...
| 组件 | 关键指标 | 典型带宽 |
|------|----------|----------|
| L1 Cache | 延迟 ~1ns，带宽 ~100GB/s | 每核独立 |
| L2 Cache | 延迟 ~3ns，带宽 ~50GB/s | 每核独立 |
| L3 Cache | 延迟 ~10ns，带宽 ~200GB/s | 多核共享 |
| DDR4/DDR5 | 延迟 ~100ns，带宽 ~25-50GB/s | 所有核共享 |
| NVMe SSD | 延迟 ~10-100μs，带宽 ~3-7GB/s | PCIe 4.0 x4 |
| HDD | 延迟 ~5-10ms，带宽 ~200MB/s | SATA |
| 网络 | 延迟 ~100μs（同机房），带宽 10-100Gbps | NIC |
