...
自我总结注意了，只要你了解了HashMap，那么其实cchmap的put和get你就知道了，get一样，put无非就是加了“锁”控制，sync、cas，包括保证可见性的entry的value为volatile,  核心需要掌握的是扩容和size的计算
...
作者的思路就是尽量避免竞争，cas修改baseCount成功就不会再去修改CounterCell[]，修改失败也不会自旋，以哈希映射的方式找到CounterCell[]对应位置的格子cas计数，依然失败就多次重复哈希映射找其他空闲格子，还失败就扩容CounterCell[]，扩容之后都竞争不过其他线程，此时就进入自旋重复哈希映射，直到修改成功，虽然最终可能也会陷入不断自旋重试的情况，但是多个线程抢多个资源和多个线程抢一个资源相比，性能明显会好很多
