...
| 数据结构 | 插入性能 | 查询性能 | 删除性能 | 内存占用 | 并发性能 |
|----------|----------|----------|----------|----------|----------|
| HashMap | O(1) | O(1) | O(1) | 高 | 低（需同步） |
| TreeMap | O(log n) | O(log n) | O(log n) | 中 | 中 |
| ConcurrentHashMap | O(1) | O(1) | O(1) | 高 | 高 |
| ConcurrentSkipListMap | O(log n) | O(log n) | O(log n) | 中 | 高 |
| CopyOnWriteArrayList | O(n) | O(1) | O(n) | 高 | 读高写低 |
| BlockingQueue | O(1) | O(1) | O(1) | 中 | 高 |
...
public class SkipList<T> {
    ...
    public void insert(T value) {
        ...
        Node<T>[] update = new Node[level + 1];
        Node<T> current = head;
...
| 元素数量 | 误判率1% | 误判率0.1% | 误判率0.01% |
|----------|----------|------------|-------------|
| 100万 | 1.2MB | 1.8MB | 2.4MB |
| 1000万 | 12MB | 18MB | 24MB |
| 1亿 | 120MB | 180MB | 240MB |
| 10亿 | 1.2GB | 1.8GB | 2.4GB |
...
| 维度 | 红黑树 | AVL树 | 选型建议 |
|------|--------|-------|----------|
| 平衡严格度 | 近似平衡 | 严格平衡 | 频繁查找选AVL |
| 插入旋转次数 | ≤2次 | ≤2次 | 无差异 |
| 删除旋转次数 | ≤3次 | O(log n) | 红黑树更优 |
| 查询性能 | O(log n) | O(log n) | AVL略快 |
| 插入性能 | O(log n) | O(log n) | 红黑树略快 |
| 内存占用 | 少1bit颜色位 | 多平衡因子 | 红黑树更省 |
| Java实现 | TreeMap | 无内置 | 红黑树方便 |
