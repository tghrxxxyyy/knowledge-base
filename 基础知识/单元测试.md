...
|------|------|----------|------|
| 行覆盖 | 代码行执行 | 执行行/总行 | >80% |
| 分支覆盖 | 条件分支 | 执行分支/总分支 | >70% |
| 函数覆盖 | 函数调用 | 调用函数/总函数 | >90% |
| 条件覆盖 | 逻辑条件 | 条件组合 | >70% |
...
// 使用
User user = new UserBuilder().withId(100L).withName("Alice").build();
```
...
| 框架 | 语言 | 特点 | 适用场景 |
|------|------|------|----------|
| Java Faker | Java | 逼真数据 | 测试数据生成 |
| Instancio | Java | 类型安全 | 复杂对象 |
| FactoryBot | Ruby | Rails集成 | Ruby项目 |
| Bogus | C# | .NET支持 | .NET项目 |

---

## 十八、Testcontainers 集成测试
...
| 容器 | 用途 | 启动时间 |
|------|------|----------|
| PostgreSQL | 关系数据库 | 5-10s |
| MySQL | 关系数据库 | 5-10s |
| Redis | 缓存 | 1-2s |
| Kafka | 消息队列 | 10-15s |
| Elasticsearch | 搜索引擎 | 10-20s |
...
| 策略 | 说明 | 适用场景 |
|------|------|----------|
| @BeforeEach | 每个测试前 | 独立测试 |
| @BeforeAll | 所有测试前 | 昂贵操作 |
| @AfterEach | 每个测试后 | 清理资源 |
| @AfterAll | 所有测试后 | 最终清理 |
...
| 风格 | 示例 | 优点 | 缺点 |
|------|------|------|------|
| should + 动词 | shouldReturnUser | 可读性好 | 较长 |
| Given-When-Then | givenUser_whenFind_thenReturn | BDD风格 | 冗长 |
| test + 方法名 | testFindById | 简单直接 | 信息少 |
| descriptive | userNotFoundThrowsException | 描述行为 | 不统一 |
...
```mermaid
graph LR
    A[Red<br/>写失败测试] --> B[Green<br/>写最少代码]
...
void shouldCalculateTotalPrice() {
    ...
    cart.add(new Product("Pen", 2.0));
    assertEquals(12.0, cart.getTotalPrice(), 0.001);
}
...
| 层级 | 占比 | 工具 | 执行速度 | 维护成本 |
|------|------|------|---------|---------|
| 单元测试 | 70-80% | JUnit/Mockito | <100ms | 低 |
| 集成测试 | 15-20% | Testcontainers | 1-10s | 中 |
| E2E测试 | 5-10% | Selenium/Cypress | 10-60s | 高 |
| 契约测试 | 5% | Pact/Contract | 1-5s | 中 |

```mermaid
flowchart TD
    A[测试金字塔] --> B[单元测试 70-80%]
    A --> C[集成测试 15-20%]
    A --> D[E2E测试 5-10%]
    A --> E[契约测试 5%]
    B --> F[快速反馈<br/>毫秒级]
...
public class TestDataExtension implements BeforeEachCallback, AfterEachCallback {
    ...
    public void beforeEach(ExtensionContext context) {
        ...
        context.getStore(NAMESPACE).put("testUser", user);
    }
    
    @Override
    public void afterEach(ExtensionContext context) {
        // 每个测试后清理
        TestUser user = context.getStore(NAMESPACE).get("testUser", TestUser.class);
        TestDataFactory.cleanup(user.getId());
    }
}
...
class UserServiceTest {
    ...
    void shouldCreateOrder() {
        TestUser user = getTestUser();  // 从扩展获取
        // ...
    }
}
```
...
| 场景 | 推荐方式 | 示例 |
|------|---------|------|
| 依赖返回值 | when().thenReturn() | when(repo.find(1L)).thenReturn(user) |
| 依赖抛异常 | when().thenThrow() | when(repo.find(1L)).thenThrow(NotFoundException) |
| 依赖无返回值 | doNothing().when() | doNothing().when(repo).delete(1L) |
| 参数匹配 | any()/eq() | when(repo.find(any(Long.class))).thenReturn(user) |
| 连续调用 | thenReturn().thenReturn() | 多次调用返回不同值 |
| 验证调用次数 | verify().times() | verify(repo, times(2)).save(any()) |
...
// 使用
User admin = aUser().withRole(Role.ADMIN).withName("Admin").build();
User regular = aUser().build();  // 默认值
```
...
| 模式 | 命名格式 | 示例 |
|------|---------|------|
| Given-When-Then | should{行为}_when{条件} | shouldReturnUser_whenExists |
| 状态断言 | should{期望状态} | shouldHaveEmptyCart |
| 异常断言 | should{动作}_when{异常条件} | shouldThrowException_whenNotFound |
| 行为验证 | should{验证行为} | shouldCallRepository_save |

---

## 十六、测试金字塔 vs 蜂巢 vs 奖杯模型
...
flowchart TD
    ...
    B -->|微服务| D[测试蜂巢]
    B -->|大型项目| E[测试奖杯]
    C --> F[80%单元/15%集成/5%E2E]
    D --> G[60%单元/30%集成/10%E2E]
    E --> H[50%单元/20%集成/20%静态/10%E2E]
```
...
| 扩展点 | 用途 | 示例 |
|--------|------|------|
| `@ExtendWith` | 注册扩展 | `@ExtendWith(MockitoExtension.class)` |
| `BeforeAllCallback` | 所有测试前执行 | 初始化资源 |
| `AfterAllCallback` | 所有测试后执行 | 清理资源 |
| `BeforeEachCallback` | 每个测试前执行 | 重置状态 |
| `AfterEachCallback` | 每个测试后执行 | 记录日志 |
| `TestExecutionListener` | 测试执行监听 | 自定义报告 |
...
class UserRepositoryTest {
    ...
    void shouldSaveUser() {
        // 使用真实数据库测试
        User user = repository.save(new User("test"));
        assertNotNull(user.getId());
    }
}
```
...
| 容器 | 用途 | 镜像 |
|------|------|------|
| PostgreSQL | 数据库测试 | postgres:15 |
| MySQL | 数据库测试 | mysql:8.0 |
| Redis | 缓存测试 | redis:7 |
| Kafka | 消息测试 | confluentinc/cp-kafka |
| RabbitMQ | 消息测试 | rabbitmq:3-management |
| Elasticsearch | 搜索测试 | elasticsearch:8.x |
...
| 策略 | 适用场景 | 实现方式 |
|------|---------|---------|
| `@BeforeEach` | 每个测试前重置 | 重置数据库/缓存 |
| `@AfterEach` | 每个测试后清理 | 删除临时文件 |
| `@BeforeAll` | 所有测试前初始化 | 初始化容器 |
| `@AfterAll` | 所有测试后清理 | 停止容器 |
| `@Transactional` | 事务回滚 | 自动回滚 |
| `@Rollback` | 显式回滚 | 手动回滚 |
...
class UserServiceTest {
    ...
    void shouldSaveUser() {
        // 事务自动回滚
        User user = service.save(new User("test"));
        assertNotNull(user.getId());
    }
