# 大数据场景设计实战

## 〇、本体介绍

**大数据场景设计**：把大数据技术栈落到**真实业务场景**——实时大屏、用户画像、实时风控、数据中台、数仓迁移。每个场景都是**技术选型 + 架构设计 + 权衡取舍**的综合体。

**核心矛盾**：
1. 场景差异大：离线报表要准确、实时风控要快、用户画像要全；
2. 同一技术栈（Flink/Iceberg/Doris）在不同场景的**架构组合与调优策略**完全不同；
3. 从 0 到 1 搭建一条生产级数据链路，要同时打通采集、存储、计算、服务四层；
4. 面试高频：大数据场景题是后端/数据工程师的必考点。

**核心主线**：实时大屏 → 用户画像 → 实时风控 → 数据中台建设 → 数仓迁移与架构演进。

---

## 一、场景一：实时大屏 / 实时报表

### 1.1 需求与约束

| 维度 | 典型值 | 设计影响 |
|------|--------|----------|
| 数据源 | 业务库 binlog + 埋点日志 + 消息 | CDC + Kafka 统一入湖 |
| 延迟要求 | 秒级（大屏刷新 1~5s） | 必须流计算，禁 T+1 |
| 查询 QPS | 高（万人同时看大屏） | 结果预计算，禁原始明细查询 |
| 数据准确性 | 允许秒级近似，禁超发 | 最终一致 + 对账 |

### 1.2 架构设计

```mermaid
graph LR
    S1[业务库 binlog] -->|CDC/Debezium| K[Kafka]
    S2[埋点日志] -->|Filebeat/Flume| K
    K --> F[Flink 实时计算]
    F -->|预聚合| D[Doris/StarRocks]
    D -->|查询| V[Grafana/BI 大屏]
    F -->|明细留存| I[Iceberg 数据湖]
```

1. **采集**：binlog → Debezium → Kafka；埋点 → Filebeat → Kafka；统一 Kafka 中枢。
2. **计算**：Flink 消费 Kafka，按**窗口预聚合**（1min/5min/1h），结果写入 OLAP。
3. **存储**：明细落 Iceberg（数据湖，供回溯/对账）；聚合结果写 Doris/StarRocks（OLAP，供查询）。
4. **服务**：BI 工具（Grafana/Superset）直连 OLAP，禁查原始明细。

> 口诀：**"明细入湖做预聚合，结果落 OLAP 供查询；大屏看聚合不对账，对账走湖做回溯。"**

### 1.3 关键技术点

| 技术点 | 做法 |
|--------|------|
| 窗口聚合 | Flink TUMBLE/HOP 窗口，按 event time + watermark |
| 维表打宽 | Flink 时态 join（`FOR SYSTEM_TIME AS OF`）关联 MySQL 维表 |
| 结果更新 | Doris 用 Unique Key 模型（upsert），同 key 覆盖 |
| 对账 | 实时结果 vs 离线 T+1 全量结果，差异告警 |

### 1.4 与「Flink」「OLAP」「实时数仓」的关系

- Flink 是流计算引擎（见「[08](08-流处理计算：Flink.md)」）；
- Doris/StarRocks 是 OLAP 引擎（见「[09](09-数据仓库与OLAP引擎.md)」）；
- 实时数仓是更完整的链路（见「[11](11-实时数仓与湖仓一体.md)」）。

---

## 二、场景二：用户画像 / 标签体系

### 2.1 需求与约束

| 维度 | 典型值 | 设计影响 |
|------|--------|----------|
| 标签类型 | 事实标签（年龄/城市）、统计标签（近30天GMV）、规则标签（是否高价值） | 离线批 + 实时流双写 |
| 更新频率 | 事实标签 T+1、行为标签分钟级 | Lambda 架构（批+流） |
| 存储规模 | 亿级用户 × 千级标签 = 万亿级 KV | 宽表 NoSQL（HBase/ES） |
| 查询模式 | 点查（单用户标签）、圈人（条件组合） | HBase RowKey + ES 倒排 |

### 2.2 架构设计

```mermaid
graph TB
    S[行为日志/业务DB] --> K[Kafka]
    K --> F[Flink 实时标签]
    F --> HBase[HBase/ES 实时标签表]
    K -->|落湖| I[Iceberg 数据湖]
    I --> S2[Spark 离线标签]
    S2 --> HBase
    HBase -->|点查/圈人| U[用户画像服务]
    U -->|人群包| R[推荐/营销系统]
```

1. **实时标签**：Flink 消费 Kafka，计算近 N 分钟行为（如"最近1小时浏览商品数"），写入 HBase/ES。
2. **离线标签**：Spark 每日跑 T+1 全量（如"近30天GMV"、"生命周期阶段"），覆盖写入 HBase/ES。
3. **存储**：HBase RowKey = `userId`（点查），ES 倒排 = 标签组合（圈人）。
4. **服务**：用户画像 API（查单用户标签）、圈人引擎（按标签组合导出人群包）。

> 口诀：**"实时算近因，离线算长期；HBase 做点查，ES 做圈人；批流双写同一张表，口径靠离线校准。"**

### 2.3 Lambda 架构在画像中的落地

| 层 | 引擎 | 职责 |
|----|------|------|
| **速度层** | Flink | 近实时标签（分钟级），写入 HBase/ES |
| **批层** | Spark/Hive | T+1 全量标签，覆盖写入 HBase/ES |
| **服务层** | HBase + ES | 查询时合并（速度层增量 + 批层全量） |

- **关键设计**：速度层和批层写**同一张表**，查询时自然合并；批层全量定期校准速度层增量。
- **RowKey 设计**：HBase 用 `userId`（前缀散列防热点），ES 用 `userId` + 标签字段倒排。

### 2.4 标签类型与计算方式

| 标签类型 | 计算方式 | 更新频率 | 存储 |
|----------|----------|----------|------|
| 事实标签 | 直接读取用户属性 | T+1 | MySQL 维表 |
| 统计标签 | Flink/Spark 聚合 | 实时/T+1 | HBase |
| 规则标签 | 规则引擎判断 | T+1 | HBase/ES |
| 预测标签 | 模型推理（ML） | 离线批量 | HBase |

---

## 三、场景三：实时风控

### 3.1 需求与约束

| 维度 | 典型值 | 设计影响 |
|------|--------|----------|
| 延迟要求 | 毫秒~秒级（交易风控 <100ms） | 流计算 + 内存规则引擎 |
| 数据源 | 交易流 + 设备指纹 + 历史行为 | 多流 join + 维表关联 |
| 规则复杂度 | 简单阈值 → 复杂 CEP（如"1h内3地登录"） | Flink CEP / 规则引擎 |
| 准确性 | 误杀率低（宁可漏过不可误杀） | 规则分层 + 人工复核 |

### 3.2 架构设计

```mermaid
graph LR
    T[交易请求] -->|MQ| F[Flink CEP 规则引擎]
    F -->|查询| R[Redis 特征/计数器]
    F -->|复杂规则| HBase[HBase 历史行为]
    F -->|命中| A[风控决策: 通过/拒绝/人工]
    F -->|异步| K[Kafka 风控日志]
    K --> I[Iceberg 风控数据湖]
    K --> D[Doris 风控报表]
```

1. **规则引擎**：Flink CEP（Complex Event Processing）匹配复杂模式——如"同一用户 1 小时内 3 个不同城市下单"。
2. **特征存储**：Redis 存滑动窗口计数器（如"近1小时订单数"），Flink 状态后端（RocksDB）存长窗口特征。
3. **决策分层**：

| 层级 | 延迟 | 规则类型 | 处置 |
|------|------|----------|------|
| L1 实时 | <100ms | 简单阈值、黑白名单 | 直接通过/拒绝 |
| L2 近实时 | 秒级 | CEP 复杂模式 | 拒绝或人工复核 |
| L3 离线 | T+1 | 模型评分、关联分析 | 事后处置、规则迭代 |

4. **事后分析**：风控日志落 Iceberg（数据湖），Spark 离线分析误杀率、迭代规则。

> 口诀：**"CEP 配复杂模式，Redis 滑窗记特征；实时近实时离线三层，误杀漏杀平衡靠迭代。"**

### 3.3 Flink CEP 示例

```java
// 模式：同一用户 1 小时内 3 个不同城市下单
Pattern<OrderEvent, ?> pattern = Pattern
    .<OrderEvent>begin("first")
        .where(new SimpleCondition<OrderEvent>() {
            public boolean filter(OrderEvent e) { return e.amount > 0; }
        })
    .next("second")
        .where(new IterativeCondition<OrderEvent>() {
            public boolean filter(OrderEvent e, Context<OrderEvent> ctx) {
                for (OrderEvent first : ctx.getEventsForPattern("first")) {
                    if (!e.city.equals(first.city)) return true;
                }
                return false;
            }
        })
    .next("third")
        .where(new IterativeCondition<OrderEvent>() {
            public boolean filter(OrderEvent e, Context<OrderEvent> ctx) {
                // 与 first、second 城市都不同
                return /* ... */;
            }
        })
    .within(Time.hours(1));

// 匹配到 → 触发风控告警
patternStream.select(new PatternSelectFunction<OrderEvent, Alert>() {
    public Alert select(Map<String, List<OrderEvent>> pattern) {
        return new Alert("3-city-1h", pattern.get("first").get(0).userId);
    }
});
```

---

## 四、场景四：数据中台建设

### 4.1 需求与约束

| 维度 | 典型值 | 设计影响 |
|------|--------|----------|
| 数据源 | 多业务系统（订单/商品/用户/物流） | 统一采集 + 数据湖入仓 |
| 服务对象 | 多个业务线（BI/推荐/营销/风控） | 统一指标口径 + 数据服务层 |
| 数据质量 | 跨系统口径不一致 | 指标标准化 + 数据治理 |
| 组织协同 | 数据团队 vs 业务团队 | 数据资产目录 + 数据产品经理 |

### 4.2 分层架构

```mermaid
graph TB
    subgraph 数据源层
        S1[业务DB] S2[日志] S3[第三方API]
    end
    subgraph 采集层
        CDC[CDC/DataX] K[Kafka]
    end
    subgraph 存储计算层
        I[Iceberg 数据湖] DWD[DWD 明细层] DWS[DWS 汇总层] ADS[ADS 应用层]
    end
    subgraph 服务层
        API[数据API] META[元数据/数据地图] MON[数据质量监控]
    end
    subgraph 消费层
        BI[BI报表] REC[推荐] MKT[营销] RISK[风控]
    end

    S1 --> CDC --> K --> I
    S2 --> K
    S3 --> K
    I --> DWD --> DWS --> ADS
    DWS --> API
    DWD --> META
    DWD --> MON
    API --> BI
    API --> REC
    API --> MKT
    RISK --> DWD
```

### 4.3 数仓分层（ODS → DWD → DWS → ADS）

| 层 | 全称 | 内容 | 引擎 |
|----|------|------|------|
| **ODS** | Operational Data Store | 原始数据，贴源 | 数据湖（Iceberg） |
| **DWD** | Data Warehouse Detail | 清洗、标准化、维度关联 | Spark/Flink |
| **DWS** | Data Warehouse Summary | 按主题汇总（用户/商品/订单日聚合） | Spark |
| **ADS** | Application Data Service | 面向应用的宽表/指标 | Doris/StarRocks |

> 口诀：**"ODS 贴源不动，DWD 清洗明细；DWS 汇总主题，ADS 面向应用；指标口径统一管，数据资产目录清。"**

### 4.4 指标体系设计

| 指标类型 | 定义 | 例子 |
|----------|------|------|
| **原子指标** | 不可再分的最小度量 | 订单金额、支付笔数 |
| **派生指标** | 原子指标 + 修饰词 + 时间周期 | 近7天新用户GMV |
| **复合指标** | 多个指标计算 | 客单价 = GMV / 订单数 |

- **指标管理**：统一指标命名规范 + 指标口径文档 + 指标血缘（从 ADS 回溯到 ODS）。
- **口径一致性**：同一指标只在一个 DWS 层计算，ADS 层只读不重算。

---

## 五、场景五：数仓迁移与架构演进

### 5.1 典型演进路径

```mermaid
graph LR
    P1["阶段1: 传统数仓<br/>Oracle/Teradata<br/>单体 T+1"] -->|"数据量暴涨"| P2["阶段2: Hadoop 数仓<br/>Hive on HDFS<br/>T+1 批处理"]
    P2 -->|"实时需求"| P3["阶段3: Lambda 架构<br/>Hive 批 + Flink 流<br/>双链路"]
    P3 --> "|维护成本高"| P4["阶段4: 湖仓一体<br/>Iceberg/Paimon + Flink/Spark<br/>流批一体"]
    P4 --> "|云原生"| P5["阶段5: Serverless 湖仓<br/>存算分离 + 弹性计算<br/>按需付费"]
```

### 5.2 迁移场景：传统数仓 → 湖仓一体

| 阶段 | 现状 | 目标 | 迁移策略 |
|------|------|------|----------|
| **评估** | Oracle/Teradata，T+1 报表 | 实时+离线统一 | 盘点表/任务/依赖 |
| **双写** | 旧库继续服务 | 湖表并行写入 | CDC 同步 + DataX 补历史 |
| **校验** | 新旧结果对比 | 口径一致 | 对账脚本，差异<0.01% |
| **切流** | 旧库只读 | 新库承接全部查询 | 灰度切流，先报表后核心 |
| **下线** | 旧库停写 | 完全迁移 | 观察期后下线 |

### 5.3 双写迁移架构

```mermaid
graph LR
    S[业务DB] -->|CDC/Debezium| K[Kafka]
    K -->|实时| I[Iceberg 数据湖]
    K -->|批量| Spark[Spark 离线]
    Spark --> I
    S -->|存量| DataX[DataX]
    DataX --> I
    I --> D[Doris/StarRocks]
    D --> Q[BI/应用]
    O[旧数仓 Oracle] -->|并行| Q
```

> 口诀：**"先双写后切流，先对账后下线；CDC 同步增量，DataX 补存量；口径一致是底线，灰度切流保安全。"**

### 5.4 迁移 CheckList

- [ ] 盘点：源表清单、任务依赖、下游消费方
- [ ] 双写：CDC 实时 + DataX 存量补录
- [ ] 对账：新旧库逐笔核对，差异阈值 <0.01%
- [ ] 灰度：先切非核心报表，再切核心链路
- [ ] 回滚：旧库保留只读，新库异常可秒切回
- [ ] 下线：观察期（通常 1~3 个月）后下线旧库

---

## 六、场景六：用户行为分析架构

### 6.1 需求与约束

| 维度 | 典型值 | 设计影响 |
|------|--------|----------|
| 数据源 | 埋点日志 + 业务事件 | 统一 Kafka 中枢 |
| 分析维度 | 用户/设备/地域/行为/时间 | 多维分析，OLAP 必须 |
| 延迟要求 | 分钟级（运营看板） | 流计算预聚合 |
| 数据规模 | 日增 10 亿事件 | 分布式存储+计算 |

### 6.2 架构设计

```mermaid
graph LR
    A[埋点 SDK] -->|上报| G[网关]
    G --> K[Kafka]
    K --> F[Flink 实时预聚合]
    F --> D[Doris/StarRocks]
    K -->|落湖| I[Iceberg]
    I --> S[Spark 离线分析]
    S --> D
    D --> BI[BI 看板]
    D --> API[数据 API]
```

## 七、场景七：实时推荐系统

### 7.1 架构设计

```mermaid
graph TB
    A[用户请求] --> R[推荐服务]
    R --> F[特征服务 Redis/HBase]
    R --> M[模型服务 TF Serving]
    R --> RET[召回/排序]
    F --> K[Kafka 实时特征]
    K --> FL[Flink 实时特征计算]
    FL --> F
    A -->|行为| K
```

### 7.2 关键技术点

| 技术点 | 做法 |
|--------|------|
| 特征计算 | Flink 实时计算用户特征（近 N 分钟行为） |
| 特征存储 | Redis 热特征 + HBase 历史特征 |
| 模型服务 | TF Serving/Triton 模型推理 |
| 实时更新 | 用户行为 → Kafka → Flink → 特征更新 → Redis |

## 八、场景八：欺诈检测流水线

### 8.1 三层检测架构

| 层级 | 延迟 | 技术 | 说明 |
|------|------|------|------|
| L1 实时 | <100ms | 规则引擎 | 简单阈值/黑白名单 |
| L2 近实时 | 秒级 | Flink CEP | 复杂模式匹配 |
| L3 离线 | T+1 | Spark ML | 模型评分/关联分析 |

### 8.2 流水线设计

```mermaid
graph LR
    T[交易请求] --> F[Flink 规则引擎]
    F -->|命中| R1[拒绝/人工复核]
    F -->|通过| P[通过]
    F -->|日志| K[Kafka]
    K --> HBase[HBase 特征]
    K --> I[Iceberg 风控湖]
    I --> S[Spark 离线分析]
    S --> M[模型迭代]
    M --> F
```

## 九、场景九：IoT 数据平台

### 9.1 IoT 数据特点

| 特点 | 影响 |
|------|------|
| 海量设备 | 每秒百万事件 |
| 时序数据 | 按时间分区 |
| 低延迟 | 设备状态秒级更新 |
| 重写轻读 | 写多读少 |

### 9.2 架构设计

```mermaid
graph LR
    D[IoT 设备] -->|MQTT| G[MQTT Broker]
    G --> K[Kafka]
    K --> F[Flink 实时处理]
    F --> T[TDengine/InfluxDB 时序]
    F --> HBase[HBase 设备状态]
    F --> I[Iceberg 数据湖]
    I --> S[Spark 离线分析]
```

## 十、场景十：实时大屏架构

### 10.1 大屏架构要点

| 要点 | 说明 |
|------|------|
| 数据延迟 | 1~5 秒刷新 |
| 查询 QPS | 万人同时看 |
| 数据准确性 | 允许秒级近似 |
| 故障降级 | OLAP 故障降级到缓存 |

### 10.2 大屏架构

```mermaid
graph LR
    A[业务库] -->|CDC| K[Kafka]
    K --> F[Flink 预聚合]
    F --> D[Doris/StarRocks]
    D --> V[Grafana 大屏]
    D --> C[结果缓存 Redis]
    C --> V
```

## 十一、数据湖架构模式

### 11.1 典型数据湖架构

```mermaid
graph TB
    subgraph 数据源
        S1[业务 DB] S2[日志] S3[第三方]
    end
    subgraph 采集层
        CDC[CDC] K[Kafka] FB[Filebeat]
    end
    subgraph 存储层
        I[Iceberg 数据湖]
    end
    subgraph 计算层
        F[Flink 流] S[Spark 批]
    end
    subgraph 服务层
        OLAP[Doris/StarRocks] API[数据 API]
    end
    S1 --> CDC --> K
    S2 --> FB --> K
    K --> F --> I
    K --> S --> I
    I --> OLAP --> API
```

## 十二、数据平台容量规划

### 12.1 容量评估方法

| 评估维度 | 计算方式 |
|----------|----------|
| 存储量 | 日增数据量 × 保留天数 × 副本 × 压缩比 |
| 计算量 | 峰值 QPS × 单查询扫描量 |
| 网络带宽 | 峰值流量 × 消息大小 |
| Kafka 容量 | 日增消息量 × 保留天数 × 副本 |

### 12.2 容量规划示例

```
场景：日增 10 亿事件，每事件 1KB

Kafka 容量：
  10 亿 × 1KB = 1TB/天
  保留 3 天 × 3 副本 = 9TB
  压缩（5:1）= 1.8TB 实际存储

Iceberg 容量：
  1TB/天 × 30 天保留 = 30TB
  压缩（3:1）= 10TB 实际存储
  副本 2 = 20TB

Doris 容量：
  聚合后数据量 ≈ 原始数据量 × 0.1（聚合比）
  100GB/天 × 7 天 = 700GB
  副本 3 = 2.1TB
```

## 十二、多活数据中心架构

### 12.1 多活架构模式

```
多活架构演进：

  1. 单活 + 异地灾备：
     1 个主中心 + 1 个灾备中心
     灾备中心只读/不服务
     RTO：分钟级，RPO：秒~分钟级

  2. 双活：
     2 个中心都提供服务
     数据双向同步（最终一致）
     RTO：秒级，RPO：零~秒级

  3. 多活（同城双活 + 异地灾备）：
     同城 2 个机房（低延迟，强一致）
     异地 1 个灾备（高可用）
     推荐方案（如支付宝/微信）

  4. 单元化（单元多活）：
     按用户 ID 分单元（如 0-9 共 10 个单元）
     每个单元独立服务+数据
     单元内强一致，跨单元最终一致
```

### 12.2 多活数据同步方案

| 方案 | 延迟 | 一致性 | 复杂度 | 适用 |
|------|------|--------|--------|------|
| 数据库同步（DTS） | 秒级 | 最终一致 | 低 | 传统架构 |
| 消息队列同步 | 毫秒级 | 最终一致 | 中 | 事件驱动 |
| 分布式事务（2PC） | 毫秒级 | 强一致 | 高 | 金融级 |
| CRDT（无冲突复制） | 毫秒级 | 最终一致 | 高 | 高并发写 |

```
核心挑战：
  1. 数据冲突：同一记录在两个中心同时修改
     解决：CRDT / 版本向量 / 业务去重

  2. 数据路由：请求路由到正确中心
     解决：用户 ID 分片 / 就近接入

  3. 数据一致性：跨中心数据同步延迟
     解决：异步同步 + 对账 + 补偿

  4. 故障切换：中心故障时快速切换
     解决：DNS 切换 / 负载均衡 / 客户端路由
```

### 12.3 单元化架构设计

```
单元化设计要点：

  单元划分：
    按用户 ID 哈希：user_id % N → 单元 N
    按地域划分：北京用户 → 北京单元
    按业务划分：订单单元 / 用户单元

  单元内自治：
    每个单元包含完整的服务栈
    数据库 / 缓存 / 消息队列 / 搜索引擎
    单元内强一致，跨单元最终一致

  跨单元交互：
    读：走本单元（本地数据）
    写：走本单元 + 跨单元同步（异步）
    查询：走搜索引擎（跨单元聚合）

  故障处理：
    单单元故障 → 流量切换到其他单元
    数据不一致 → 对账 + 补偿
    部分失败 → 降级 + 重试
```

## 十三、IoT 大数据平台架构

### 13.1 IoT 数据链路

```
IoT 数据链路架构：

  设备层：
    传感器 → 网关 → MQTT/CoAP
    协议：MQTT（轻量）/ CoAP（受限设备）/ HTTP（简单）

  接入层：
    MQTT Broker（EMQX / Mosquitto）→ Kafka
    设备认证（TLS/Token）
    协议解析（Protobuf/JSON）

  计算层：
    实时：Flink（异常检测/实时聚合）
    批处理：Spark（历史分析/模型训练）
    边缘计算：网关预处理（过滤/聚合）

  存储层：
    时序数据：TimescaleDB / InfluxDB / IoTDB
    实时数据：Kafka / Redis
    历史数据：HDFS / 数据湖

  应用层：
    实时监控（Grafana / 自研大屏）
    异常告警（规则引擎 / ML）
    数据分析（OLAP / 报表）
    预测性维护（ML 模型）
```

### 13.2 IoT 数据存储选型

| 数据类型 | 存储方案 | 特点 |
|----------|----------|------|
| 实时指标 | Redis / InfluxDB | 毫秒级读写 |
| 时序历史 | TimescaleDB / IoTDB | 自动分区/压缩 |
| 原始日志 | Kafka → HDFS | 追加写入/批量分析 |
| 设备元数据 | MySQL / PostgreSQL | 关系型/事务 |
| 告警规则 | Redis / ES | 快速匹配 |
| 模型特征 | HBase / Redis | 低延迟读取 |

### 13.3 IoT 大数据挑战

```
核心挑战：
  1. 海量写入：百万设备 × 每秒上报 = 百万 QPS
     解决：Kafka 缓冲 + Flink 聚合 + 时序库写入

  2. 时间乱序：设备时钟不同步
     解决：服务端时间戳 + Watermark + 允许迟到

  3. 数据质量：设备故障/网络抖动导致数据异常
     解决：Flink 实时清洗 + 异常检测 + 数据修复

  4. 存储成本：原始数据量大，长期存储成本高
     解决：降采样（1s → 1min）+ 压缩 + 冷热分层

  5. 实时分析：设备维度/时间维度灵活查询
     解决：预聚合 + OLAP 引擎 + 物化视图
```

## 十四、推荐系统大数据架构

### 14.1 推荐系统数据流

```
推荐系统架构：

  数据采集：
    用户行为（点击/浏览/购买） → Kafka
    商品特征（属性/价格/库存） → Kafka/DB
    上下文（时间/地点/设备） → Kafka

  特征工程：
    实时特征：Flink → Redis（用户实时兴趣）
    离线特征：Spark → Hive/Hudi（用户画像）
    物品特征：Spark → 向量数据库（物品嵌入）

  召回层：
    向量召回：向量数据库（语义匹配）
    协同召回：Spark ALS（相似用户）
    热门召回：实时统计（全局/分群热门）
    规则召回：业务规则（新品/限时/地域）

  排序层：
    精排：Cross-Encoder / LightGBM
    重排：多样性打散 + 业务规则

  服务层：
    推荐 API（毫秒级响应）
    A/B 实验平台
    实时反馈回路（用户反馈 → 特征更新）
```

### 14.2 推荐系统存储选型

| 存储 | 用途 | 特点 |
|------|------|------|
| Redis | 实时特征/在线缓存 | 毫秒级读写 |
| 向量数据库 | 向量召回 | 近似最近邻 |
| Hive/Hudi | 离线特征/历史数据 | 批量计算 |
| MySQL | 配置/规则/AB 实验 | 事务保证 |
| ES | 全文搜索/类目筛选 | 倒排索引 |
| ClickHouse | 实时分析/指标 | 列式 OLAP |

## 用户行为分析平台架构（实时埋点→Kafka→Flink→ClickHouse）

```mermaid
flowchart LR
    SDK[埋点 SDK] --> GW[网关] --> K[(Kafka)]
    K --> FL[Flink 实时预聚合]
    FL --> CK[(ClickHouse)]
    CK --> BI[BI 看板]
    K --> LAKE[(Iceberg)]
    LAKE --> SP[Spark 离线分析]
    SP --> CK
```

| 层 | 组件 | 职责 |
|----|------|------|
| 采集 | 埋点 SDK + 网关 | 事件上报、协议解析 |
| 消息 | Kafka | 削峰解耦、事件缓冲 |
| 计算 | Flink | 实时预聚合、维表 JOIN |
| 存储 | ClickHouse（实时）+ Iceberg（离线） | 多维分析、历史回溯 |
| 服务 | BI 看板 + 数据 API | 可视化、自助分析 |

## 实时推荐系统架构（特征计算→向量检索→排序）

```mermaid
flowchart TB
    A[用户请求] --> R[推荐服务]
    R --> FE[特征服务 Redis/HBase]
    R --> VS[向量检索 Milvus]
    R --> MD[模型服务 TF Serving]
    R --> SORT[排序+重排]
    FE --> K[Kafka 实时特征]
    K --> FL[Flink 实时特征计算]
    FL --> FE
    A -->|行为| K
```

| 阶段 | 技术 | 延迟要求 |
|------|------|---------|
| 特征计算 | Flink 实时 + Spark 离线 | 秒级 |
| 向量检索 | Milvus/Pinecone | <50ms |
| 模型推理 | TF Serving/Triton | <100ms |
| 排序重排 | 业务规则 + 模型打分 | <50ms |

## IoT 数据平台设计（设备接入→时序存储→告警→数据湖）

```
IoT 数据链路：

  设备层：传感器 → 网关 → MQTT
  接入层：MQTT Broker → Kafka
  计算层：
    实时：Flink（异常检测/告警/聚合）
    边缘：网关预处理（过滤/聚合）
  存储层：
    时序：TimescaleDB/TDengine（热数据）
    告警：Redis（滑动窗口计数器）
    数据湖：Iceberg（历史分析）
  应用层：
    实时监控：Grafana
    告警通知：钉钉/邮件
    数据分析：OLAP
```

## 实时大屏架构（采集→流处理→预聚合→缓存→前端）

```mermaid
flowchart LR
    DB[(业务DB)] --> CDC[CDC] --> K[(Kafka)]
    K --> FL[Flink 预聚合]
    FL --> D[(Doris/StarRocks)]
    FL --> R[(Redis 热指标)]
    D --> BI[BI 看板]
    R --> WS[WebSocket 推送]
    WS --> SCREEN[大屏展示]
```

| 指标 | 目标值 | 说明 |
|------|--------|------|
| 端到端延迟 | <5s | CDC→大屏展示 |
| 查询 QPS | 100+ | 大屏并发 |
| 数据一致性 | 100% | 实时/离线一致 |
| 可用性 | 99.9% | 高可用 |

## 数据平台容量评估方法论

```
容量评估公式：

  存储量 = 日增数据量 × 保留天数 × 副本 × 压缩比倒数
  计算量 = 峰值 QPS × 单查询扫描量
  Kafka 容量 = 日增消息量 × 保留天数 × 副本

示例（日增 10 亿事件，每事件 1KB）：
  Kafka：10亿 × 1KB = 1TB/天 × 3天 × 3副本 × 1/5压缩 = 1.8TB
  Iceberg：1TB/天 × 30天 × 1/3压缩 × 2副本 = 20TB
  Doris：100GB/天 × 7天 × 3副本 = 2.1TB

规划原则：
  存储留 30% 余量
  计算留 50% 峰值余量
  网络留 40% 带宽余量
```

## 三十三、实时推荐系统架构深度设计

### 33.1 推荐系统分层架构

```mermaid
graph TB
    subgraph 数据采集层
        A1[用户行为日志] --> K[Kafka]
        A2[商品特征库] --> K
        A3[实时特征] --> K
    end
    subgraph 特征工程层
        K --> FL[Flink 实时特征]
        K --> SP[Spark 离线特征]
        FL --> REDIS[Redis 特征库]
        SP --> HBASE[HBase 特征库]
    end
    subgraph 召回层
        REDIS --> R1[协同过滤召回]
        REDIS --> R2[向量召回]
        REDIS --> R3[热门召回]
        REDIS --> R4[规则召回]
    end
    subgraph 排序层
        R1 --> SORT[粗排+精排+重排]
        R2 --> SORT
        R3 --> SORT
        R4 --> SORT
    end
    subgraph 服务层
        SORT --> API[推荐 API]
        API --> APP[业务应用]
    end
```

### 33.2 特征工程设计

| 特征类型 | 计算引擎 | 更新频率 | 存储方案 |
|----------|----------|----------|----------|
| 实时特征 | Flink | 秒级 | Redis |
| 近实时特征 | Flink | 分钟级 | Redis |
| 离线特征 | Spark | 小时/天级 | HBase/Hive |
| 物品特征 | Spark | 天级 | MySQL/Hive |
| 上下文特征 | Flink | 实时 | Redis |

```java
// Flink 实时特征计算示例
public class RealtimeFeature extends KeyedProcessFunction<String, Event, Feature> {
    private transient ValueState<Long> clickCount;
    private transient ValueState<Long> viewCount;
    
    @Override
    public void open(Configuration parameters) {
        clickCount = getRuntimeContext().getState(
            new ValueStateDescriptor<>("clickCount", Long.class));
        viewCount = getRuntimeContext().getState(
            new ValueStateDescriptor<>("viewCount", Long.class));
    }
    
    @Override
    public void processElement(Event event, Context ctx, Collector<Feature> out) {
        // 滑动窗口特征计算
        if (event.getType().equals("click")) {
            clickCount.update(clickCount.value() + 1);
        } else if (event.getType().equals("view")) {
            viewCount.update(viewCount.value() + 1);
        }
        
        // 输出特征
        Feature feature = new Feature(event.getUserId());
        feature.setClickCount(clickCount.value());
        feature.setViewCount(viewCount.value());
        feature.setCtr(clickCount.value() * 1.0 / (viewCount.value() + 1));
        out.collect(feature);
    }
}
```

### 33.3 向量召回与 ANN 检索

```text
向量召回流程：
  1. 用户特征向量：实时计算用户兴趣向量
  2. 物品特征向量：预计算物品 embedding
  3. ANN 检索：近似最近邻搜索 Top-K
  4. 过滤：业务规则过滤（已购买/下架等）
  5. 输出：召回结果列表

向量数据库选型：
  Milvus：分布式、高性能、云原生
  Pinecone：托管服务、简单易用
  FAISS：Facebook 开源、本地部署
  Weaviate：开源、支持混合查询
```

---

## 三十四、金融风控实时检测架构

### 34.1 三层决策体系详解

| 层级 | 延迟 | 技术 | 规则类型 | 处置方式 |
|------|------|------|----------|----------|
| L1 实时 | <100ms | Flink CEP | 简单阈值/黑名单 | 自动拦截 |
| L2 近实时 | 秒级 | Flink + Redis | 复杂模式匹配 | 拦截或人工复核 |
| L3 离线 | T+1 | Spark ML | 模型评分/关联分析 | 事后处置 |

### 34.2 风控特征工程

```mermaid
graph LR
    A[交易数据] --> B[特征计算]
    B --> C[实时特征]
    B --> D[离线特征]
    C --> E[Redis]
    D --> F[HBase]
    E --> G[规则引擎]
    F --> G
    G --> H{风险决策}
    H -->|高风险| I[拦截]
    H -->|中风险| J[人工审核]
    H -->|低风险| K[放行]
```

| 特征类型 | 示例 | 计算方式 | 更新频率 |
|----------|------|----------|----------|
| 统计特征 | 近1小时交易次数 | 滑动窗口计数 | 秒级 |
| 行为特征 | 连续登录天数 | 状态累计 | 天级 |
| 关系特征 | 交易对手风险分 | 图计算 | 小时级 |
| 设备特征 | 设备指纹异常分 | 规则匹配 | 实时 |
| 模型特征 | XGBoost 风险评分 | 模型推理 | 秒级 |

### 34.3 Flink CEP 复杂模式匹配

```java
// 模式：同一用户 1 小时内在 3 个不同城市交易
Pattern<Transaction, ?> pattern = Pattern
    .<Transaction>begin("first")
        .where(new SimpleCondition<Transaction>() {
            public boolean filter(Transaction t) { return t.amount > 10000; }
        })
    .next("second")
        .where(new IterativeCondition<Transaction>() {
            public boolean filter(Transaction t, Context<Transaction> ctx) {
                for (Transaction first : ctx.getEventsForPattern("first")) {
                    if (!t.city.equals(first.city)) return true;
                }
                return false;
            }
        })
    .next("third")
        .where(new IterativeCondition<Transaction>() {
            public boolean filter(Transaction t, Context<Transaction> ctx) {
                // 与 first、second 城市都不同
                Set<String> cities = new HashSet<>();
                for (Transaction first : ctx.getEventsForPattern("first")) {
                    cities.add(first.city);
                }
                for (Transaction second : ctx.getEventsForPattern("second")) {
                    cities.add(second.city);
                }
                return !cities.contains(t.city);
            }
        })
    .within(Time.hours(1));
```

---

## 三十五、IoT 数据平台深度设计

### 35.1 IoT 数据链路架构

```mermaid
graph TB
    subgraph 设备层
        D1[传感器] --> GW[网关]
        D2[摄像头] --> GW
    end
    subgraph 接入层
        GW --> MQTT[MQTT Broker]
        MQTT --> K[Kafka]
    end
    subgraph 计算层
        K --> FL[Flink 实时处理]
        K --> SP[Spark 离线分析]
    end
    subgraph 存储层
        FL --> TS[时序数据库]
        FL --> HBASE[设备状态]
        FL --> ICE[数据湖]
    end
    subgraph 应用层
        TS --> GF[Grafana 监控]
        HBASE --> API[设备管理 API]
        ICE --> ANAL[离线分析]
    end
```

### 35.2 IoT 数据存储选型

| 数据类型 | 存储方案 | 特点 | 适用场景 |
|----------|----------|------|----------|
| 实时指标 | InfluxDB/TDengine | 高写入吞吐 | 设备监控 |
| 设备状态 | Redis/HBase | 低延迟读写 | 状态查询 |
| 原始日志 | Kafka → HDFS | 追加写入 | 历史回溯 |
| 设备元数据 | MySQL | 事务保证 | 设备管理 |
| 告警规则 | Redis | 快速匹配 | 实时告警 |

### 35.3 IoT 数据挑战与解决方案

| 挑战 | 问题描述 | 解决方案 |
|------|----------|----------|
| 海量写入 | 百万设备 × 每秒上报 | Kafka 缓冲 + Flink 聚合 |
| 时间乱序 | 设备时钟不同步 | Watermark + 允许迟到 |
| 数据质量 | 设备故障导致异常 | 实时清洗 + 异常检测 |
| 存储成本 | 原始数据量大 | 降采样 + 冷热分层 |
| 实时分析 | 多维度灵活查询 | 预聚合 + OLAP 引擎 |

---

## 三十六、实时大屏架构深度设计

### 36.1 大屏架构核心要点

| 要点 | 目标值 | 实现方案 |
|------|--------|----------|
| 数据延迟 | <5s | CDC → Flink → Redis |
| 查询 QPS | 1000+ | 结果预计算 + 缓存 |
| 数据准确性 | 100% | 实时/离线对账 |
| 可用性 | 99.9% | 多活部署 + 降级 |

### 36.2 大屏数据流优化

```mermaid
graph LR
    A[业务DB] -->|CDC| K[Kafka]
    K --> F1[Flink 1min 窗口]
    K --> F5[Flink 5min 窗口]
    K --> FH[Flink 1h 窗口]
    F1 --> R1[Redis 1min 指标]
    F5 --> R5[Redis 5min 指标]
    FH --> RH[Redis 1h 指标]
    R1 --> WS[WebSocket 推送]
    R5 --> WS
    RH --> WS
    WS --> SCREEN[大屏展示]
```

### 36.3 大屏降级策略

| 故障场景 | 降级方案 | 影响 |
|----------|----------|------|
| Flink 故障 | Redis 缓存兜底 | 数据延迟增加 |
| Redis 故障 | 本地缓存兜底 | 数据不一致 |
| OLAP 故障 | 历史数据展示 | 无实时数据 |
| 网络抖动 | WebSocket 重连 | 展示中断 |

---

## 三十七、数据平台容量评估深度方法论

### 37.1 容量评估公式

```text
存储容量 = 日增量 × 保留天数 × 副本数 × 压缩比倒数 × 冗余系数

计算资源 = 日增量 / 单节点处理能力 × 并发系数 × 安全系数

网络带宽 = 峰值 QPS × 消息大小 × 副本数

Kafka 容量 = 日增消息量 × 保留天数 × 副本数 × 压缩比倒数
```

### 37.2 容量规划示例

```text
场景：日增 10 亿事件，每事件 1KB，保留 30 天

Kafka 容量：
  日增量：10亿 × 1KB = 1TB/天
  保留 3 天 × 3 副本 = 9TB
  压缩（5:1）= 1.8TB 实际存储

Iceberg 容量：
  日增量：1TB/天 × 30 天 = 30TB
  压缩（3:1）= 10TB 实际存储
  副本 2 = 20TB

Doris 容量：
  聚合后数据量 ≈ 原始数据量 × 0.1
  100GB/天 × 7 天 = 700GB
  副本 3 = 2.1TB

计算资源：
  Flink：10 亿事件/天 ≈ 12K 事件/秒
  需要 TaskManager：12K / 1K ≈ 12 个
  需要 JobManager：2 个（HA）
```

### 37.3 容量规划 Checklist

```text
存储规划：
  ☐ 峰值写入量（突发流量预估）
  ☐ 数据保留策略（热/温/冷分层）
  ☐ 副本数（可用性要求）
  ☐ 压缩比（时序数据 10:1+）
  ☐ 冗余系数（故障恢复空间）

计算规划：
  ☐ 峰值 QPS（并发查询预估）
  ☐ 单查询扫描量（OLAP 场景）
  ☐ 批处理窗口（离线任务）
  ☐ 增长率预估（业务增长）

网络规划：
  ☐ 峰值流量（数据同步）
  ☐ 消息大小（Kafka 场景）
  ☐ 跨机房带宽（多活场景）
```

---

## 三十八、场景设计常见反模式深度分析

### 38.1 反模式分类与危害

| 反模式 | 问题描述 | 危害 | 解决方案 |
|--------|----------|------|----------|
| 过度设计 | 引入不必要的组件 | 维护成本高 | 从简单架构开始演进 |
| 单点故障 | 单一组件故障导致全挂 | 系统不可用 | 关键路径冗余部署 |
| 数据倾斜 | 热点 key 导致单节点压力 | 性能瓶颈 | 分散 key + 负载均衡 |
| 过早优化 | 业务未验证就做复杂架构 | 资源浪费 | 先跑通再优化 |
| 忽视监控 | 出了问题才发现 | 故障恢复慢 | 架构设计同步设计监控 |
| 强一致性陷阱 | 所有场景都追求强一致 | 性能下降 | 区分实时/离线需求 |

### 38.2 架构演进原则

```text
演进路径：
  1. 单体架构 → 验证业务可行性
  2. 微服务拆分 → 解决扩展性问题
  3. 数据中台 → 统一数据治理
  4. 湖仓一体 → 实时+离线统一

演进原则：
  1. 业务驱动：技术架构服务于业务目标
  2. 渐进式：小步快跑，持续迭代
  3. 可观测：监控先行，故障可诊断
  4. 成本意识：平衡性能与成本
```

### 38.3 常见故障处理流程

```mermaid
graph TD
    A[故障告警] --> B{故障分级}
    B -->|P0 严重| C[立即响应]
    B -->|P1 重要| D[30分钟响应]
    B -->|P2 一般| E[4小时响应]
    C --> F[定位问题]
    D --> F
    E --> F
    F --> G{是否影响业务}
    G -->|是| H[降级/切换]
    G -->|否| I[修复问题]
    H --> I
    I --> J[复盘改进]
```

---

## 十五、实时推荐系统架构详解

### 推荐系统架构

| 层级 | 组件 | 技术栈 | 说明 |
|------|------|--------|------|
| 数据采集 | 行为埋点 | Kafka + Flink | 实时采集用户行为 |
| 特征存储 | 特征仓库 | Redis + HBase | 在线特征 + 离线特征 |
| 推荐引擎 | 模型服务 | TensorFlow Serving | 在线推理 |
| 推荐结果 | 缓存层 | Redis Cluster | 推荐结果缓存 |
| 展示层 | AB 实验 | Apollo + 自研 | 实验分流 |

### 推荐流程

```mermaid
flowchart LR
    A[用户请求] --> B[召回]
    B --> C[粗排]
    C --> D[精排]
    D --> E[重排]
    E --> F[返回结果]
    G[行为日志] -->|实时| H[特征更新]
    H --> B
```

### 推荐指标体系

| 指标 | 说明 | 目标 |
|------|------|------|
| CTR | 点击率 | 核心指标 |
| CVR | 转化率 | 商业目标 |
| 人均阅读量 | 用户参与度 | 留存相关 |
| 新颖度 | 内容多样性 | 用户体验 |
| 公平性 | 推荐公平性 | 长期目标 |

## 十六、金融风控系统架构详解

### 风控系统架构

| 层级 | 组件 | 说明 |
|------|------|------|
| 数据层 | 实时数据 | 用户行为 + 设备指纹 |
| 规则引擎 | DRL/Flink CEP | 规则匹配 |
| 模型引擎 | ML Pipeline | 机器学习模型 |
| 决策引擎 | 风控决策 | 综合判定 |
| 执行层 | 风控动作 | 拦截/人工审核/放行 |

### 风控指标体系

| 指标 | 说明 | 目标 |
|------|------|------|
| 风险率 | 风险交易占比 | < 0.1% |
| 误拦率 | 正常交易被拦截 | < 1% |
| 召回率 | 风险交易识别率 | > 95% |
| 实时性 | 风控响应时间 | < 100ms |

### 风控规则示例

```
# 同一用户短时间内多设备登录
IF (user.login_count_1h > 3 AND user.device_count > 2)
AND (user.ip_changed = true)
THEN risk_score += 30

# 交易金额异常
IF (transaction.amount > user.avg_amount_30d * 3)
AND (transaction.hour NOT IN user.normal_hours)
THEN risk_score += 20

# 综合判定
IF risk_score >= 50 THEN BLOCK
ELIF risk_score >= 30 THEN REVIEW
ELSE PASS
```

## 十七、IoT 数据平台架构详解

### IoT 平台架构

| 层级 | 组件 | 技术栈 |
|------|------|--------|
| 设备接入 | MQTT Broker | EMQX / Mosquitto |
| 协议解析 | 网关 | 自研 / Kura |
| 数据采集 | 流处理 | Flink / Kafka Streams |
| 数据存储 | 时序数据库 | InfluxDB / TDengine |
| 数据分析 | 批处理 | Spark / Presto |
| 数据可视化 | Dashboard | Grafana |

### IoT 数据特点

```
数据特点：
  1. 高吞吐：百万设备 × 10 条/秒 = 千万条/秒
  2. 时序性：强时序关系
  3. 海量数据：PB 级/月
  4. 数据稀疏：部分设备间歇性上报
  5. 数据缺失：网络不稳定导致丢包
```

### IoT 指标体系

| 指标 | 说明 | 目标 |
|------|------|------|
| 设备在线率 | 设备连接状态 | > 99% |
| 数据完整率 | 数据丢失率 | < 0.1% |
| 响应延迟 | 命令下发延迟 | < 1s |
| 存储成本 | 每条数据成本 | 持续优化 |

## 十八、实时大屏指标计算详解

### 大屏指标计算架构

| 指标类型 | 计算方式 | 技术栈 |
|----------|----------|--------|
| 实时指标 | 流处理 | Flink / Spark Streaming |
| 准实时指标 | 微批 | Spark Structured Streaming |
| 历史指标 | 批处理 | Spark / Presto |
| 复杂指标 | ML | TensorFlow / PyTorch |

### 大屏指标示例

```sql
-- 实时 GMV（Flink SQL）
SELECT
  TUMBLE_START(proctime, INTERVAL '1' MINUTE) AS window_start,
  SUM(amount) AS total_gmv,
  COUNT(DISTINCT user_id) AS active_users
FROM orders
WHERE status = 'completed'
GROUP BY TUMBLE(proctime, INTERVAL '1' MINUTE);

-- 实时库存预警
SELECT
  product_id,
  current_stock,
  predicted_stock_1h,
  CASE
    WHEN predicted_stock_1h < 0 THEN 'OUT_OF_STOCK'
    WHEN predicted_stock_1h < 10 THEN 'LOW'
    ELSE 'NORMAL'
  END AS status
FROM inventory_forecast;
```

## 十九、容量评估方法论详解

### 容量评估流程

```mermaid
flowchart TD
    A[业务需求] --> B[容量评估]
    B --> C[资源规划]
    C --> D[部署方案]
    D --> E[监控告警]
    E --> F[持续优化]
```

### 容量评估公式

```
存储容量 = 日增量 × 保留天数 × 副本数 × 压缩比
  示例：100GB × 365 × 3 × 0.3 = 32.85TB

计算容量 = QPS × 平均 CPU 时间 × 副本数
  示例：1000 × 10ms × 3 = 30 cores

网络容量 = QPS × 平均包大小 × 副本数
  示例：1000 × 10KB × 3 = 30MB/s
```

### 容量评估模板

| 资源 | 评估维度 | 计算公式 | 安全系数 |
|------|---------|----------|----------|
| 存储 | 数据量 | 日增量 × 保留天数 | 1.5~2x |
| 计算 | QPS | 峰值 QPS × 单次耗时 | 2~3x |
| 内存 | 缓存命中 | 命中率 × 数据量 | 1.2~1.5x |
| 网络 | 带宽 | 峰值 QPS × 包大小 | 2~3x |

## 二十、反模式识别与避免

### 常见反模式

| 反模式 | 说明 | 解决方案 |
|--------|------|----------|
| 简单堆砌 | 大量技术选型未考虑实际需求 | 需求驱动 |
| 过早优化 | 未遇到性能问题先优化 | 按需优化 |
| 数据孤岛 | 数据未打通 | 数据中台 |
| 过度设计 | 架构过于复杂 | 简化架构 |
| 单点瓶颈 | 关键路径单点 | 分布式改造 |

### 反模式识别信号

```
反模式信号：
  1. 技术选型与业务不匹配
  2. 架构复杂度持续上升
  3. 性能问题频发
  4. 维护成本持续增长
  5. 团队技术债务积累
```

## 二十一、数据埋点规范详解

### 埋点规范

| 规范 | 说明 |
|------|------|
| 事件命名 | 业务_动作_结果（如 order_create_success） |
| 参数统一 | 使用 snake_case |
| 必填字段 | user_id, device_id, timestamp |
| 可选字段 | 业务特定字段 |
| 上报时机 | 明确上报时机（点击/加载/完成） |

### 埋点质量监控

```
埋点质量指标：
  完整率：埋点字段完整率 > 99%
  准确率：埋点数据准确率 > 99.9%
  时效性：埋点上报延迟 < 5s
  覆盖率：关键路径埋点覆盖率 > 100%
```

## 二十二、性能基线建立详解

### 性能基线建立流程

| 步骤 | 说明 |
|------|------|
| 压测环境 | 与生产环境配置一致 |
| 压测数据 | 使用生产数据子集 |
| 压测场景 | 覆盖核心业务场景 |
| 压测工具 | JMeter/Locust/k6 |
| 压测时间 | 持续 30min+ |
| 结果分析 | P50/P90/P99 指标 |

### 性能基线模板

| 场景 | QPS | RT(P99) | 错误率 |
|------|-----|---------|--------|
| 首页查询 | 1000 | 50ms | < 0.1% |
| 商品详情 | 500 | 100ms | < 0.1% |
| 下单 | 200 | 200ms | < 0.01% |
| 支付 | 100 | 500ms | < 0.001% |

## 二十三、容量告警配置详解

### 告警规则配置

| 告警类型 | 阈值 | 级别 | 通知方式 |
|----------|------|------|----------|
| 磁盘使用率 | > 80% | Warning | 企业微信 |
| 磁盘使用率 | > 90% | Critical | 电话 |
| CPU 使用率 | > 80% | Warning | 企业微信 |
| CPU 使用率 | > 95% | Critical | 电话 |
| 内存使用率 | > 80% | Warning | 企业微信 |
| 内存使用率 | > 95% | Critical | 电话 |

### 告警处理流程

```
告警处理：
  1. 接收告警（企业微信/电话）
  2. 初步判断（是否误告警）
  3. 问题定位（查看监控）
  4. 紧急处理（扩容/重启/降级）
  5. 根因分析（事后复盘）
  6. 改进措施（优化/监控加强）
```

## 实时推荐系统（Lambda/Kappa架构）

### Lambda架构实现

```
Batch Layer：
  全量数据 → HDFS/Hive
  离线计算 → Spark/MapReduce
  批处理视图 → 模型训练

Speed Layer：
  实时数据 → Kafka → Flink/Spark Streaming
  实时视图 → 近似推荐

Serving Layer：
  合并批处理视图 + 实时视图
  查询服务 → 推荐API

数据流：
  用户行为 → Kafka → Flink(实时特征) → 特征存储
              ↓
         Hadoop(离线特征) → 特征存储
              ↓
         特征存储 → 模型训练 → 推荐服务
```

### Kappa架构实现

```
Kappa架构（全流式）：
  统一用流处理：
    Kafka → Flink → 实时视图 → 推荐服务

  优势：
    - 架构简单（一套系统）
    - 实时性好（秒级）
    - 维护成本低

  劣势：
    - 历史数据重算困难
    - 流处理复杂度高

  适用场景：
    - 实时性要求高
    - 数据量适中
    - 团队流处理经验丰富
```

## 金融风控（实时规则/特征计算/模型推理）

### 风控系统架构

```mermaid
flowchart TB
    A[交易数据] --> B[Kafka]
    B --> C[Flink实时计算]
    C --> D{风控决策}
    D -->|通过| E[放行]
    D -->|拒绝| F[拒绝]
    D -->|人工审核| G[审核队列]
    H[规则引擎] --> D
    I[模型推理] --> D
```

### 实时风控规则

| 规则类型 | 示例 | 检测窗口 |
|----------|------|----------|
| 频率限制 | 同一卡1小时>5次 | 1小时滑动窗口 |
| 金额限制 | 单笔>5万 | 实时 |
| 地域检测 | 异地交易 | 1天 |
| 设备检测 | 新设备交易 | 7天 |
| 行为检测 | 异常操作序列 | 5分钟 |

### 特征计算

```scala
// Flink实时特征计算
val transactions = env
  .addSource(new KafkaSource(...))
  .keyBy(_.cardNo)

// 1小时交易次数
val hourlyCount = transactions
  .window(SlidingEventTimeWindows.of(Time.hours(1), Time.minutes(5)))
  .count()

// 最近5次平均金额
val recentAvg = transactions
  .window(SlidingEventTimeWindows.of(Time.minutes(5), Time.minutes(1)))
  .aggregate(new AverageAmount())
```

## IoT平台（设备管理/数据采集/规则引擎）

### IoT平台架构

```
设备层：
  传感器 → MQTT/CoAP → 网关 → 平台

平台层：
  设备接入：EMQX/MQTT Broker
  设备管理：注册/认证/OTA
  数据采集：Kafka → Flink → 时序数据库
  规则引擎：Flink SQL / 复杂事件处理

应用层：
  数据可视化：Grafana/自定义Dashboard
  告警管理：阈值/异常检测
  设备控制：下发命令
```

### 时序数据存储

| 数据库 | 特点 | 适用场景 |
|--------|------|----------|
| InfluxDB | 轻量级/内置可视化 | 中小规模 |
| TimescaleDB | 基于PostgreSQL | SQL兼容 |
| TDengine | 国产/高性能 | 大规模IoT |
| Prometheus | 指标监控 | 监控场景 |

## 实时大屏（数据摄入/聚合/展示）

### 实时大屏架构

```
数据源：
  Kafka → Flink聚合 → Redis/ClickHouse

渲染层：
  数据服务 → WebSocket/SSE → 前端(ECharts/AntV)

架构要点：
  - Flink实时聚合（窗口计算）
  - Redis缓存（低延迟查询）
  - WebSocket推送（实时更新）
  - 前端动画（视觉效果）
```

### Flink实时聚合

```sql
-- Flink SQL实时统计
SELECT
  TUMBLE_START(event_time, INTERVAL '10' SECOND) as window_start,
  region,
  COUNT(*) as event_count,
  SUM(amount) as total_amount
FROM events
GROUP BY TUMBLE(event_time, INTERVAL '10' SECOND), region
```

## 容量评估（集群规模/存储需求/带宽计算）

### 容量评估公式

```
存储评估：
  日数据量 = 设备数 × 每设备数据量 × 保留天数
  压缩比 = 原始数据 : 压缩后数据（约3:1~10:1）
  实际存储 = 日数据量 × 保留天数 ÷ 压缩比 × 副本数(3)

计算评估：
  实时计算 = 日数据量 ÷ 实时窗口大小
  离线计算 = 日数据量 ÷ 计算时间（小时）

带宽评估：
  入站带宽 = 设备数 × 每设备数据量 × 采集频率
  内部带宽 = 入站带宽 × 副本数 × 写放大系数
```

### 集群规模参考

| 数据量 | 集群规模 | 存储 |
|--------|----------|------|
| < 1TB/天 | 5~10节点 | 100TB |
| 1~10TB/天 | 10~50节点 | 1PB |
| 10~100TB/天 | 50~200节点 | 10PB |
| > 100TB/天 | 200+节点 | 100PB+ |

## 指标体系（北极星指标/过程指标）

### 北极星指标设计

| 业务 | 北极星指标 | 过程指标 |
|------|-----------|----------|
| 电商 | GMV（成交总额） | 浏览量/转化率/客单价 |
| 社交 | DAU（日活用户） | 发帖量/互动率/留存率 |
| 搜索 | 搜索成功率 | 点击率/结果质量/响应时间 |
| 视频 | 播放量/时长 | 完播率/分享率/互动率 |

### 指标分层

```
指标分层体系：
  L0 原始指标：设备数/消息量/延迟
  L1 过程指标：处理速率/成功率/资源使用率
  L2 业务指标：转化率/留存率/DAU
  L3 决策指标：ROI/增长率/市场份额

数据流向：
  原始数据 → L0 → L1 → L2 → L3
```

## 埋点规范（事件定义/参数标准/版本管理）

### 埋点事件规范

```json
{
  "event": "page_view",
  "timestamp": 1700000000000,
  "user_id": "user_12345",
  "device_id": "device_67890",
  "platform": "iOS",
  "app_version": "3.2.1",
  "properties": {
    "page_name": "home",
    "page_title": "首页",
    "referrer": "search",
    "load_time_ms": 123
  },
  "context": {
    "network": "wifi",
    "os_version": "17.0",
    "screen_width": 390,
    "screen_height": 844
  }
}
```

### 埋点管理流程

```
埋点流程：
  1. 需求评审 → 确定埋点方案
  2. 埋点设计 → 事件/属性定义
  3. 开发实现 → 前端/后端埋点
  4. 测试验证 → 埋点数据校验
  5. 上线监控 → 数据质量检查

版本管理：
  - 埋点文档版本化
  - 参数变更通知
  - 下线流程审批
```

## 数据平台组织（平台团队/数据工程师/分析师）

### 团队角色

| 角色 | 职责 | 技能 |
|------|------|------|
| 平台工程师 | 基础设施维护 | Hadoop/K8s/运维 |
| 数据工程师 | 数据管道开发 | Spark/Flink/Kafka |
| 数据分析师 | 数据分析/BI | SQL/Python/可视化 |
| 算法工程师 | 模型开发 | ML/统计学 |
| 数据产品经理 | 数据需求/治理 | 业务理解 |

### 数据治理组织

```
数据治理架构：
  数据委员会：制定策略/标准
  数据Owner：业务数据负责人
  数据Steward：数据质量负责人
  数据Engineer：技术实现负责人

治理流程：
  1. 数据标准化 → 统一命名/格式
  2. 数据质量 → 监控/清洗/告警
  3. 数据安全 → 分级/脱敏/权限
  4. 数据生命周期 → 采集/存储/归档/销毁
```

## SLA（数据质量/延迟/可用性）

### SLA指标

| 指标 | 定义 | 目标 |
|------|------|------|
| 数据质量 | 完整性/准确性/一致性 | > 99.9% |
| 数据延迟 | 数据可用时间 | < 5分钟 |
| 系统可用性 | 系统正常运行时间 | > 99.95% |
| 故障恢复 | 故障到恢复时间 | < 30分钟 |

### SLA违约处理

```
违约等级：
  P0：核心数据不可用 → 立即响应
  P1：数据延迟>30分钟 → 15分钟响应
  P2：数据质量<99% → 2小时响应
  P3：非核心数据问题 → 下个工作日

补偿机制：
  - SLA违约报告
  - 根因分析
  - 改进计划
  - 预防措施
```

## 与其他板块的关系

- **08 流处理计算：Flink** — 实时大屏、风控的计算引擎；
- **09 数据仓库与 OLAP 引擎** — 数仓建模、OLAP 查询引擎；
- **11 实时数仓与湖仓一体** — 实时数仓完整架构；
- **12 数据治理与数据质量** — 指标体系、数据质量保障；
- **场景设计/推荐系统设计**（如果存在）— 用户画像的消费方；
- **场景设计/实时风控系统设计**（如果存在）— 风控场景的完整版；
- **SRE与稳定性工程** — 数据链路的稳定性保障；
- **分库分表与数据迁移** — 数据库层面的迁移策略。

---

## 十四、速查表

| 场景 | 核心链路 | 关键技术 |
|------|----------|----------|
| 实时大屏 | Kafka → Flink 预聚合 → Doris → BI | 窗口聚合、维表打宽、Upsert |
| 用户画像 | Kafka → Flink 实时 + Spark 离线 → HBase/ES | Lambda、RowKey、倒排索引 |
| 实时风控 | 交易流 → Flink CEP → Redis/HBase → 决策 | CEP 模式匹配、三层决策 |
| 数据中台 | 多源 → ODS → DWD → DWS → ADS → API | 数仓分层、指标体系、数据治理 |
| 数仓迁移 | CDC + DataX → 双写 → 对账 → 切流 | 双写一致性、灰度切流、回滚 |
| 用户行为 | 埋点 → Kafka → Flink → Doris → BI | 多维分析、实时预聚合 |
| 实时推荐 | 用户行为 → Kafka → Flink 特征 → 模型推理 | 特征计算、模型服务 |
| IoT 数据 | 设备 → MQTT → Kafka → Flink → 时序 DB | 海量写入、时序存储 |

## 七、实时推荐系统架构

### 7.1 四阶段流程

```mermaid
graph LR
    A[特征计算] --> B[向量检索]
    B --> C[排序]
    C --> D[A/B测试]
    
    A --> A1[实时特征]
    A --> A2[离线特征]
    B --> B1[ANN近似最近邻]
    C --> C1[粗排]
    C --> C2[精排]
    C --> C3[重排]
```

### 7.2 技术栈选型

| 阶段 | 技术方案 | 说明 |
|------|----------|------|
| 特征计算 | Flink + Redis | 实时特征毫秒级更新 |
| 向量检索 | Milvus/Pinecone/FAISS | ANN 检索，毫秒级 |
| 排序 | TensorFlow Serving/Triton | GPU 推理加速 |
| A/B测试 | 自建/火山引擎 | 灰度分流+效果评估 |
| 特征存储 | Feast/Hopsworks | Feature Store 统一管理 |

## 八、金融风控实时检测架构

### 8.1 三层决策体系

```text
第一层：实时规则引擎（毫秒级）
  技术：Flink CEP / Drools
  场景：黑名单拦截、金额阈值、频次限制
  
第二层：实时特征计算（秒级）
  技术：Flink + Redis
  场景：滑动窗口聚合、用户行为画像
  
第三层：图计算（离线/近实时）
  技术：Neo4j/TigerGraph + Spark GraphX
  场景：团伙识别、关联交易网络
```

### 8.2 架构图

```mermaid
graph TB
    A[交易数据源] --> B[Kafka]
    B --> C[Flink实时计算]
    C --> D[规则引擎]
    C --> E[特征计算]
    E --> F[Redis特征库]
    D --> G{风险决策}
    G -->|高风险| H[拦截]
    G -->|中风险| I[人工审核]
    G -->|低风险| J[放行]
    B --> L[图数据库]
    L --> M[团伙分析]
```

## 九、IoT 数据平台设计

### 9.1 五层架构

```mermaid
graph TB
    subgraph 设备接入层
        A1[MQTT Broker]
        A2[HTTP Gateway]
    end
    subgraph 边缘处理层
        B1[边缘网关]
        B2[数据过滤]
    end
    subgraph 流处理层
        C1[Kafka]
        C2[Flink]
    end
    subgraph 时序存储层
        D1[TDengine/InfluxDB]
        D2[时序聚合]
    end
    subgraph 数据湖层
        E1[Iceberg/Hudi]
        E2[离线分析]
    end
    subgraph AI层
        F1[预测性维护]
        F2[异常检测]
    end
    A1 --> B1
    B1 --> C1
    C1 --> C2
    C2 --> D1
    C2 --> E1
    E1 --> F1
```

### 9.2 关键设计决策

| 设计点 | 方案 | 理由 |
|--------|------|------|
| 设备接入 | MQTT 5.0 + EMQ X | 轻量协议，百万连接 |
| 边缘过滤 | Kubernetes Edge | 降低带宽成本 |
| 时序存储 | TDengine | 超高写入吞吐 |
| 数据湖 | Iceberg on S3 | 支持 ACID + 时间旅行 |
| AI推理 | ONNX Runtime | 跨平台部署 |

## 十、实时大屏架构

### 10.1 数据流

```text
埋点 SDK → Kafka → Flink 预聚合 → Redis → WebSocket → 前端大屏

关键指标：
  端到端延迟：<2s（埋点到大屏展示）
  吞吐量：支持 10万+ QPS 数据写入
  可用性：99.9%+（大屏不能黑）
```

### 10.2 技术选型

| 组件 | 方案 | 说明 |
|------|------|------|
| 埋点 | 自研 SDK | 支持 Web/App/小程序 |
| 消息队列 | Kafka | 高吞吐缓冲 |
| 预聚合 | Flink Window | 滑动/滚动窗口 |
| 缓存 | Redis Cluster | 亚毫秒读取 |
| 推送 | WebSocket | 实时推送 |
| 可视化 | ECharts/DataV | 大屏渲染 |

## 十一、数据平台容量评估方法论

### 11.1 评估公式

```text
存储容量 = 用户量 × 事件频率 × 单事件大小 × 数据保留期 × 副本数

示例计算：
  用户量：1000万
  事件频率：10次/用户/天
  单事件大小：1KB
  数据保留期：365天
  副本数：3

  日增量 = 1000万 × 10 × 1KB = 100GB/天
  年存储 = 100GB × 365 × 3（副本）= ~110TB

计算资源 = 日增量 / 单节点处理能力 × 安全系数(1.5)
```

### 12.2 容量规划 Checklist

```text
☐ 峰值写入量（突发流量预估）
☐ 峰值查询量（并发查询预估）
☐ 数据保留策略（热/温/冷分层）
☐ 副本数（可用性要求）
☐ 增长率预估（业务增长）
☐ 压缩比（时序数据 10:1+）
☐ 冗余系数（故障恢复空间）
```

## 十二、场景设计常见反模式

| 反模式 | 问题 | 解决方案 |
|--------|------|----------|
| 过度设计 | 引入不必要的组件 | 从简单架构开始演进 |
| 单点故障 | 单一组件故障导致全挂 | 关键路径冗余部署 |
| 数据倾斜 | 热点 key 导致单节点压力 | 分散 key + 负载均衡 |
| 过早优化 | 业务未验证就做复杂架构 | 先跑通再优化 |
| 忽视监控 | 出了问题才发现 | 架构设计同步设计监控 |
| 强一致性陷阱 | 所有场景都追求强一致 | 区分实时/离线需求 |

---

## 指标体系搭建实战

```mermaid
flowchart TD
    BIZ[业务目标] --> |拆解| DIM[维度定义]
    BIZ --> |拆解| MET[指标定义]
    MET --> ATOMIC[原子指标: 订单数/GMV/UV]
    MET --> DERIVE[派生指标: 单UV价值=GMV/UV]
    DIM --> TIME[时间维度]
    DIM --> REGION[地域维度]
    DIM --> CHANNEL[渠道维度]
    ATOMIC --> |组合| REPORT[报表/看板]
    DERIVE --> |组合| REPORT
```

### 指标分层模型

| 层级 | 指标类型 | 示例 | 存储位置 |
|------|----------|------|----------|
| ODS | 原始数据 | 原始日志 | 数据湖 |
| DWD | 原子指标 | 订单数、支付金额 | 明细层 |
| DWS | 派生指标 | 单UV价值、转化率 | 汇总层 |
| ADS | 复合指标 | ROI、用户LTV | 应用层 |

### 指标命名规范

```
[业务过程]_[修饰词]_[原子指标]
示例：
  order_cnt                    -- 订单数
  pay_amount_1d                -- 当日支付金额
  uv_cnt_wechat               -- 微信渠道UV
  gmv_30d_premium_user         -- 高级用户30天GMV
```

## 需求优先级评估框架

| 评估维度 | 权重 | 评分标准(1-5) |
|----------|------|---------------|
| 业务价值 | 30% | 5=核心收入，1=锦上添花 |
| 技术复杂度 | 20% | 5=简单，1=极其复杂 |
| 数据可得性 | 20% | 5=已有数据，1=需新建采集 |
| 紧急程度 | 15% | 5=立即，1=不急 |
| 复用性 | 15% | 5=多场景复用，1=一次性 |

```mermaid
flowchart LR
    REQ[需求池] --> SCORE[评分矩阵]
    SCORE --> P0[P0: 立即做]
    SCORE --> P1[P1: 排期做]
    SCORE --> P2[P2: 有空做]
    SCORE --> P3[P3: 暂不做]
```

## 实时推荐系统架构

```mermaid
flowchart TB
    subgraph 离线层
        HIVE[离线特征] --> TRAIN[模型训练]
        TRAIN --> STORE[模型存储]
    end
    subgraph 近线层
        KAFKA[Kafka] --> FLINK[Flink实时特征]
        FLINK --> FEAT[实时特征]
    end
    subgraph 在线层
        FEAT --> RANK[排序服务]
        STORE --> RANK
        RANK --> REC[推荐结果]
    end
    USER[用户请求] --> RANK
```

### 推荐系统数据流

| 阶段 | 数据源 | 处理引擎 | 延迟 | 输出 |
|------|--------|----------|------|------|
| 特征工程 | 用户行为日志 | Flink | 秒级 | 实时特征 |
| 模型训练 | 离线数据集 | Spark/TensorFlow | 小时级 | 模型文件 |
| 在线推理 | 实时特征+模型 | TensorFlow Serving | 毫秒级 | 推荐列表 |
| 效果评估 | 曝光/点击日志 | Spark | 分钟级 | CTR/CVR |

## 数据埋点规范

### 埋点三要素

| 要素 | 说明 | 示例 |
|------|------|------|
| Event | 行为事件 | click_button |
| Property | 事件属性 | button_id, page_id |
| User | 用户标识 | user_id, device_id |

### 埋点命名规范

```
[模块]_[动作]_[对象]
示例：
  home_click_banner       -- 首页点击banner
  search_click_result     -- 搜索点击结果
  order_submit_pay        -- 订单提交支付
```

### 埋点质量检查

```python
# 埋点质量检查脚本
def check_tracking(event):
    errors = []
    if not event.get("event_name"):
        errors.append("缺少事件名")
    if not event.get("user_id"):
        errors.append("缺少用户标识")
    if not event.get("timestamp"):
        errors.append("缺少时间戳")
    if event.get("properties") is None:
        errors.append("缺少属性字典")
    return errors
```

## 自助分析平台设计

```mermaid
flowchart TB
    USER[业务用户] --> SQL[SQL查询界面]
    SQL --> ENGINE[查询引擎]
    ENGINE --> META[元数据层]
    META --> DICT[数据字典]
    META --> LINEAGE[血缘关系]
    ENGINE --> CACHE[查询缓存]
    ENGINE --> RESULT[结果展示]
    RESULT --> CHART[可视化]
    RESULT --> EXPORT[导出]
```

### 自助分析核心能力

| 能力 | 说明 | 技术实现 |
|------|------|----------|
| SQL编辑器 | 自动补全、语法高亮 | Monaco Editor |
| 数据字典 | 表/字段自动展示 | 元数据服务 |
| 血缘追踪 | 影响分析 | 解析SQL AST |
| 查询加速 | 预计算+缓存 | Presto/Trino |
| 权限控制 | 行列级权限 | Ranger/Sentry |
| 结果导出 | Excel/CSV/JSON | 异步下载 |

## 面试高频问题（15+ 条）

1. **实时大屏怎么做？** Kafka 采集 → Flink 窗口预聚合 → Doris/StarRocks 存储 → BI 查询；明细落湖供对账。
2. **用户画像用什么架构？** Lambda：Flink 实时标签 + Spark 离线标签，写同一张 HBase/ES 表。
3. **HBase RowKey 怎么设计？** 前缀散列（防热点）+ 用户 ID；查询模式决定 RowKey 顺序。
4. **实时风控用什么技术？** Flink CEP 做复杂模式匹配，Redis 存滑动窗口计数器，三层决策（实时/近实时/离线）。
5. **什么是 Lambda 架构？** 批层（全量准确）+ 速度层（实时增量），查询时合并。
6. **数仓分层 ODS/DWD/DWS/ADS 各做什么？** ODS 贴源、DWD 清洗明细、DWS 汇总主题、ADS 面向应用。
7. **指标口径怎么统一？** 统一原子指标 + 命名规范 + 指标血缘，同指标只在一处计算。
8. **传统数仓怎么迁到湖仓？** CDC 双写 + DataX 存量 → 对账 → 灰度切流 → 观察期下线旧库。
9. **数据湖和数据仓库的区别？** 湖存原始多格式、灵活但治理弱；仓存结构化、治理强但灵活性差；湖仓一体兼得。
10. **Flink 和 Spark 怎么选型？** 实时用 Flink，离线批用 Spark；流批一体场景两者皆可。
11. **实时数仓和离线数仓怎么打通？** 同一份 Iceberg 数据，Flink 流写、Spark 批读，口径天然一致。
12. **数据质量怎么保障？** 元数据（Atlas 血缘）+ 质量规则（Griffin 校验）+ 监控告警 + 数据 Owner。
13. **Kafka 在大数据链路里的角色？** 统一消息中枢：承接采集、缓冲削峰、解耦上下游。
14. **湖仓一体的核心是什么？** 开放表格式（Iceberg/Paimon）+ 统一 Catalog + 多引擎共读一份数据。
15. **数据中台的核心能力？** 数据汇聚 → 数据治理 → 数据服务 → 数据应用；指标体系是中枢。
16. **数仓迁移怎么保证口径一致？** 新旧库逐笔对账 + 差异阈值控制 + 灰度切流 + 快速回滚。
