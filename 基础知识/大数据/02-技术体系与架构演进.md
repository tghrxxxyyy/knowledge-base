...
| 组件 | 批处理方案 | 流处理方案 | 融合方案 |
|------|-----------|-----------|---------|
| 计算 | Spark | Flink | Apache Beam |
| 存储 | HDFS/S3 | Kafka | 数据湖 |
| 调度 | Airflow | Flink | K8s+Volcano |
| 监控 | Prometheus | Prometheus | 统一监控 |

```mermaid
flowchart LR
    A[数据源] --> B{处理模式?}
    B -->|批处理| C[Spark on K8s]
    B -->|流处理| D[Flink on K8s]
    B -->|融合| E[Apache Beam]
    C --> F[数据湖]
    D --> F
    E --> F
    F --> G[统一查询<br/>Trino/StarRocks]
```
...
### 35.1 Hadoop 1→2→3 演进

```
Hadoop 1.x（2006）：
  组件：HDFS + MapReduce
  问题：MapReduce 既是计算框架又是资源调度框架
  局限：只支持 MapReduce 一种计算模型

Hadoop 2.x（2013）：
  新增：YARN（资源调度框架）
  改进：MapReduce 只是 YARN 上的一个计算框架
  支持：Spark/Tez/Flink 等多种计算框架
  优势：资源利用率提升、多框架支持

Hadoop 3.x（2017）：
  新增：Erasure Coding（纠删码）、YARN Timeline Service v2
  改进：NameNode HA、YARN 调度优化
  优势：存储成本降低（EC 比副本省 50%）、调度性能提升

演进路线：
  Hadoop 1.x → Hadoop 2.x（YARN）→ Hadoop 3.x（EC+优化）→ 云原生（K8s）
```

### 35.2 Hadoop 组件版本兼容性

| 组件 | Hadoop 2.x | Hadoop 3.x | 说明 |
|------|------------|------------|------|
| HDFS | 2.x | 3.x | EC 支持 |
| YARN | 2.x | 3.x | 调度优化 |
| MapReduce | 2.x | 3.x | 兼容 |
| Spark | 1.x/2.x | 2.x/3.x | 需匹配版本 |
| Hive | 1.x/2.x | 2.x/3.x | 需匹配版本 |
| HBase | 1.x | 2.x | 需匹配版本 |

---

## 三十六、YARN vs K8s 调度深入对比
...
|------|------|------------|
| 调度单位 | Container | Pod |
| 资源模型 | CPU/Memory | CPU/Memory/GPU/自定义 |
| 调度策略 | FIFO/Capacity/Fair | Priority/Preemption |
| 弹性伸缩 | 手动/自动 | HPA/VPA/Autoscaler |
| 服务发现 | 无原生 | Service/DNS |
| 负载均衡 | 无原生 | Service/Ingress |
| 配置管理 | 无原生 | ConfigMap/Secret |
| 日志收集 | 无原生 | Sidecar/EFK |
| 监控告警 | 无原生 | Prometheus/Grafana |

### 36.2 迁移决策树

```mermaid
flowchart TD
    A[YARN → K8s 迁移] --> B{现有生态?}
    B -->|Hadoop 生态重| C[渐进式迁移]
    B -->|新项目/轻量| D[直接上 K8s]
    C --> E[Spark on K8s]
    C --> F[Flink on K8s]
    C --> G[Hive on K8s]
...
```
批流融合架构：
  统一存储：数据湖（Iceberg/Hudi/Delta）
  统一计算：Flink（批流一体）/ Spark Structured Streaming
  统一调度：YARN/K8s
  统一治理：元数据/质量/血缘

  技术选型：
    存储层：Iceberg（推荐）/ Hudi / Delta Lake
    计算层：Flink（流为主）/ Spark（批为主）
    调度层：K8s（云原生）/ YARN（传统）
    治理层：Atlas + Griffin + DataHub
```
...
public class BatchStreamJob {
    public static void main(String[] args) throws Exception {
        ...
        // 流处理：实时统计
        DataStream<OrderStats> streamStats = orders
            .keyBy(Order::getProductId)
            .window(TumblingProcessingTimeWindows.of(Time.minutes(1)))
            .aggregate(new OrderAggregator());
        
        // 批处理：离线统计
        DataStream<OrderStats> batchStats = orders
            .keyBy(Order::getProductId)
            .window(TumblingProcessingTimeWindows.of(Time.days(1)))
            .aggregate(new OrderAggregator());
        
        env.execute("Batch Stream Job");
    }
}
```
...
| 格式 | 特点 | 压缩 | 适用场景 |
|------|------|------|---------|
| Parquet | 列式、嵌套 | 高 | OLAP 查询 |
| ORC | 列式、索引 | 高 | Hive 查询 |
| Avro | 行式、Schema | 中 | 流式数据 |
| JSON | 半结构化 | 低 | 日志、API |
| Delta Lake | ACID 事务 | 高 | 数据湖 |

### 34.3 数据压缩算法对比

| 算法 | 压缩比 | 速度 | CPU 开销 | 适用场景 |
|------|--------|------|---------|---------|
| Gzip | 高 | 慢 | 高 | 冷数据归档 |
| Snappy | 中 | 快 | 低 | 热数据 |
| LZ4 | 中 | 极快 | 极低 | 实时流 |
...
|------|------|-----------|
| 资源模型 | 内存+CPU | 内存+CPU+GPU |
| 调度策略 | FIFO/容量/公平 | 请求+限制+亲和性 |
| 弹性伸缩 | 有限 | 原生支持 |
| 服务发现 | 有限 | 原生 DNS |
| 网络模型 | 简单 | CNI/Service Mesh |
| 存储 | HDFS | PV/PVC/CSI |
...
### 36.1 Lambda vs Kappa vs 统一架构

| 架构 | 批处理 | 流处理 | 一致性 | 复杂度 |
|------|--------|--------|--------|--------|
| Lambda | 离线批 | 实时流 | 可能不一致 | 高 |
| Kappa | 统一流 | 统一流 | 一致 | 中 |
| 统一引擎 | 一个引擎 | 一个引擎 | 一致 | 低 |
...
## 补充：Hadoop→Spark→Flink 演进

### 技术演进路线

```text
大数据技术演进：
  第一阶段（2006-2013）：Hadoop 生态
    ├── HDFS：分布式存储
    ├── MapReduce：批处理
    ├── Hive：SQL 接口
    └── 问题：编程复杂、延迟高

  第二阶段（2014-2019）：Spark 生态
    ├── Spark Core：内存计算
    ├── Spark SQL：SQL 接口
    ├── Spark Streaming：微批处理
    └── 改进：性能提升、易用性

  第三阶段（2020-至今）：Flink 生态
    ├── Flink Core：流处理
    ├── Flink SQL：SQL 接口
    ├── Flink CDC：数据同步
    └── 改进：实时处理、流批一体
...
数据仓库：
  ├── 定义：面向主题的、集成的、相对稳定的数据集合
  ├── 特点：结构化数据、预定义Schema、ACID事务
  ├── 适用：BI报表、历史分析
  └── 技术：Hive、Greenplum、ClickHouse

数据湖：
  ├── 定义：存储原始数据的存储库
  ├── 特点：原始数据、按需处理、Schema on Read
  ├── 适用：数据探索、机器学习
...
| 数据探索 | 数据湖 | 灵活、低成本 |
| 数据共享 | 数据中台 | 服务化、能力复用 |
| 机器学习 | 数据湖+数据仓库 | 原始数据+聚合数据 |
| 实时分析 | 数据仓库+流处理 | 实时数据+历史数据 |
...
| 场景 | 推荐技术 | 备选技术 | 说明 |
|------|----------|----------|------|
| BI报表 | ClickHouse | StarRocks | 高性能OLAP |
| 数据湖 | Delta Lake | Iceberg | ACID事务 |
| 流处理 | Flink | Spark Streaming | 实时处理 |
| 批处理 | Spark | Hive | 大数据量 |
| 数据同步 | Flink CDC | Canal | 实时同步 |
| 数据治理 | Atlas | DataHub | 元数据管理 |
...
| 级别 | 描述 | 特征 | 技术栈 |
|------|------|------|--------|
| L1 | 初始级 | 手动处理 | Excel/SQL |
| L2 | 可重复级 | 脚本自动化 | Shell/Python |
| L3 | 已定义级 | 流程标准化 | Hive/Spark |
| L4 | 已管理级 | 平台化 | 数据仓库+数据湖 |
| L5 | 优化级 | 智能化 | AI驱动优化 |
...
| 技术 | 优点 | 缺点 | 适用场景 |
|------|------|------|----------|
| Hadoop | 生态成熟 | 复杂度高 | 大数据存储 |
| Spark | 性能高 | 内存需求大 | 批处理 |
| Flink | 实时处理 | 学习曲线陡 | 流处理 |
| Hive | SQL支持好 | 延迟高 | 离线分析 |
| ClickHouse | 高性能 | 运维复杂 | OLAP分析 |
...
| 角色 | 职责 | 技能要求 |
|------|------|----------|
| 数据工程师 | 数据采集、处理、存储 | ETL、大数据技术 |
| 数据分析师 | 数据分析、报表、洞察 | SQL、统计学 |
| 数据科学家 | 机器学习、模型、预测 | Python、机器学习 |
| 数据架构师 | 架构设计、技术选型 | 架构设计、技术视野 |
| 数据治理师 | 数据质量、安全、合规 | 数据治理、法规 |
...
class SLAMonitor:
    ...
    def generate_sla_report(self):
        ...
        report = {
            ...
            'sla_score': 1 - len(self.violations) / 100
        }
        return report
```
...
class DataGovernance:
    ...
    def define_standard(self, standard_name, standard_definition):
        """定义数据标准"""
    ...
    def define_policy(self, policy_name, policy_definition):
        """定义数据策略"""
    ...
    def audit_compliance(self, data_source):
        """审计合规性"""
        compliance_report = {
        ...
        for standard_name, standard in self.standards.items():
            compliance_report['standards_compliance'][standard_name] = True
        
        # 检查策略合规性
        for policy_name, policy in self.policies.items():
            compliance_report['policies_compliance'][policy_name] = True
...
### Hadoop 1.x → 2.x → 3.x 关键变化

```text
Hadoop 1.x 架构：
  JobTracker（单点）→ TaskTracker
  问题：JobTracker 单点瓶颈，资源利用率低
...
Hadoop 3.x 架构（2017）：
  YARN 调度器优化（公平调度/容量调度）
  纠删码（Erasure Coding）替代 3 副本
  改进：存储效率提升 50%
  问题：生态复杂度持续增长

Hadoop 4.x+（云原生）：
  YARN on K8s（Spark on K8s 原生支持）
  对象存储替代 HDFS
  问题：传统 Hadoop 生态逐渐式微
```
...
| 组件 | Hadoop 1.x | Hadoop 2.x | Hadoop 3.x+ |
|------|------------|------------|-------------|
| 资源管理 | JobTracker | YARN | YARN + K8s |
| 存储 | HDFS | HDFS | HDFS + 对象存储 |
| 计算 | MR v1 | MR v2/Spark/Flink | Spark/Flink |
| 安全 | Kerberos 基础 | Kerberos + Knox | Ranger + Knox |
| 纠删码 | 无 | 无 | 支持 |
| 纠删码效率 | 3x 冗余 | 3x 冗余 | 1.5x 冗余 |
...
| 维度 | YARN | Kubernetes |
|------|------|------------|
| 资源模型 | 内存/CPU slot | 内存/CPU/GPU/Pod |
| 调度器 | FIFO/公平/容量 | 优先级/公平/自定义 |
| 容器 | JVM 进程 | OCI 容器 |
| 弹性 | 静态资源池 | HPA/VPA 自动扩缩容 |
| 生态集成 | Hadoop 生态 | 云原生生态 |
| 运维复杂度 | 中 | 高 |
| 存储 | HDFS | PV/PVC/CSI |
...
```mermaid
flowchart TB
    A[Spark on K8s] --> B[YARN on K8s]
    B --> C[Kubernetes 集群]
    C --> D[Pod 调度]
    C --> E[存储卷]
    C --> F[网络策略]

    B --> G[YARN ResourceManager]
    B --> H[YARN NodeManager]
    G --> I[资源分配]
    H --> J[容器执行]
```
...
| 架构 | 批处理 | 流处理 | 统一 | 说明 |
|------|--------|--------|------|------|
| Lambda | 批处理层+速度层 | 实时+离线 | ❌ | 两套代码维护 |
| Kappa | 流处理 | 流处理覆盖批 | ❌ | 代码简化 |
| 湖仓一体 | Spark/Flink | Flink/Flink | ✅ | 存储统一 |
...
批流融合技术栈：
  ...
  调度层：
    批调度：Airflow、DolphinScheduler
    流调度：Flink Checkpoint
    混合调度：统一调度器

  服务层：
    批服务：Trino、Spark SQL
    流服务：Flink SQL、Kafka Streams
    统一服务：Trino（批流统一查询）
```
...
评分维度（满分 10 分）：
  ...
  3. 生态（15%）：集成工具、第三方支持
  4. 运维（15%）：部署复杂度、监控、告警
  5. 成本（15%）：硬件要求、许可费用、人力成本
  6. 扩展性（10%）：水平扩展、容错能力
  7. 安全（10%）：认证、授权、加密、审计

  选型示例：
    OLAP：ClickHouse（8.5分）vs Doris（8.2分）vs StarRocks（8.0分）
    存储：HDFS（7分）vs S3（8.5分）vs Delta Lake（8分）
    计算：Spark（9分）vs Flink（8.5分）vs Presto（7.5分）
    调度：Airflow（8分）vs DolphinScheduler（7.5分）vs Nifi（7分）
```
...
阶段四：湖仓一体
  存储：数据湖 + 事务层（Delta Lake/Iceberg）
  计算：批流一体（Flink/Spark）
  查询：统一 SQL 接口（Trino）
  目标：数据资产化、智能决策

  技术栈选择：
    存储：Iceberg（开放标准）> Delta Lake > Hudi
    计算：Flink（流优先）> Spark（批优先）
    查询：Trino（性能）> Presto（兼容）
    元数据：DataHub（轻量）> Atlas（重量）
```
...
| 维度 | 传统数仓 | 数据中台 | 湖仓一体 |
|------|---------|---------|---------|
| 核心目标 | 分析查询 | 数据服务 | 分析+服务 |
| 数据模型 | 维度建模 | 统一建模 | 灵活建模 |
| 存储 | MPP/OLAP | 混合 | 数据湖+事务 |
| 计算 | 批处理为主 | 批+流 | 批流一体 |
| 服务 | BI/报表 | API/标签 | BI+API |
| 治理 | 弱 | 强 | 中 |
| 扩展性 | 中 | 高 | 高 |
| 适用场景 | 企业级分析 | 互联网业务 | 现代数据栈 |

### 架构选择决策

```text
选型建议：
  1. 传统企业 + 分析为主 → 数仓（Teradata/Snowflake）
  2. 互联网 + 服务为主 → 数据中台（自研）
  3. 混合需求 + 灵活性 → 湖仓一体（Databricks/自研）
  4. 小团队 + 成本敏感 → 湖仓（Iceberg + Trino）
  5. 大团队 + 技术储备 → 数据中台 + 湖仓
```
...
工具推荐：
  ...
  选型模板：Excel/Git
  PoC 环境：K8s 集群/云服务
  监控评估：Grafana/Prometheus
```

---

## 三十八、数据湖 vs 数据仓库 vs 数据中台辨析