...
### 28.1 Lambda 与 Kappa 架构选型

| 架构 | 优势 | 劣势 | 适用场景 | 团队要求 |
|------|------|------|---------|---------|
| Lambda | 离线+实时双链路成熟 | 两套代码维护成本高 | 历史数据回溯+实时并行 | 大团队、遗留系统迁移 |
| Kappa | 统一代码、统一流处理 | 重算历史成本高、依赖强 | 纯实时场景、新系统 | 精干团队、流处理成熟 |
| 湖仓一体 | 批流统一存储层 | 表格式选型复杂 | 多模态数据、成本敏感 | 中等团队、云原生 |
...
Lambda 架构分层设计：
  ...
  速度层（Speed Layer）：
    - 增量实时计算
    - Flink/Kafka Streams 实时作业
    - 输出：实时视图（Real-time View）
    - 延迟：秒级/毫秒级
...
| 分层 | 存储选型 | 计算延迟 | 保留策略 | 典型任务 |
|------|---------|---------|---------|---------|
| ODS 原始层 | Kafka（7-14天） | 实时 | 短期保留 | 数据采集、CDC 同步 |
| DWD 明细层 | Kafka（3-7天） | 秒级 | 中期保留 | 清洗、关联、转换 |
| DWS 汇总层 | Kafka + ClickHouse | 秒级 | 长期保留 | 聚合、窗口、特征 |
| ADS 应用层 | Redis + ClickHouse | 毫秒级 | 按需 | 查询、服务、导出 |
...
public class StreamBatchWordCount {
    public static void main(String[] args) throws Exception {
        ...
        counts.print();
        env.execute("StreamBatch WordCount");
    }
...
# flink-conf.yaml
# 流批一体配置
execution.runtime-mode: streaming  # 或 batch
...
| 维度 | 流批一体 | 纯流处理 | 说明 |
|------|---------|---------|------|
| 代码复用 | 高（同一套代码） | 低（流批分离） | 维护成本差异显著 |
| 延迟 | 秒级（流模式） | 毫秒级 | 批模式引入延迟 |
| 吞吐 | 高（批模式优化） | 中（流模式限制） | 批模式更适合大任务 |
| 状态管理 | 统一（RocksDB） | 统一 | 一致性保障相同 |
| 适用场景 | 混合负载 | 纯实时 | 根据业务选择 |
...
| 策略 | 实现方式 | 检测范围 | 恢复方式 | 适用场景 |
|------|---------|---------|---------|---------|
| 全量对账 | 每日离线全量比对 | 所有指标 | 回滚修复 | 核心指标 |
| 抽样对账 | 随机抽样比对 | 部分指标 | 预警通知 | 非核心指标 |
| 实时对账 | Flink CEP 检测 | 关键指标 | 自动告警 | 实时性要求高 |
| 双写对账 | 同时写入实时/离线 | 所有指标 | 一致性保障 | 数据一致性要求高 |
...
SELECT 
    ...
    r.realtime_value,
    o.offline_value,
    ABS(r.realtime_value - o.offline_value) as diff,
    CASE 
        WHEN ABS(r.realtime_value - o.offline_value) < 0.01 THEN '一致'
        WHEN ABS(r.realtime_value - o.offline_value) < 0.1 THEN '轻微偏差'
        ELSE '严重偏差'
    END as status
FROM realtime_metrics r
JOIN offline_metrics o 
    ON r.metric_date = o.metric_date 
    AND r.metric_name = o.metric_name
WHERE ABS(r.realtime_value - o.offline_value) > 0.01
ORDER BY diff DESC;
```

---

## 三十一、Trino on Iceberg 实践

### 31.1 Trino + Iceberg 配置

```yaml
# Trino catalog 配置（Iceberg）
connector.type=iceberg
connector.catalog=iceberg_catalog
connector.config-class=org.apache.iceberg.spark.SparkCatalogConfig

# Iceberg catalog 配置
catalog.name=iceberg_catalog
catalog.type=hive
catalog.uri=thrift://hive-metastore:9083
catalog.warehouse=s3://iceberg-warehouse
...
-- 数据演化（更新数据）
UPDATE iceberg_catalog.db.events 
SET event_type = 'completed' 
WHERE event_id = 123;
```

### 31.3 Trino on Iceberg 性能优化

| 优化策略 | 实现方式 | 效果 | 适用场景 |
|---------|---------|------|---------|
| 分区剪裁 | 按时间/类型分区 | 查询加速 5-10x | 时间范围查询 |
| 列裁剪 | Parquet 列式存储 | IO 减少 3-5x | 宽表查询 |
| 谓词下推 | 过滤条件下推到存储层 | IO 减少 2-3x | 条件查询 |
| 小文件合并 | 定期 Compaction | 查询加速 2-5x | 小文件过多 |
| 缓存利用 | 数据缓存到本地 | 查询加速 2-3x | 重复查询 |
...
| 策略 | 实现方式 | 成本节省 | 适用场景 |
|------|---------|---------|---------|
| 冷热分层 | 按访问频率分层存储 | 30-50% | 访问模式清晰 |
| 数据压缩 | Parquet + Zstandard | 20-40% | 文本数据为主 |
| 小文件合并 | 定期 Compaction | 10-20% | 小文件过多 |
| 保留策略 | 自动过期删除 | 10-30% | 历史数据清理 |
| 列式存储 | Parquet/ORC | 20-50% | 分析型查询 |
...
spark:
  ...
  dynamicAllocation.executorIdleTimeout: 60s
...
| 指标 | 告警阈值 | 监控方式 | 处理建议 |
|------|---------|---------|---------|
| 存储增长率 | > 20%/月 | CloudWatch/S3 metrics | 清理过期数据 |
| 计算成本 | > 预算 80% | Billing alerts | 优化作业配置 |
| 数据传输 | > 1TB/月 | VPC Flow Logs | 就近访问优化 |
| 查询成本 | > $0.1/查询 | Trino metrics | 优化查询逻辑 |
...
| 监控维度 | 关键指标 | 采集工具 | 告警阈值 |
|---------|---------|---------|---------|
| 数据延迟 | 端到端延迟 | Flink Metrics | > 5s |
| 数据质量 | 空值率、重复率 | Great Expectations | > 1% |
| 系统资源 | CPU、内存、GC | Prometheus | > 80% |
| 业务指标 | 数据量、吞吐量 | Grafana | 异常波动 |
...
# Prometheus 告警规则
groups:
  - name: realtime_data_warehouse
    rules:
      - alert: DataDelayHigh
        expr: flink_job_latency_seconds > 5
        for: 2m
        labels:
          severity: warning
        annotations:
          summary: "数据延迟过高"
          description: "作业 {{ $labels.job }} 延迟 {{ $value }}s"
      
      - alert: DataQualityIssue
        expr: data_quality_null_rate > 0.01
        for: 5m
        labels:
          severity: critical
        annotations:
          summary: "数据质量问题"
          description: "表 {{ $labels.table }} 空值率 {{ $value }}"
      
      - alert: StorageCostHigh
        expr: aws_s3_storage_cost > 1000
        for: 1h
        labels:
          severity: warning
        annotations:
...
graph TB
    ...
    C --> F{数据量?}
    F -->|TB级| G[Flink + Kafka]
    F -->|PB级| H[Flink + Iceberg]
    D --> I{成本敏感?}
    I -->|是| J[Spark Structured Streaming]
    I -->|否| K[Flink SQL]
    E --> L{生态要求?}
    L -->|成熟| M[Spark + Hive]
    L -->|云原生| N[Spark + Iceberg]
```
...
| 场景 | 推荐技术栈 | 优势 | 劣势 | 适用团队 |
|------|-----------|------|------|---------|
| 实时分析 | Flink + Kafka + ClickHouse | 延迟低、查询快 | 运维复杂 | 精干团队 |
| 离线分析 | Spark + Hive + Iceberg | 成本低、生态好 | 延迟高 | 大团队 |
| 湖仓一体 | Flink + Iceberg + Trino | 批流统一 | 技术门槛高 | 中等团队 |
| 实时特征 | Flink + Redis + HBase | 特征实时 | 存储成本高 | 算法团队 |
...
| 阶段 | 技术栈 | 特点 | 团队要求 | 投入 |
|------|--------|------|---------|------|
| 初期 | Kafka + Flink + ClickHouse | 快速验证 | 小团队 | 低 |
| 中期 | Flink + Iceberg + Trino | 批流统一 | 中等团队 | 中 |
| 成熟 | 湖仓一体 + 数据网格 | 数据自治 | 大团队 | 高 |
| 进阶 | 数据网格 + 数据编织 | 分布式治理 | 专业团队 | 高 |
...
gantt
    ...
    section 基础建设
    Kafka集群部署     :a1, 2024-01-01, 30d
    Flink集群部署     :a2, after a1, 30d
    监控体系建立       :a3, after a2, 30d
    section 能力提升
    Iceberg引入       :b1, 2024-04-01, 60d
    数据质量体系       :b2, after b1, 60d
    性能优化          :b3, after b2, 30d
    section 生态完善
    数据治理平台       :c1, 2024-07-01, 90d
    数据资产化         :c2, after c1, 60d
    section 智能化
    数据网格实践       :d1, 2024-10-01, 90d
    自动化运维         :d2, after d1, 60d
```
