...
| 维度 | 检查内容 | 工具 | 处理方式 |
|------|----------|------|----------|
| 完整性 | 数据是否完整 | 自定义规则 | 告警+重试 |
| 准确性 | 数据是否正确 | Schema校验 | 拒绝+日志 |
| 一致性 | 多源数据一致 | 对账脚本 | 修复+告警 |
| 时效性 | 数据是否及时 | 监控指标 | 告警+扩容 |
| 唯一性 | 数据是否重复 | 去重规则 | 去重+告警 |
...
sequenceDiagram
    ...
    participant FlinkSQL as Flink SQL
    
    DB->>FlinkCDC: Binlog变更事件
    FlinkCDC->>FlinkCDC: 解析Binlog
    FlinkCDC->>Kafka: 发送变更数据
    Kafka->>FlinkSQL: 消费变更数据
    FlinkSQL->>FlinkSQL: 实时处理
    FlinkSQL->>目标: 写入目标系统
```
...
### Flink CDC vs Canal vs Debezium对比

| 维度 | Flink CDC | Canal | Debezium |
|------|-----------|-------|----------|
| 数据源 | 多种数据库 | MySQL | 多种数据库 |
| 输出目标 | Flink处理 | Kafka/MQ | Kafka/MQ |
| 实时性 | 毫秒级 | 秒级 | 秒级 |
| 事务支持 | 支持 | 不支持 | 不支持 |
| 依赖 | Flink | Canal Server | Kafka Connect |
| 运维复杂度 | 中 | 高 | 中 |
...
| 拦截器类型 | 功能 | 使用场景 |
|------------|------|----------|
| Timestamp | 添加时间戳 | 时间序列数据 |
| Host | 添加主机信息 | 多源数据区分 |
| Regex | 正则过滤 | 日志过滤 |
| Static | 添加静态字段 | 固定标签 |
| Search and Replace | 搜索替换 | 数据清洗 |
| Custom | 自定义拦截器 | 特殊需求 |
...
```java
// 自定义日志解析拦截器
public class LogParserInterceptor implements Interceptor {
    @Override
    public Event intercept(Event event) {
        ...
        if (matcher.matches()) {
            ...
            headers.put("remote_addr", matcher.group(1));
            headers.put("time_local", matcher.group(2));
            headers.put("request", matcher.group(3));
            headers.put("status", matcher.group(4));
            headers.put("body_bytes_sent", matcher.group(5));
            headers.put("http_referer", matcher.group(6));
            headers.put("http_user_agent", matcher.group(7));
        }
...
| 参数 | 建议值 | 说明 |
|------|--------|------|
| pipeline.workers | CPU核心数 | 处理线程数 |
| pipeline.batch.size | 1000-5000 | 批量处理大小 |
| pipeline.batch.delay | 5-50 | 批量等待时间 |
| persistent_queues | true | 持久化队列 |
...
## vs Canal vs Debezium vs Maxwell对比

### CDC工具对比

| 维度 | Canal | Debezium | Maxwell | Flink CDC |
|------|-------|----------|---------|-----------|
| 数据源 | MySQL | 多种数据库 | MySQL | 多种数据库 |
| 输出目标 | Kafka/MQ | Kafka/MQ | Kafka | Flink处理 |
| 部署模式 | Server | Connect | Server | 嵌入式 |
| 事务支持 | 不支持 | 不支持 | 不支持 | 支持 |
| 运维复杂度 | 高 | 中 | 中 | 低 |
| 性能 | 高 | 高 | 中 | 高 |
...
| 指标类别 | 指标名 | 说明 | 告警阈值 |
|---------|--------|------|---------|
| 延迟 | binlog_lag | CDC延迟（秒） | >300s |
| 延迟 | sync_delay | 端到端同步延迟 | >300s |
| 积压 | consumer_lag | Kafka消费积压 | >100000 |
| 积压 | backlog_size | 数据积压量 | >1GB |
...
class RangeRule:
    def __init__(self, field, min_val, max_val):
        ...
        self.name = f"{field}必须在{min_val}-{max_val}之间"
...
| 问题 | 排查步骤 | 解决方案 |
|------|----------|----------|
| 延迟高 | 检查网络/Kafka | 增加消费者 |
| 数据丢失 | 检查ACK机制 | 开启事务 |
| Schema不兼容 | 检查Schema变更 | 兼容性处理 |
...
```python
# 数据对账脚本示例
def reconcile_data(source_count, target_count, tolerance=0.01):
    """数据对账检查"""
    diff = abs(source_count - target_count)
    diff_rate = diff / source_count if source_count > 0 else 0
    
    if diff_rate > tolerance:
        # 差异超出容差
        alert(f"数据差异: {diff_rate:.2%}")
        # 触发数据修复
        trigger_data_repair()
        return False
...
# 增量对账
def incremental_reconciliation(source_table, target_table, time_range):
    """增量数据对账"""
    source_count = query_count(source_table, time_range)
    target_count = query_count(target_table, time_range)
    
    if not reconcile_data(source_count, target_count):
        # 记录差异明细
        log_difference_details(source_table, target_table, time_range)
        # 触发数据修复任务
        schedule_repair_task(source_table, target_table, time_range)
```
...
### Canal vs Debezium vs Maxwell对比
