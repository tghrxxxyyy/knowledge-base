# 操作系统（深度原理）

> 操作系统是"资源的管理者 + 硬件的抽象层"：向上给应用一套干净稳定的接口（进程、文件、网络、内存），向下把 CPU、内存、磁盘、网卡的细节藏起来。本文从**后端工程师**视角深挖 OS 的六大支柱——**进程线程、CPU 调度、同步互斥、内存管理、文件系统、I/O 与网络**，并落到 Linux 真实机制与 Java 应用（线程、堆、Netty、容器）的对应关系。
>
> 与基础知识其他文档互补：底层硬件与编译链接见「[计算机原理](计算机原理.md)」、JVM 内存模型见「[Java虚拟机](Java虚拟机.md)」、Java 并发与锁见「[并发编程](并发编程.md)」、排查命令见「[Linux排查](Linux排查.md)」、网络协议见「[网络](网络.md)」。本文聚焦**内核机制与工程落地**。

---

## 一、操作系统是干什么的

### 1.1 三个本质职责

| 职责 | 含义 | 典型机制 |
|------|------|----------|
| **资源抽象** | 把杂乱硬件伪装成统一接口 | 文件（抽象磁盘）、进程（抽象 CPU/内存）、socket（抽象网卡） |
| **资源多路复用** | 一份硬件服务多个程序 | 时间复用（分时调度）、空间复用（虚拟内存、文件共享） |
| **隔离与安全** | 程序之间互不干扰、无法越权 | 用户态/内核态、虚拟地址空间、权限位 |

> 一句话：**OS 是"把一台机器变成一堆虚拟机器的管理程序"**——每个进程都以为自己独占整台机器。

### 1.2 用户态 / 内核态 / 系统调用

```mermaid
flowchart LR
    APP[应用: 用户态 ring3] -->|syscall 陷阱指令| KERNEL[内核: 内核态 ring0]
    KERNEL -->|返回 + 结果| APP
    KERNEL --> HW[CPU/内存/磁盘/网卡]
```

- **用户态（ring3）**：受限，不能直接访问硬件、不能执行特权指令。
- **内核态（ring0）**：完整权限，管理一切资源。
- **切换（syscall）**：应用发起**陷阱（trap/软中断）**陷入内核，执行系统调用（`read/write/fork/socket/mmap`），再返回用户态。
- **切换开销**：保存/恢复寄存器与上下文、权限位切换、可能污染 TLB/Cache。**频繁小粒度系统调用是大坑** → 用缓冲区/批量读写（如 `BufferedWriter`、`sendfile`、批量 SQL）。

> 口诀：**"系统调用是唯一合法入口；缓冲是为了少进内核。"**

### 1.3 内核架构流派

| 流派 | 特点 | 代表 |
|------|------|------|
| **宏内核（Monolithic）** | 全部内核服务放内核态，性能好，但一崩全崩 | Linux、Unix |
| **微内核（Micro）** | 只留最小核心，驱动/文件系统放用户态，稳定安全，但 IPC 慢 | QNX、L4、Minix |
| **混合内核** | 宏内核里融微内核思想（模块化、可卸载） | macOS(XNU)、Windows NT |

Linux 的**模块机制**（`insmod/rmmod`）是"可伸缩宏内核"：驱动可动态加载/卸载，兼顾性能与灵活性。

---

## 二、进程与线程

### 2.1 核心概念

| 概念 | 资源 | 切换成本 | 特点 |
|------|------|----------|------|
| **进程** | 独立地址空间 + PCB(task_struct) | 高（切页表/刷 TLB） | 资源分配的最小单位 |
| **线程** | 共享进程地址空间，独立栈/TLS/寄存器 | 中 | 调度的最小单位 |
| **协程/虚拟线程** | 用户态栈，不占内核资源 | 极低 | 由 runtime 调度（Go、Java Loom） |

Linux 中进程与线程都用 `task_struct` 表示（`clone()` 加共享标志即线程），没有独立"线程结构"。

### 2.2 进程状态机（Linux）

```mermaid
flowchart TD
    R[Running 运行/就绪] -->|等待资源| S[Sleeping 可中断休眠 S]
    R -->|不可中断等 IO| D[D 不可中断休眠]
    S -->|被唤醒| R
    D -->|IO 完成| R
    R -->|收到 SIGKILL/退出| Z[Zombie 僵尸 Z]
    S -->|fork 出子进程| Z
```

| 状态 | 含义 | 排查要点 |
|------|------|----------|
| R | 运行或就绪 | 正常 |
| S | 可中断休眠（等待） | 正常 |
| **D** | **不可中断休眠**（内核在等磁盘 IO 等） | 大量 D 说明磁盘 IO 卡住，`iostat` 看 wa% |
| Z | 僵尸（父进程未 wait 回收） | 父进程没 `wait`；查父子进程 |
| T | 停止（信号暂停） | 正常 |

### 2.3 fork / exec / COW

- **fork()**：复制父进程（成本高，用**写时复制 COW** 优化——父子共享物理页，只有一方写入才真正拷贝）。
- **exec()**：用新程序映像覆盖当前进程（保持 pid）。
- **wait()**：回收子进程，避免僵尸。
- **并发模型启示**：`fork` 出子进程处理连接（PPC/prefork）代价高、连接数有限，所以高并发服务转向**事件驱动 + 线程/协程**（见第七节 I/O）。

### 2.4 线程模型

| 模型 | 说明 | 代表 |
|------|------|------|
| 1:1（内核线程） | 用户线程映射内核线程，Java 默认 | Java Thread、Linux pthread |
| N:1（用户线程） | 多用户线程跑一个内核线程，一个阻塞全阻塞 | 早期绿色线程 |
| M:N | 多对多，折中但复杂 | 部分 runtime |

> **Java 线程 = 1 个内核线程**，创建销毁贵、阻塞会占内核栈。所以**线程池、协程（Java 21 虚拟线程 Loom）**是减少线程数/阻塞占用的关键。虚拟线程本质是运行在内核线程之上的海量用户态任务（M:N 思想）。

---

## 三、CPU 调度

### 3.1 调度目标

- **吞吐**（单位时间完成任务数）、**延迟**（响应快）、**公平**（不饿死）、**实时**（截止期保证）。目标互相冲突，需权衡。

### 3.2 Linux CFS（完全公平调度）

- 每个任务按**权重（nice 值）**换算成虚拟运行时间 `vruntime`，每次选 `vruntime` 最小的任务跑。
- 红黑树组织就绪队列，`O(log n)` 选取。
- **nice 每降 1，权重约 ×1.25**；nice 只是相对权重，不是绝对优先级。
- 组调度（cgroup CPU 子系统）：把"任务集合"当调度实体，保证容器/组内公平 → **容器 CPU 限制的基础**。

```mermaid
flowchart LR
    T[任务] -->|nice 权重换算| V[vruntime]
    V --> RB[红黑树按 vruntime 排序]
    RB -->|取最左最小| RUN[运行]
    RUN -->|时间片耗尽| V
```

### 3.3 实时调度与优先级反转

- **SCHED_FIFO / SCHED_RR**：实时策略，低延迟场景（音视频）。
- **优先级反转**：高优先级任务等待低优先级任务释放锁 → 被中等优先级任务插队饿死。
- **解决**：**优先级继承（Priority Inheritance）**——低优先级任务临时提升到高优先级去快速释放锁（Linux `rt_mutex` 实现）。
- **优先级篡改/CPU 配额**：容器用 **CFS 带宽控制**（`cpu.cfs_quota_us/cpu.cfs_period_us`）限制 CPU 用量。

### 3.4 上下文切换的成本

- 切换线程：保存/恢复寄存器 + 栈指针 + 用户态/内核态来回，约**微秒级**，但真正的代价是**缓存污染**（L1/TLB 失效）。
- 切换进程：还要切页表（CR3/TLB flush），更贵。
- 大规模并发（万级线程）会因**频繁上下文切换**反而变慢 → 事件驱动/协程（减少线程数）的核心理由。
- 排查：`vmstat 1` 看 `cs`（上下文切换数）；`pidstat -w` 看进程级。

> 口诀：**"CFS 公平看 vruntime，红黑树里取最左；实时调度防反转，用优先级继承；并发上量先别加线程，事件驱动或协程。"**

---

## 四、同步与互斥

### 4.1 原子性与原语

- **原子操作**：`LOCK` 前缀指令（x86）/`ldxr-stxr`（ARM）——在硬件层面保证读-改-写不被打断（Java `Atomic*` 的 CAS 底层）。
- **内存屏障（Memory Barrier）**：约束 CPU/编译器重排，保证顺序/可见性（Java `volatile` 底层就有屏障）。
- **自旋锁**：忙等，不睡，适合临界区极短；持锁期间不允许被抢占（内核 `spinlock`）。
- **互斥量（mutex）**：竞争时睡眠，适合长临界区；持锁可被抢占。

| 原语 | 忙等/睡眠 | 适用 |
|------|-----------|------|
| 自旋锁 | 忙等 | 临界区纳秒级，多核（synchronized 轻量级锁自旋阶段） |
| mutex | 睡眠 | 临界区较长，竞争不激烈 |
| 读写锁 | 读共享/写互斥 | 读多写少 |
| 信号量 | 可计数 | 资源池、生产者消费者 |

### 4.2 futex（快速用户态锁）

Linux 主流锁的**内核地基**（Java `synchronized` 重量级、JUC `ReentrantLock` 的 park/unpark 都落在这里）：

```mermaid
flowchart TD
    A[用户态尝试 CAS 抢锁] -->|成功| B[直接进入临界区, 不进内核]
    A -->|失败| C[通过 futex 系统调用进内核]
    C --> D[挂到等待队列睡眠]
    D -->|释放方 futex_wake| E[被唤醒重新抢锁]
```

- **快速路径**：锁空闲时只做一次用户态 CAS，**不陷入内核**，开销极低。
- **慢速路径**：真正冲突才系统调用阻塞。
- 这就是现代锁"看似无锁"的性能来源。

### 4.3 死锁、饥饿、活锁

- **死锁四必要条件**：互斥、占有并等待、不可剥夺、循环等待。
- **预防**：破坏任一条件（一次性申请、全局按序加锁——最常见）。
- **避免**：银行家算法（需预知资源需求，实践少）。
- **检测与恢复**：资源分配图找环；超时重试。
- **饥饿**：一直得不到调度（锁不公平）；**活锁**：不断让出又互相让（都在"礼貌谦让"却无进展，如重试碰撞）。

> 与 Java 的对应：`synchronized` 无锁→偏向锁→轻量级锁（CAS 自旋）→重量级锁（内核 mutex/futex），就是"先用户态快速路径、后内核阻塞"的经典演绎（详见「[并发编程](并发编程.md)」与「[源码系列/JVM与并发底层源码](../源码系列/JVM与并发底层源码.md)」）。

---

## 五、内存管理

### 5.1 虚拟地址空间布局（64 位 Linux）

```text
高地址 0x7FFFFFFFFFFF
┌────────────────────────┐
│ 内核空间（共享，权限保护）│ ← 用户态不可访问
├────────────────────────┤
│ 栈 Stack（向下生长，每线程独立）│
├────────────────────────┤
│ mmap 区域（动态库/大块映射）│ ← mmap 分配大内存
├────────────────────────┤
│ 堆 Heap（向上生长，brk）│ ← malloc 小内存
├────────────────────────┤
│ BSS（未初始化全局） │
├────────────────────────┤
│ 数据段（已初始化全局）│
├────────────────────────┤
│ 代码段 text │
└────────────────────────┘ 低地址
```

- 虚拟内存三大作用：**隔离**（进程互不可见）、**扩展**（可 > 物理内存）、**按需加载**（只把用到的页调入）。
- 地址翻译由 **MMU + 多级页表 + TLB（快表缓存）** 完成。

### 5.2 分页与页表深讲

- 页大小默认 **4KB**；多级页表（x86-64 四级）解决"单级页表太大"问题。
- **TLB**：页表缓存，命中 1 周期，未命中需逐级走页表（几十周期）。进程切换刷 TLB 是昂贵操作。
- **HugePage（大页）**：如 2MB/1GB 页，减少页表项、提升 TLB 命中率；**MySQL/ES/Redis 大内存场景**常开启（`vm.nr_hugepages`）。

### 5.3 分配器：伙伴系统 / slab / malloc

| 层 | 作用 |
|----|------|
| **伙伴系统** | 内核按 2 的幂次管理物理页，抗**外部碎片** |
| **slab** | 内核对象缓存（如 `task_struct`、inode），复用+少**内部碎片** |
| **malloc/brk/mmap** | 用户态堆：小分配走 `brk` 扩展堆，大分配（>128KB）走 `mmap`（独立匿名映射，释放即还给系统） |

- **外部碎片**：空闲但不连续，凑不成大块 → 伙伴系统合并相邻空闲页。
- **内部碎片**：分配单元内的浪费 → slab 按对象大小精确切分。

### 5.4 缺页与写时复制

```mermaid
flowchart TD
    A[访问虚拟地址] --> B{页表项有效?}
    B -->|是| C[TLB/MMU 映射访问]
    B -->|否| D[缺页中断]
    D --> E{类型}
    E -->|合法未加载| F[从磁盘调入 + 更新页表]
    E -->|COW 写保护| G[拷贝物理页 + 取消写保护]
    E -->|非法访问| H[SIGSEGV 段错误]
```

- **按需调页（Demand Paging）**：只加载用到的页 → 程序启动快、内存省。
- **COW（写时复制）**：`fork()`、共享库、文件映射都靠它——共享只读页，写入才复制。Java 程序频繁 `fork`（如 Redis BGSAVE）时内存峰值翻倍，**需评估物理内存**。

### 5.5 OOM 与内存限制

- **OOM Killer**：内存耗尽时，内核按 `oom_score`（内存占用/存活时间/root 权重）杀进程。`dmesg | grep -i oom` 查凶手。
- **cgroup 内存限制**：容器/进程组可设 `memory.limit_in_bytes`；超限触发 OOM 或回收。
- **Java 在容器里的坑**：早期 JVM 用 `-Xmx` 不感知 cgroup，导致**容器被 OOM 杀掉**；JDK 10+ `UseContainerSupport` 默认感知，但**建议显式设置 `-Xmx` 并预留 headroom**（别让堆顶到 cgroup 上限）。

> 口诀：**"虚拟内存三作用：隔离扩展按需装；TLB 快表别打脸，大页 2M 省页表；外部碎片靠伙伴，内部碎片靠 slab；OOM 先看 dmesg，容器堆要留余量。"**

---

## 六、文件系统与存储

### 6.1 VFS 与核心对象

Linux 一切皆文件，通过 **VFS（虚拟文件系统）** 统一接口对接各类 FS：

| 对象 | 含义 |
|------|------|
| **超级块 super_block** | 整个文件系统的元信息（类型、大小、挂载点） |
| **inode** | 文件的元数据（权限、时间、数据块位置），不含文件名 |
| **dentry（目录项）** | 文件名 → inode 的映射，含路径缓存 |
| **file（文件描述符）** | 打开实例，含偏移量、读写模式 |

- **硬链接**：多个目录项指向同一 inode（同一个文件）；不能跨文件系统、不能链目录。
- **软链接**：独立 inode，内容是目标路径（类似快捷方式）；目标删除即失效。
- 硬链接数归零 + 无进程打开 → inode 才真正释放（这就是"文件删了但空间不释放"的根源，见 [Linux排查](Linux排查.md)）。

### 6.2 页缓存与写回

- **页缓存（Page Cache）**：读写文件先在内存缓存页，命中免磁盘 → 第二次读快。
- **脏页（Dirty）**：内存中已改未落盘，由 **writeback（pdflush）** 周期性刷盘。
- **崩溃一致性**：脏页未刷盘就掉电 → 数据丢失；**日志文件系统**（ext4 的 journal）用"先写日志、再写数据"保证崩溃可恢复。

### 6.3 零拷贝

传统 `read + write` 有 **4 次拷贝 + 4 次用户态/内核态切换**；零拷贝降到 2 次：

| 方式 | 拷贝次数 | 说明 |
|------|----------|------|
| read+write | 4（内核↔用户各 2） | 传统 |
| **mmap+write** | 3 | 省一次内核→用户拷贝，但 mmap 需 flush 且页错误开销 |
| **sendfile** | 2 | 文件→socket 零用户态参与（Java 见 `FileChannel.transferTo`） |
| **splice** | 2（管道） | 内核内搬运，可组合复杂路径 |

> 高吞吐文件服务（如 Nginx 静态、Kafka 日志、Netty 大文件）都用 **sendfile** 类机制。

### 6.4 文件描述符

- **fd 表**：进程级，`0/1/2` 为 stdin/stdout/stderr；每 `open` 一个 fd。
- 限制：`ulimit -n`（进程级）、`/proc/sys/fs/file-max`（系统级）。
- **泄漏**：不 close 导致 "too many open files"；排查 `lsof -p <pid> | wc -l`。

---

## 七、I/O 模型与网络（重点）

### 7.1 五种 I/O 模型

```mermaid
flowchart LR
    BIO[阻塞 I/O] --> NIO[非阻塞 I/O] --> MUX[I/O 多路复用]
    MUX --> AIO[异步 I/O]
    SIG[信号驱动 I/O]
```

| 模型 | 阻塞点 | 特点 |
|------|--------|------|
| 阻塞 I/O | recv 阻塞到数据就绪 | 一连接一线程，并发低 |
| 非阻塞 I/O | 轮询返回 EAGAIN | 用户态忙轮询，浪费 CPU |
| **I/O 多路复用** | select/poll/epoll 等"一批 fd" | 一个线程管海量连接，**高并发主流** |
| 信号驱动 | SIGIO 通知就绪 | 少见 |
| 异步 I/O（AIO） | 内核完成才通知 | 真异步，Linux 支持弱，多用多路复用模拟 |

### 7.2 select / poll / epoll 深讲

| | select | poll | **epoll** |
|--|--------|------|-----------|
| 上限 | 1024（FD_SETSIZE） | 无 | 无 |
| 数据结构 | 位图 | 链表 | **红黑树 + 就绪链表** |
| 传递方式 | 每次拷贝全部 fd 进内核 | 同 | 内核只记录注册的 fd |
| 就绪通知 | 返回后需遍历全部 | 同 | **只返回就绪的 fd** |
| 触发模式 | LT | LT | **LT + ET（边缘触发）** |
| 性能 | O(n) | O(n) | **O(就绪数)** |

- **红黑树**：管理注册的所有 fd（增删 O(log n)）。
- **就绪链表**：只放有事件的 fd，`epoll_wait` 直接取走 → 连接数 100 万也不怕遍历。
- **LT（水平触发）**：数据没读完就一直通知（默认，简单安全）。
- **ET（边缘触发）**：只在"有数据→读空"的状态变化时通知一次（效率高，需一次读完，易漏）。
- **惊群**：多个线程 `epoll_wait` 同一 fd，一个事件唤醒全部 → Linux 4.5+ 提供 `EPOLLEXCLUSIVE` 缓解。

> 这就是 **Netty / Redis / Nginx 单线程（或少量线程）扛十万并发的底层原理**。Java 的 `Selector`（NIO）在 Linux 就是 epoll 封装；Netty 的 `EventLoop` 即"一个线程跑一个 Selector 循环"。

### 7.3 收包路径（一个网络包怎么到应用）

```mermaid
flowchart LR
    NIC[网卡 DMA 收包] --> RING[环形缓冲区 ring buffer]
    RING --> IRQ[硬中断通知 CPU]
    IRQ --> SI[软中断 softirq 处理协议栈]
    SI --> SK[sk_buff 挂到 socket 接收队列]
    SK --> APP[应用 epoll/recv 取走]
```

- **硬中断**：通知有包到，越短越好。
- **软中断 softirq**：内核线程 `ksoftirqd` 处理 TCP/IP 协议栈（`ksoftirqd/N` 高 CPU 常见于网络风暴）。
- **背压**：接收队列满 → 丢包/丢 UDP 包；调优看 `/proc/net/softnet_stat`、`net.core.somaxconn`、`net.ipv4.tcp_max_syn_backlog`。
- **高并发调优**：RPS/RFS 多队列、绑定网卡中断到多核（`irqbalance`/`smp_affinity`）。

---

## 八、进程间通信 IPC

| 方式 | 特点 | 适用 |
|------|------|------|
| **管道/PIPE** | 半双工字节流，父子/同血缘 | 命令串联、父子进程 |
| **FIFO（命名管道）** | 文件系统中，无血缘也可 | 少量通信 |
| **消息队列** | 内核缓冲的"消息"，有类型 | 小消息异步 |
| **信号量** | 计数同步原语 | 资源互斥 |
| **共享内存** | 最快，直接读写同一物理页 | 大数据量高频（Kafka/中间件常用） |
| **信号 Signal** | 异步通知（SIGKILL/SIGTERM/SIGINT） | 控制进程、优雅停机 |
| **Socket** | 跨主机也支持 | 通用网络通信 |

- **共享内存** 最常用作高性能 IPC（配合信号量/锁做同步）；Java 侧见 **mmap + FileChannel**（Kafka 日志分段、RocketMQ commitlog 都映射 mmap）。
- 对比表关键：**速度上 共享内存 > 管道/消息队列 > Socket**；跨机器只能用 Socket。

---

## 九、容器与虚拟化

### 9.1 namespace + cgroup = 容器

```mermaid
flowchart TD
    C[容器] --> N[namespace: 隔离视角]
    C --> G[cgroup: 限制资源]
    N --> NS1[MNT 挂载 / PID 进程 / NET 网络 / USER 用户 / UTS 主机名 / IPC]
    G --> CG1[CPU 配额 / 内存限制 / IO 带宽 / 进程数 pids]
```

- **namespace**：让容器"看起来"是独立机器（独立的 PID 1、网络栈、文件系统、主机名）。
- **cgroup**：真正限制"能用多少"（CPU 时间片、内存上限、IOPS）。
- **overlayfs 镜像分层**：只读层共享 + 读写层增量 → 镜像复用、启动快。

### 9.2 容器对 Java 应用的影响（后端高频坑）

| 问题 | 说明 | 对策 |
|------|------|------|
| 堆不认识 cgroup | 老 JVM 按宿主机内存算 `-Xmx` | JDK 10+ `UseContainerSupport`；显式 `-Xmx` |
| CPU 限制与 CFS | 容器限制 n 核，JVM 默认按核数调线程池/GC 并行 | 显式设 `-XX:ActiveProcessorCount` |
| 线程数爆 | `-Xss` × 线程数 超内存 | 用线程池/虚拟线程，限制最大线程数 |
| swap 导致 GC 异常 | 容器开 swap，堆换出后 GC 卡顿 | 容器内关 swap，堆预留 headroom |

> 云原生容器资源与调度详见「[云原生/容器与Docker](../云原生/容器与Docker.md)」「[云原生/Kubernetes核心](../云原生/Kubernetes核心.md)」。

---

## 十、系统启动与运维底子

- **启动流程**：BIOS/UEFI → Bootloader（GRUB）→ 内核 → `init`（PID 1）→ systemd 并行拉起服务。
- **systemd**：现代 init，`systemctl`、`journalctl` 日志、target 依赖管理。
- **/proc 与 /sys**：内核状态的门面——`/proc/<pid>/stack`、`/proc/meminfo`、`/sys/fs/cgroup/` 等都是排查依据。
- **守护进程**：`fork` + `setsid` 脱离终端，`nohup`/`systemd service` 管理。

---

## 十一、Linux CFS 调度器深入

### 11.1 虚拟运行时间（vruntime）

```
CFS 核心：每个任务维护 vruntime（虚拟运行时间）
  nice 0 基准：实际运行 1ms → vruntime + 1ms
  nice -20（高优先级）：实际运行 1ms → vruntime + 0.3ms
  nice +19（低优先级）：实际运行 1ms → vruntime + 23.7ms

调度决策：每次选 vruntime 最小的任务运行
  → 高优先级任务 vruntime 增长慢 → 获得更多 CPU
  → 低优先级任务 vruntime 增长快 → 获得更少 CPU
```

### 11.2 组调度与 CFS 带宽控制

```
CFS 组调度：
  把进程组（cgroup）当调度实体
  组内公平分配，组间按权重分配
  → 容器 CPU 限制的基础

CFS 带宽控制（容器限流）：
  cpu.cfs_quota_us：周期内可用时间（微秒）
  cpu.cfs_period_us：周期长度（默认 100ms）
  
  示例：2 核容器
    quota=200000 period=100000 → 每 100ms 可用 200ms CPU
    → 限制为 2 核
```

## 十二、虚拟内存深入

### 12.1 多级页表

```
x86-64 四级页表：
  PGD → PUD → PMD → PTE → 物理页

好处：
  单级页表需要 2^47 × 8B = 1TB
  四级页表只在需要时分配下级页表
  → 节省内存（稀疏地址空间）
```

### 12.2 TLB 与大页

```
TLB（Translation Lookaside Buffer）：
  页表缓存，命中 1 周期，未命中需逐级查页表（几十周期）
  进程切换 → TLB 刷新（昂贵）

HugePage（大页）：
  2MB 或 1GB 页
  减少页表项数量
  提升 TLB 命中率
  MySQL/ES/Redis 大内存场景常开启

配置：
  echo 1024 > /proc/sys/vm/nr_hugepages  # 预分配 1024 个 2MB 大页
```

## 十三、文件系统深入

### 13.1 ext4 vs xfs 对比

| 维度 | ext4 | xfs |
|------|------|-----|
| 最大文件 | 16TB | 8EB |
| 最大卷 | 1EB | 8EB |
| 日志 | 有序日志 | 外部日志 |
| 碎片 | 有（长期使用） | 无（连续分配） |
| 适合 | 通用 | 大文件/高并发写 |
| 配额 | 支持 | 支持 |
| 在线扩容 | 是 | 是（不能缩） |

### 13.2 I/O 调度器

| 调度器 | 说明 | 适用 |
|--------|------|------|
| mq-deadline | 多队列截止期调度 | 通用（SSD/HDD） |
| bfq | 预算公平队列 | 桌面/交互 |
| kyber | 轻量级双队列 | 快速设备（NVMe SSD） |
| none | 无调度（直接下发） | 超快设备/虚拟化 |

```bash
# 查看当前调度器
cat /sys/block/sda/queue/scheduler

# 设置调度器
echo kyber > /sys/block/nvme0n1/queue/scheduler
```

## 十四、Linux Namespaces 与 Cgroups 深入

### 14.1 Namespaces 类型

| Namespace | 隔离内容 | 说明 |
|-----------|----------|------|
| PID | 进程 ID | 容器内 PID 1 |
| MNT | 挂载点 | 独立文件系统视图 |
| NET | 网络 | 独立网络栈（IP/端口） |
| UTS | 主机名 | 独立 hostname |
| IPC | 进程间通信 | 独立信号量/消息队列 |
| USER | 用户 | 独立 UID/GID |
| Cgroup | Cgroup 根 | 隐藏宿主 cgroup 层级 |

### 14.2 Cgroups v2

```
Cgroups v2（统一层级）：
  单一层级树（v1 有多个）
  统一资源控制（CPU/内存/IO/PID）
  压力阻滞信息（PSI）：监控资源压力

资源控制：
  cpu.max：CPU 带宽限制（quota/period）
  memory.max：内存上限
  memory.swap.max：swap 上限
  io.max：I/O 带宽限制
```

## 十五、容器内部原理

### 15.1 容器启动流程

```mermaid
flowchart LR
    A[containerd 收到创建请求] --> B[拉取镜像]
    B --> C[创建 rootfs（overlayfs）]
    C --> D[创建 namespaces]
    D --> E[配置 cgroups]
    E --> F[挂载文件系统]
    F --> G[启动容器进程 PID 1]
```

### 15.2 OverlayFS

```
OverlayFS = 联合文件系统
  upperdir（读写层）：容器运行时写入
  lowerdir（只读层）：镜像层（可多个）
  merged（合并视图）：容器看到的统一视图

写时复制（COW）：
  修改 lowerdir 文件 → 复制到 upperdir → 修改 upperdir 副本
  → 镜像层共享，容器启动快
```

## 十六、Linux 网络栈

### 16.1 Netfilter/iptables

```
Netfilter = Linux 内核网络过滤框架
  PREROUTING：DNAT（目标地址转换）
  INPUT：入站流量过滤
  FORWARD：转发流量过滤
  OUTPUT：出站流量过滤
  POSTROUTING：SNAT（源地址转换）

iptables = Netfilter 的用户态工具
  -A：追加规则
  -t：指定表（filter/nat/mangle）
  -s/-d：源/目标地址
  -p：协议（tcp/udp/icmp）
  --dport：目标端口
```

### 16.2 收包路径深入

```mermaid
flowchart LR
    A[网卡 DMA 收包] --> B[环形缓冲区 ring buffer]
    B --> C[硬中断通知 CPU]
    C --> D[softirq 处理协议栈]
    D --> E[sk_buff 挂到 socket 队列]
    E --> F[应用 epoll/recv 取走]
```

### 16.3 网络调优参数

| 参数 | 默认 | 建议 | 说明 |
|------|------|------|------|
| `net.core.somaxconn` | 128 | 1024~4096 | 全连接队列上限 |
| `net.ipv4.tcp_max_syn_backlog` | 128 | 1024~4096 | 半连接队列上限 |
| `net.ipv4.tcp_tw_reuse` | 0 | 1 | TIME_WAIT 复用 |
| `net.ipv4.ip_local_port_range` | 32768~60999 | 1024~65535 | 可用端口范围 |
| `net.core.netdev_max_backlog` | 1000 | 10000+ | 网卡接收队列 |

## 十七、I/O 调度器深入

### 17.1 I/O 调度器对比

| 调度器 | 算法 | 适用 | 特点 |
|--------|------|------|------|
| mq-deadline | 截止期优先 | HDD/SSD 通用 | 保证延迟 |
| bfq | 预算公平 | 桌面/交互 | 公平性好 |
| kyber | 双队列 | NVMe SSD | 轻量低延迟 |
| none | 无调度 | 超快设备 | 直接下发 |

### 17.2 I/O 调优

```bash
# 查看 I/O 统计
iostat -xz 1

# 关键指标
# %util：磁盘使用率（>80% 瓶颈）
# await：平均 I/O 等待时间（>10ms 异常）
# r/s w/s：读写 IOPS
```

## 十八、Linux 内核 cgroup 资源限制

### cgroup v1 vs v2

```text
cgroup v1（经典版）：
  每个资源独立层级：
    /sys/fs/cgroup/cpu/docker/<container-id>
    /sys/fs/cgroup/memory/docker/<container-id>
    /sys/fs/cgroup/blkio/docker/<container-id>

  优点：每个控制器独立管理
  缺点：层级复杂、资源分配不一致

cgroup v2（统一层级）：
  单一层级：
    /sys/fs/cgroup/system.slice/docker-<id>.scope/

  统一资源分配（linux/resource_control）
  更好的内存管理（memory.high, memory.low）
  PSI（Pressure Stall Information）监控

现代容器运行时默认使用 cgroup v2
```

### cgroup 资源限制实现

```bash
# CPU 限制
# cgroup v1
echo 200000 > /sys/fs/cgroup/cpu/docker/<id>/cpu.cfs_quota_us  # 200ms/100ms
echo 100000 > /sys/fs/cgroup/cpu/docker/<id>/cpu.cfs_period_us  # 100ms 周期

# cgroup v2
echo "200000 100000" > /sys/fs/cgroup/system.slice/docker-<id>.scope/cpu.max

# 内存限制
# cgroup v1
echo 1073741824 > /sys/fs/cgroup/memory/docker/<id>/memory.limit_in_bytes  # 1GB

# cgroup v2
echo 1073741824 > /sys/fs/cgroup/system.slice/docker-<id>.scope/memory.max

# IO 限制（块设备）
# cgroup v2
echo "8:0 rbps=104857600 wbps=52428800" > /sys/fs/cgroup/system.slice/docker-<id>.scope/io.max
# 读 100MB/s，写 50MB/s

# 检查 cgroup 限制
cat /proc/self/cgroup
cat /sys/fs/cgroup/system.slice/docker-<id>.scope/memory.max
cat /sys/fs/cgroup/system.slice/docker-<id>.scope/cpu.max
```

### PSI（Pressure Stall Information）

```bash
# 查看资源压力
cat /proc/pressure/cpu
# some avg10=0.50 avg60=0.30 avg300=0.10 total=123456
# full avg10=0.00 avg60=0.00 avg300=0.00 total=0

cat /proc/pressure/memory
# some avg10=1.20 avg60=0.80 avg300=0.50 total=789012
# full avg10=0.60 avg60=0.40 avg300=0.20 total=345678

cat /proc/pressure/io
# some avg10=2.00 avg60=1.50 avg300=1.00 total=234567
# full avg10=0.80 avg60=0.60 avg300=0.30 total=123456

# 含义：
# some：至少一个任务因资源不足而等待
# full：所有任务都因资源不足而等待
# avg10/60/300：10s/60s/300s 平均压力百分比
# total：总等待时间（微秒）

# 告警阈值建议：
# CPU：some > 50% 持续 10s
# Memory：full > 30% 持续 10s
# IO：full > 20% 持续 10s
```

## 十九、Linux 网络优化

### TCP 优化参数

```bash
# /etc/sysctl.conf
# TCP 连接优化
net.core.somaxconn = 65535
net.ipv4.tcp_max_syn_backlog = 65535
net.ipv4.tcp_fin_timeout = 30
net.ipv4.tcp_tw_reuse = 1

# TCP 缓冲区
net.core.rmem_max = 16777216
net.core.wmem_max = 16777216
net.ipv4.tcp_rmem = 4096 87380 16777216
net.ipv4.tcp_wmem = 4096 65536 16777216

# TCP keepalive
net.ipv4.tcp_keepalive_time = 600
net.ipv4.tcp_keepalive_probes = 3
net.ipv4.tcp_keepalive_intvl = 15

# TCP 性能
net.ipv4.tcp_slow_start_after_idle = 0
net.core.netdev_max_backlog = 65536

# 端口范围
net.ipv4.ip_local_port_range = 1024 65535

# 启用 BBR 拥塞控制
net.core.default_qdisc = fq
net.ipv4.tcp_congestion_control = bbr
```

### 网络监控与诊断

```bash
# 实时网络流量监控
iftop -i eth0
nload eth0
nethogs  # 按进程查看

# 连接状态统计
ss -s
ss -ant | awk '{print $1}' | sort | uniq -c | sort -rn
# ESTAB  1523
# TIME-WAIT  3456
# LISTEN  128

# 包丢包率
ethtool -S eth0 | grep -i drop
ip -s link show eth0

# TCP 重传统计
netstat -s | grep retrans
cat /proc/net/netstat | awk '{print $8}'  # RetransSegs

# 延迟监控
mtr -r -c 100 <target-ip>
ping -c 100 <target-ip>
traceroute <target-ip>

# 抓包分析
tcpdump -i eth0 -w capture.pcap host 10.0.0.1
tcpdump -i eth0 -nn port 80
tcpdump -i eth0 'tcp[tcpflags] & (tcp-syn) != 0'
```

## 二十、Linux 内存管理深入

### 虚拟内存区域（VMA）

```c
// mm_types.h
struct vm_area_struct {
    unsigned long vm_start;     // 起始地址
    unsigned long vm_end;       // 结束地址
    pgprot_t vm_page_prot;      // 页保护
    unsigned long vm_flags;     // 标志（读/写/执行）

    struct rb_node rb;          // 红黑树节点
    struct list_head vm_next;   // 链表

    const struct vm_operations_struct *vm_ops;  // 操作函数
    unsigned long vm_pgoff;     // 文件偏移（页为单位）
    struct file *vm_file;       // 关联文件（mmap）
};

// VMA 操作
struct vm_operations_struct {
    void (*open)(struct vm_area_struct *vma);
    void (*close)(struct vm_area_struct *vma);
    vm_fault_t (*fault)(struct vm_area_struct *vma, struct vm_fault *vmf);
};
```

### 缺页异常处理

```text
缺页异常（Page Fault）流程：
  1. CPU 访问虚拟地址 → 页表项无效（Present=0）
  2. 触发缺页异常 → 陷入内核态
  3. 内核处理缺页异常：
     ├── 匿名页（堆/栈/匿名 mmap）
     │   ├── 分配物理页
     │   ├── 如果是交换页 → 从 swap 读回
     │   └── 更新页表项
     ├── 文件映射页（文件 mmap）
     │   ├── 从磁盘读取文件数据
     │   ├── 分配物理页
     │   └── 更新页表项
     └── 写时复制（COW）
         ├── 分配新物理页
         ├── 复制原页内容
         ├── 更新页表项为新页
         └── 进程可以独立修改

缺页类型：
  主缺页（Major Fault）：需要磁盘 I/O（慢，ms 级）
  次缺页（Minor Fault）：不需要磁盘 I/O（快，μs 级）
```

### hugepage 与大页内存

```bash
# 查看 hugepage 配置
cat /proc/meminfo | grep Huge
# HugePages_Total: 1024
# HugePages_Free: 512
# Hugepagesize: 2048 kB

# 预分配 hugepage
echo 1024 > /proc/sys/vm/nr_hugepages

# 应用程序使用 hugepage
# 方式 1：显式 mmap
mmap(NULL, 2*1024*1024, PROT_READ|PROT_WRITE,
     MAP_PRIVATE|MAP_ANONYMOUS|MAP_HUGETLB, -1, 0)

# 方式 2：透明大页（THP）
echo always > /sys/kernel/mm/transparent_hugepage/enabled

# 检查 THP 使用
cat /proc/meminfo | grep AnonHugePages

# 性能对比：
# 普通页（4KB）：TLB 命中率低
# 大页（2MB）：TLB 命中率高，减少 TLB miss
# 适用场景：数据库、HPC、大内存应用
```

## 二十一、Linux 文件系统

### ext4 vs xfs

```text
ext4：
  ├── 日志模式：data=ordered（默认）/ data=writeback / data=journal
  ├── 最大文件：16TB
  ├── 最大卷：1EB
  ├── 特性：extent、delayed allocation、inline data
  └── 适用：通用场景、小文件多、Boot 分区

xfs：
  ├── 日志模式：元数据日志
  ├── 最大文件：8EB
  ├── 最大卷：8EB
  ├── 特性：extent-based allocation、delayed allocation、Online defrag
  └── 适用：大文件、高并发 I/O、数据库

性能对比：
  ├── 小文件：ext4 更好（元数据操作更快）
  ├── 大文件：xfs 更好（extent 分配更高效）
  ├── 并发 I/O：xfs 更好（细粒度锁）
  └── 删除大量小文件：ext4 更好（xfs 延迟删除）
```

### 文件系统性能调优

```bash
# 挂载选项优化
# ext4
mount -o noatime,nodiratime,data=ordered,barrier=0 /dev/sda1 /data

# xfs
mount -o noatime,nodiratime,logbufs=8,logbsize=256k /dev/sda1 /data

# I/O 调度器
cat /sys/block/sda/queue/scheduler
# [mq-deadline] kyber bfq none

# SSD 推荐：
echo none > /sys/block/sda/queue/scheduler  # 或 kyber
echo 256 > /sys/block/sda/queue/nr_requests
echo 1024 > /sys/block/sda/queue/read_ahead_kb

# HDD 推荐：
echo mq-deadline > /sys/block/sda/queue/scheduler
echo 256 > /sys/block/sda/queue/nr_requests
echo 2048 > /sys/block/sda/queue/read_ahead_kb

# 文件系统检查
e2fsck -f /dev/sda1       # ext4
xfs_repair /dev/sda1      # xfs

# 碎片整理
e4defrag /data            # ext4
xfs_fsr /data             # xfs
```

## 二十二、Linux 进程调度器

### CFS（完全公平调度器）

```text
CFS 原理：
  使用虚拟运行时间（vruntime）衡量进程运行时间
  vruntime 越小 → 越优先获得 CPU
  vruntime 增长速度与 nice 值成反比

数据结构：
  红黑树（rbtree）：按 vruntime 排序
  左节点：vruntime 最小 → 最优先调度
  选取最左节点运行

nice 值对 vruntime 的影响：
  nice 0（默认）：vruntime 每 ms +1
  nice -20（最高）：vruntime 每 ms +0.25
  nice 19（最低）：vruntime 每 ms +8.0

调度周期：
  目标延迟（target latency）：默认 6ms
  如果进程数 > 目标延迟内的最大进程数：
    每个进程至少运行 min_granularity（默认 0.75ms）

抢占时机：
  1. 时钟中断：检查 vruntime，当前进程 vs 最左节点
  2. 进程唤醒：如果 vruntime 小于当前进程，立即抢占
  3. 进程创建：新进程 vruntime = 当前最小 vruntime
```

### 实时调度策略

```text
SCHED_FIFO（先进先出）：
  优先级 1-99（数字越大优先级越高）
  同优先级：按 FIFO 顺序
  不会主动让出 CPU
  直到：被更高优先级抢占、主动 sleep、被 kill

SCHED_RR（时间片轮转）：
  优先级 1-99（数字越大优先级越高）
  同优先级：时间片轮转（默认 100ms）
  时间片用完 → 放到队尾
  直到：被更高优先级抢占、主动 sleep、被 kill

SCHED_DEADLINE（截止时间调度）：
  三参数：runtime、deadline、period
  保证：在每个 period 内，至少运行 runtime 时间
  最高优先级：高于所有 FIFO/RR 进程
  适用：硬实时应用

设置实时调度：
  chrt -f 50 ./myapp      # FIFO，优先级 50
  chrt -r 50 ./myapp      # RR，优先级 50
  chrt -d --sched-runtime 5000000 --sched-deadline 10000000 --sched-period 10000000 ./myapp
```

## 进程调度器对比

```mermaid
flowchart LR
    subgraph CFS调度器
        CFS[完全公平调度] --> VRUNTIME[虚拟运行时间]
        VRUNTIME --> RB_TREE[红黑树]
    end
    subgraph 实时调度器
        RT[实时调度] --> FIFO[FIFO]
        RT --> RR[RR轮转]
    end
```

### 调度器参数

| 参数 | 说明 | 调整建议 |
|------|------|----------|
| sched_latency_ns | 调度延迟 | 默认6ms |
| sched_min_granularity_ns | 最小时间片 | 默认0.75ms |
| sched_wakeup_granularity_ns | 唤醒粒度 | 默认1ms |
| sched_migration_cost_ns | 迁移成本 | 默认0.5ms |

## 内存管理机制

```bash
# 查看内存使用
free -h
cat /proc/meminfo

# 查看OOM评分
cat /proc/<pid>/oom_score
cat /proc/<pid>/oom_score_adj

# 调整OOM优先级
echo -1000 > /proc/<pid>/oom_score_adj  # 不会被OOM Kill
```

### 内存回收机制

| 机制 | 触发条件 | 说明 |
|------|----------|------|
| kswapd | 内存不足 | 后台异步回收 |
| direct reclaim | 紧急回收 | 同步阻塞回收 |
| OOM Killer | 内存耗尽 | 杀死高内存进程 |
| CGroup限制 | 容器限制 | 按cgroup限制 |

### 虚拟内存管理

```mermaid
flowchart TB
    VIRTUAL[虚拟内存] --> MMU[MMU地址转换]
    MMU --> TLB[TLB快表]
    MMU --> PAGE[页表]
    PAGE --> DISK[磁盘交换区]
    TLB --> CACHE[Cache]
```

## 文件系统优化

```bash
# ext4 文件系统优化
tune2fs -o journal_data_writeback /dev/sda1
mount -o noatime,nodiratime /dev/sda1 /data

# 查看文件系统状态
df -hT
iostat -x 1
```

### 文件系统选择

| 文件系统 | 特点 | 适用场景 |
|----------|------|----------|
| ext4 | 稳定可靠 | 通用场景 |
| XFS | 高性能大文件 | 数据库、大文件 |
| Btrfs | 快照、压缩 | 虚拟化、备份 |
| ZFS | 企业级 | 存储服务器 |

## I/O 模型对比

```mermaid
flowchart TB
    subgraph 阻塞I/O
        BLOCK[阻塞] --> WAIT[等待]
        WAIT --> READ[读取]
    end
    subgraph 非阻塞I/O
        NONBLOCK[非阻塞] --> POLL[轮询]
        POLL --> READ2[读取]
    end
    subgraph I/O多路复用
        SELECT[select/poll] --> MULTIPLE[多连接]
        MULTIPLE --> READ3[读取]
    end
    subgraph 异步I/O
        AIO[异步] --> CALLBACK[回调]
        CALLBACK --> READ4[读取]
    end
```

### I/O 模型性能对比

| 模型 | 并发能力 | 实现复杂度 | 性能 |
|------|----------|------------|------|
| 阻塞I/O | 低 | 简单 | 低 |
| 非阻塞I/O | 中 | 中 | 中 |
| I/O多路复用 | 高 | 中 | 高 |
| 异步I/O | 极高 | 复杂 | 极高 |

## 容器原理深入

```mermaid
flowchart TB
    subgraph 容器技术
        NAMESPACE[命名空间] --> CGROUP[CGroup资源限制]
        NAMESPACE --> UNIONFS[联合文件系统]
        UNIONFS --> READONLY[只读层]
        UNIONFS --> WRITABLE[可写层]
    end
    subgraph 容器运行时
        CONTAINERD[containerd] --> RUNC[runc]
        RUNC --> LXC[LXC/LXCFS]
    end
```

### 容器 vs 虚拟机

| 维度 | 容器 | 虚拟机 |
|------|------|--------|
| 启动速度 | 毫秒级 | 分钟级 |
| 资源占用 | MB级 | GB级 |
| 隔离级别 | 进程级 | 硬件级 |
| 性能损失 | 接近原生 | 10-20% |
| 镜像大小 | 小 | 大 |

## CFS 调度器深度解析

### CFS 工作原理

```text
CFS（Completely Fair Scheduler）完全公平调度器：
  - 使用虚拟运行时间（vruntime）衡量进程运行时间
  - vruntime 越小，优先级越高
  - 使用红黑树维护 vruntime，O(log n) 复杂度
  - nice 值影响权重，权重影响 vruntime 增长速度

vruntime 计算：
  delta_exec = 实际运行时间
  weight = nice 0 对应权重 1024
  vruntime += delta_exec * (NICE_0_WEIGHT / weight)
  
nice 值与权重：
  nice -20: weight = 88761
  nice 0:   weight = 1024
  nice 19:  weight = 15
```

### CFS 调度参数

| 参数 | 说明 | 生产建议 |
|------|------|----------|
| sched_latency_ns | 调度周期 | 6ms |
| sched_min_granularity_ns | 最小时间片 | 0.75ms |
| sched_wakeup_granularity_ns | 唤醒粒度 | 1ms |
| sched_migration_cost_ns | 迁移代价 | 0.5ms |
| sched_nr_migrate | 最大迁移进程数 | 32 |

---

## 四级页表与 TLB

### 四级页表结构

```text
四级页表结构（x86-64）：
  PGD（Page Global Directory）→ P4D → PUD → PMD → PTE
  
  虚拟地址 = PGD索引 + P4D索引 + PUD索引 + PMD索引 + PTE索引 + 页内偏移
  
  每级页表占用 4KB（一个页面）
  每个页表项 8 字节
  每个页表可索引 512 个条目（4KB/8B）
  
  页表遍历开销：
    无 TLB：5 次内存访问
    有 TLB：1 次内存访问
```

### TLB（Translation Lookaside Buffer）

```text
TLB 原理：
  - 缓存最近使用的页表项
  - 减少页表遍历次数
  - TLB 命中：1 个时钟周期
  - TLB 未命中：5+ 个时钟周期
  
TLB 优化：
  1. 大页（HugePage）：减少页表项数量
  2. TLB 预取：提前加载页表项
  3. 进程亲和性：减少 TLB 刷新
  4. ASID：地址空间标识，避免切换刷新
```

### 大页（HugePage）

```bash
# 查看大页配置
cat /proc/meminfo | grep Huge

# 配置大页
echo 1024 > /proc/sys/vm/nr_hugepages  # 分配 1024 个 2MB 大页

# 应用使用大页
# JVM：-XX:+UseLargePages
# Redis：hugepage-enabled yes
```

---

## I/O 调度器

### I/O 调度器对比

| 调度器 | 算法 | 适用场景 |
|--------|------|----------|
| noop | 无调度 | SSD/虚拟化 |
| deadline | 截止时间 | 数据库 |
| cfq | 完全公平 | 通用 |
| mq-deadline | 多队列截止 | NVMe SSD |
| bfq | 预算公平队列 | 桌面系统 |

### I/O 调度参数

```bash
# 查看当前调度器
cat /sys/block/sda/queue/scheduler

# 设置调度器
echo mq-deadline > /sys/block/sda/queue/scheduler

# I/O 队列深度
cat /sys/block/sda/queue/nr_requests

# 预读设置
blockdev --getra /dev/sda
blockdev --setra 4096 /dev/sda  # 设置预读为 4KB
```

---

## 网络协议栈

### Linux 网络收包路径

```text
Linux 网络收包路径：
  1. 网卡 DMA：数据写入 ring buffer
  2. 硬中断：通知 CPU 有数据到达
  3. 软中断（softirq）：处理协议栈
  4. socket 队列：数据存入 socket 缓冲区
  5. 应用层：recv/recvfrom 读取数据

关键优化：
  1. RPS（Receive Packet Steering）：软件层面多队列
  2. RFS（Receive Flow Steering）：数据导向 CPU
  3. XPS（Transmit Packet Steering）：发送多队列
  4. GRO（Generic Receive Offload）：合并小包
```

### 网络调优参数

```bash
# TCP 连接优化
net.ipv4.tcp_max_syn_backlog = 65535
net.core.somaxconn = 65535
net.ipv4.tcp_fin_timeout = 30
net.ipv4.tcp_keepalive_time = 600

# 网络缓冲区
net.core.rmem_max = 16777216
net.core.wmem_max = 16777216
net.ipv4.tcp_rmem = 4096 87380 16777216
net.ipv4.tcp_wmem = 4096 65536 16777216

# 网卡队列
ethtool -L eth0 combined 8  # 设置多队列
ethtool -g eth0             # 查看环形缓冲区
```

---

## 缓存一致性

### 多核缓存一致性协议

```text
MESI 协议：
  M（Modified）：已修改，独占
  E（Exclusive）：未修改，独占
  S（Shared）：未修改，共享
  I（Invalid）：无效

缓存一致性问题：
  1. 伪共享（False Sharing）：同一缓存行不同变量
  2. 缓存颠簸（Cache Thrashing）：频繁失效
  3. 缓存污染（Cache Pollution）：无用数据占据缓存

解决方法：
  1. 缓存行填充（Padding）：避免伪共享
  2. 亲和性绑定（Affinity）：减少迁移
  3. 缓存预取（Prefetch）：提前加载
```

### 伪共享问题

```java
// 伪共享问题示例
class Counter {
    volatile long count1;  // 缓存行 1
    volatile long count2;  // 缓存行 1（伪共享！）
}

// 解决方案：缓存行填充
class Counter {
    volatile long count1;
    long p1, p2, p3, p4, p5, p6, p7;  // 填充到不同缓存行
    volatile long count2;
}
```

---

## cgroup v2

### cgroup v2 架构

```text
cgroup v2 架构：
  统一层级：所有资源控制器在一个层级
  统一接口：/sys/fs/cgroup/ 根目录
  更好的隔离：每个进程组独立控制

资源控制器：
  cpu：CPU 时间分配
  memory：内存限制
  io：I/O 带宽限制
  pids：进程数限制
  cpuset：CPU/内存绑定
```

### cgroup v2 配置

```bash
# 查看 cgroup 版本
stat -fc %T /sys/fs/cgroup
# cgroup2fs 表示 v2
# tmpfs 表示 v1

# 创建 cgroup
mkdir /sys/fs/cgroup/mygroup

# 设置 CPU 限制
echo "200000 100000" > /sys/fs/cgroup/mygroup/cpu.max  # 200ms 周期内使用 100ms

# 设置内存限制
echo 1073741824 > /sys/fs/cgroup/mygroup/memory.max  # 1GB

# 添加进程
echo $PID > /sys/fs/cgroup/mygroup/cgroup.procs
```

### cgroup v2 vs v1

| 维度 | cgroup v1 | cgroup v2 |
|------|-----------|-----------|
| 层级 | 多层级 | 单一层级 |
| 接口 | 分散 | 统一 |
| 隔离 | 弱 | 强 |
| 兼容性 | 好 | 需要适配 |
| 适用 | 旧系统 | 新系统 |

---

## 操作系统深度机制解析

### 三级页表结构详解

```
三级页表结构（x86-64）：
  PGD（Page Global Directory）→ P4D → PUD → PMD → PTE → 物理页

  虚拟地址格式（48位）：
    ┌───────────┬───────────┬───────────┬───────────┬───────────┬───────────┐
    │ PGD索引   │ P4D索引   │ PUD索引   │ PMD索引   │ PTE索引   │ 页内偏移  │
    │ 9 bits   │ 9 bits   │ 9 bits   │ 9 bits   │ 9 bits   │ 12 bits  │
    └───────────┴───────────┴───────────┴───────────┴───────────┴───────────┘

  页表遍历开销：
    无 TLB：5 次内存访问（每次访问一级页表）
    有 TLB：1 次内存访问（TLB 命中）

  TLB 条目结构：
    VPN（虚拟页号）→ PFN（物理页号）+ 权限位
    TLB 容量：通常 64-1024 条目
    TLB 命中率：>99%（良好工作集）

  大页对 TLB 的影响：
    4KB 页：1GB 内存需要 256K 个 TLB 条目
    2MB 页：1GB 内存只需要 512 个 TLB 条目
    → 大页减少 TLB 条目需求，提升命中率
```

### TLB Miss 处理流程

```mermaid
flowchart TD
    A[CPU 访问虚拟地址] --> B{TLB 命中?}
    B -->|是| C[直接访问物理地址\n1 周期]
    B -->|否| D[触发 TLB Miss]
    D --> E[CPU 查页表 Walker]
    E --> F{页表项有效?}
    F -->|是| G[更新 TLB\n访问物理地址]
    F -->|否| H[触发缺页异常]
    H --> I{缺页类型}
    I -->|主缺页| J[从磁盘调入\nms 级延迟]
    I -->|次缺页| K[分配物理页\nμs 级延迟]
    I -->|COW| L[复制物理页\nμs 级延迟]
    J --> M[更新页表项]
    K --> M
    L --> M
    M --> N[重新执行访问指令]
```

### I/O 调度器深度对比

```
I/O 调度器选择决策：

┌─────────────────┬─────────────────┬─────────────────┬─────────────────┐
│ 设备类型        │ 推荐调度器      │ 原因            │ 关键参数        │
├─────────────────┼─────────────────┼─────────────────┼─────────────────┤
│ NVMe SSD        │ none/kyber      │ 低延迟          │ nr_requests=256 │
│ SATA SSD        │ mq-deadline     │ 平衡性能        │ read_ahead_kb=4 │
│ HDD             │ mq-deadline     │ 减少寻道        │ read_ahead_kb=2 │
│ 虚拟化设备      │ none            │ 直接下发        │ 无              │
│ 数据库          │ deadline        │ 保证延迟        │ iosched=5ms     │
└─────────────────┴─────────────────┴─────────────────┴─────────────────┘
```

```bash
# I/O 调度器性能测试
fio --name=test --ioengine=libaio --direct=1 --bs=4k \
    --rw=randread --size=1G --numjobs=4 --runtime=60

# 不同调度器性能对比
# mq-deadline: IOPS=100K, 延迟=0.1ms
# kyber: IOPS=120K, 延迟=0.08ms
# none: IOPS=130K, 延迟=0.07ms
```

### Linux 网络栈深度解析

```mermaid
flowchart TB
    subgraph 用户态
        APP[应用程序]
        SOCKET[Socket API]
    end
    
    subgraph 内核态
        SYSCALL[系统调用]
        TCP[TCP/UDP 协议栈]
        IP[IP 层]
        NETFILTER[Netfilter/iptables]
        DRIVER[网卡驱动]
    end
    
    subgraph 硬件
        NIC[网卡]
        DMA[DMA 引擎]
        RING[环形缓冲区]
    end
    
    APP --> SOCKET
    SOCKET --> SYSCALL
    SYSCALL --> TCP
    TCP --> IP
    IP --> NETFILTER
    NETFILTER --> DRIVER
    DRIVER --> DMA
    DMA --> RING
    RING --> NIC
```

### 网络收包路径详解

```text
Linux 网络收包完整路径：

1. 网卡 DMA 收包
   ├── 数据从网卡 DMA 到 ring buffer
   ├── ring buffer 大小：256-4096 描述符
   └── 每个描述符指向一个 skb（socket buffer）

2. 硬中断通知
   ├── 网卡触发 IRQ（硬中断）
   ├── 中断处理程序：禁用网卡中断，调度 softirq
   └── NAPI：批量处理减少中断频率

3. 软中断处理（softirq）
   ├── ksoftirqd 内核线程处理
   ├── 协议栈处理：Ethernet → IP → TCP/UDP
   ├── 校验和验证
   └── 数据放入 socket 接收队列

4. 应用层读取
   ├── epoll/select/poll 监听 fd
   ├── recv/recvfrom 从 socket 队列读取
   └── 数据拷贝到用户态缓冲区

关键性能指标：
  ├── ring buffer 溢出：ethtool -S eth0 | grep rx_missed
  ├── softirq 延迟：/proc/net/softnet_stat
  ├── socket 队列溢出：netstat -s | grep overflow
  └── 应用层读取延迟：strace -e recvfrom
```

### 网络调优参数详解

```bash
# TCP 连接建立优化
net.ipv4.tcp_max_syn_backlog = 65535      # 半连接队列
net.core.somaxconn = 65535                # 全连接队列
net.ipv4.tcp_syncookies = 1               # SYN Cookie 防 SYN Flood

# TCP 传输优化
net.ipv4.tcp_window_scaling = 1           # 窗口缩放
net.ipv4.tcp_timestamps = 1               # 时间戳（RTT 计算）
net.ipv4.tcp_sack = 1                     # 选择性确认

# TCP 内存优化
net.ipv4.tcp_mem = 786432 1048576 1572864 # TCP 内存（页）
net.ipv4.tcp_rmem = 4096 87380 16777216   # TCP 接收缓冲区
net.ipv4.tcp_wmem = 4096 65536 16777216   # TCP 发送缓冲区

# 网卡多队列
ethtool -L eth0 combined 8               # 设置 8 个队列
ethtool -g eth0                          # 查看环形缓冲区大小

# RPS/RFS（软件多队列）
echo "ff" > /sys/class/net/eth0/queues/rx-0/rps_cpus  # 绑定 8 个 CPU
echo 32768 > /sys/class/net/eth0/queues/rx-0/rps_flow_cnt
```

### 缓存一致性协议（MESI）

```text
MESI 协议状态：
  M（Modified）：已修改，独占，内存过期
  E（Exclusive）：未修改，独占，内存有效
  S（Shared）：未修改，共享，内存有效
  I（Invalid）：无效

状态转换：
  初始：I
  读命中：I → S 或 E（取决于其他缓存是否有副本）
  写命中：I/M/S/E → M（通知其他缓存失效）
  读未命中：I → S/E（从内存或其他缓存加载）
  写未命中：I → M（从内存加载后修改）

伪共享问题：
  同一缓存行的不同变量被不同 CPU 访问
  导致缓存行频繁失效，性能下降

解决方案：
  1. 缓存行填充：padding 到 64 字节（缓存行大小）
  2. 亲和性绑定：避免变量在不同 CPU 间迁移
  3. 原子操作：使用 CAS 而非锁
```

```java
// 伪共享问题示例
class Counter {
    volatile long count1;  // 缓存行 1
    volatile long count2;  // 缓存行 1（伪共享！）
}

// 解决方案：缓存行填充
class Counter {
    volatile long count1;
    long p1, p2, p3, p4, p5, p6, p7;  // 填充到不同缓存行
    volatile long count2;
}

// JDK 8+ 使用 @Contended 注解
class Counter {
    @sun.misc.Contended
    volatile long count1;
    
    @sun.misc.Contended
    volatile long count2;
}
```

### 内存屏障与原子操作

```text
内存屏障类型：
  Load Barrier（加载屏障）：确保屏障前的读操作完成后，才能执行屏障后的读操作
  Store Barrier（存储屏障）：确保屏障前的写操作完成后，才能执行屏障后的写操作
  Full Barrier（完全屏障）：同时具有加载和存储屏障

x86 内存模型：
  TSO（Total Store Order）：存储操作不会重排，加载操作可能重排
  读屏障：lfence
  写屏障：sfence
  完全屏障：mfence

ARM 内存模型：
  弱内存模型：所有操作都可能重排
  需要显式内存屏障
  dmb：数据内存屏障
  dsb：数据同步屏障
  isb：指令同步屏障

Java volatile 底层实现：
  写 volatile：StoreStore 屏障 + 写操作 + StoreLoad 屏障
  读 volatile：LoadLoad 屏障 + 读操作 + LoadStore 屏障
  → 保证可见性和有序性
```

---

## CFS 调度器实战（vruntime/红黑树/时间片分配）

### vruntime 计算与调度决策

```
CFS 核心算法：
  vruntime += delta_exec × (NICE_0_WEIGHT / weight)

  示例：
    nice 0 进程运行 10ms → vruntime += 10 × (1024/1024) = 10
    nice -5 进程运行 10ms → vruntime += 10 × (1024/3355) ≈ 3
    nice 5 进程运行 10ms  → vruntime += 10 × (1024/413) ≈ 25

  调度决策：每次时钟中断检查 vruntime
    当前进程 vruntime > 红黑树最左节点 vruntime → 抢占
    新进程 vruntime = 当前最小 vruntime → 不会被立即饿死
```

| nice 值 | 权重 | vruntime 增长速度 | CPU 占比 |
|---------|------|-------------------|----------|
| -20 | 88761 | 最慢 | 最高 |
| -10 | 7141 | 较慢 | 较高 |
| 0 | 1024 | 基准 | 1/N（N=进程数） |
| 10 | 145 | 较快 | 较低 |
| 19 | 15 | 最快 | 最低 |

### 时间片分配与抢占

```
时间片计算：
  target_latency（目标延迟）= 默认 6ms
  min_granularity（最小时间片）= 默认 0.75ms

  如果进程数 ≤ target_latency / min_granularity（约 8 个）：
    每个进程至少运行 min_granularity
  如果进程数 > 8：
    每个进程运行 target_latency / 进程数

抢占触发时机：
  1. 时钟中断（1ms 一次）：检查 vruntime 差值
  2. 进程唤醒：如果唤醒进程 vruntime < 当前进程，立即抢占
  3. 进程创建：新进程 vruntime = 当前最小 vruntime
  4. fork：子进程继承父进程 vruntime
```

```mermaid
flowchart TD
    A[时钟中断] --> B{当前 vruntime > 最左节点?}
    B -->|是| C[设置 TIF_NEED_RESCHED 标志]
    C --> D[返回用户态前检查标志]
    D --> E[schedule 选择最左节点]
    B -->|否| F[继续运行]
```

## 四级页表（PML4/PDP/PD/PT）深入

### 页表结构详解

```
x86-64 四级页表完整结构：
  PML4（Page Map Level 4）= PGD
    └── PDP（Page Directory Pointer）= PUD
        └── PD（Page Directory）= PMD
            └── PT（Page Table）= PTE
                └── 物理页（4KB）

虚拟地址分解（48位）：
  ┌─────────┬─────────┬─────────┬─────────┬─────────┬─────────┐
  │ PML4    │ PDP     │ PD      │ PT      │ 页内偏移│
  │ 9 bits  │ 9 bits  │ 9 bits  │ 9 bits  │ 12 bits │
  └─────────┴─────────┴─────────┴─────────┴─────────┴─────────┘

页表项（PTE）结构：
  位 0：Present（是否存在）
  位 1：R/W（读写权限）
  位 2：U/S（用户/内核）
  位 63：NX（不可执行）
  位 12-51：物理页号（PFN）
```

### 页表遍历流程

```
CPU 访问虚拟地址 0x7FFF12345678：
  1. CR3 寄存器 → PML4 基地址
  2. PML4 索引（位 39-47）→ PDP 表项
  3. PDP 索引（位 30-38）→ PD 表项
  4. PD 索引（位 21-29）→ PT 表项
  5. PT 索引（位 12-20）→ PTE
  6. PTE → 物理页号 + 页内偏移（位 0-11）

开销分析：
  无 TLB：5 次内存访问（每次 ~100ns）= 500ns
  有 TLB：1 次内存访问 = 100ns
  TLB 命中率提升 5x 性能
```

## TLB Miss 处理（Soft Miss/Hard Miss）

### Soft Miss vs Hard Miss

```
Soft TLB Miss（软缺失）：
  TLB 中无映射，但页表在内存中
  处理：CPU 硬件 Page Walker 自动遍历页表
  延迟：~100ns（5 次内存访问）
  频率：高（每次新页首次访问）

Hard TLB Miss（硬缺失/缺页异常）：
  TLB 中无映射，且页表项无效（页不在内存）
  处理：陷入内核，触发 Page Fault
  延迟：~10ms（磁盘 I/O）或 ~1μs（COW/次缺页）
  频率：低（页首次访问或被换出）

Soft Page Fault（次缺页）：
  页在内存但页表未建立映射
  处理：分配物理页 + 更新页表
  延迟：~1μs

Hard Page Fault（主缺页）：
  页不在内存，需从磁盘调入
  处理：磁盘 I/O + 更新页表
  延迟：~10ms
```

| 缺失类型 | 处理方式 | 延迟 | 触发条件 |
|----------|----------|------|----------|
| Soft TLB Miss | Page Walker 遍历页表 | ~100ns | TLB 无映射 |
| Soft Page Fault | 分配物理页 | ~1μs | 页表未映射 |
| Hard Page Fault | 磁盘 I/O | ~10ms | 页被换出 |
| COW Page Fault | 复制物理页 | ~1μs | 写时复制 |

## I/O 调度器（mq-deadline/bfq/kyber 对比）

### 调度器算法对比

```
mq-deadline（多队列截止期）：
  算法：为每个请求设置截止时间，优先处理即将超时的请求
  特点：保证延迟，公平性好
  适用：通用场景（SSD/HDD）
  参数：read_expire=500ms, write_expire=5s

bfq（预算公平队列）：
  算法：按进程分配 I/O 预算，轮转执行
  特点：公平性最好，交互响应快
  适用：桌面/交互系统
  参数：quantum=8, back_seek_max=16KB

kyber（轻量级双队列）：
  算法：读写分离队列，按延迟目标调整深度
  特点：轻量级，低延迟
  适用：快速设备（NVMe SSD）
  参数：latency_target=2ms, read_lat_nsec=2000000

none（无调度）：
  算法：直接下发到设备
  特点：零开销，最低延迟
  适用：超快设备/虚拟化环境
```

| 调度器 | 延迟 | 吞吐 | 公平性 | 适用场景 |
|--------|------|------|--------|----------|
| mq-deadline | 中 | 高 | 好 | 通用（HDD/SSD） |
| bfq | 低 | 中 | 最好 | 桌面/交互 |
| kyber | 最低 | 高 | 一般 | NVMe SSD |
| none | 最低 | 最高 | 无 | 超快设备/虚拟化 |

```bash
# 查看当前调度器
cat /sys/block/sda/queue/scheduler
# [mq-deadline] kyber bfq none

# 设置调度器
echo kyber > /sys/block/nvme0n1/queue/scheduler

# I/O 调度器性能测试
fio --name=test --ioengine=libaio --direct=1 --bs=4k \
    --rw=randread --size=1G --numjobs=4 --runtime=60

# 不同调度器 IOPS 对比（NVMe SSD）
# mq-deadline: ~100K IOPS
# kyber: ~120K IOPS
# none: ~130K IOPS
```

## 网络协议栈（sk_buff/套接字缓冲）

### sk_buff 结构

```
sk_buff（Socket Buffer）= Linux 网络包的核心数据结构

sk_buff 结构：
  head → 指向网络包数据起始
  data → 指向协议数据起始
  tail → 指向协议数据结束
  end  → 指向分配内存结束

  ├── 协议头（Ethernet/IP/TCP/UDP）
  ├── 载荷数据
  └── 元数据（dev, protocol, cb[]）

套接字缓冲区：
  sk_buff → 挂到 socket 的接收/发送队列
  接收队列：sk_buff 链表
  发送队列：sk_buff 链表
  零拷贝：sendfile 时 sk_buff 直接引用文件页
```

### 收包路径详解

```
完整收包路径：
  1. 网卡 DMA 收包
     ├── 数据写入 ring buffer（环形缓冲区）
     ├── ring buffer 大小：256-4096 描述符
     └── 每个描述符指向一个 skb（sk_buff）

  2. 硬中断通知
     ├── 网卡触发 IRQ（硬中断）
     ├── 中断处理：禁用网卡中断，调度 NAPI
     └── NAPI：批量处理减少中断频率

  3. 软中断处理（softirq）
     ├── ksoftirqd 内核线程处理
     ├── 协议栈处理：Ethernet → IP → TCP/UDP
     ├── 校验和验证
     └── 数据放入 socket 接收队列

  4. 应用层读取
     ├── epoll/select/poll 监听 fd
     ├── recv/recvfrom 从 socket 队列读取
     └── 数据拷贝到用户态缓冲区
```

## CPU 缓存一致性（MESI/MOESI/MESIF）

### 协议对比

```
MESI 协议（Intel 早期）：
  M（Modified）：已修改，独占，内存过期
  E（Exclusive）：未修改，独占，内存有效
  S（Shared）：未修改，共享，内存有效
  I（Invalid）：无效

MOESI 协议（AMD）：
  增加 O（Owned）：已修改，共享，内存过期
  → M 状态可降级为 O，无需写回内存

MESIF 协议（Intel 现代）：
  增加 F（Forward）：独占转发者
  → 读请求由 F 状态缓存响应，减少内存访问
```

| 协议 | 状态数 | 写回策略 | 转发机制 | 代表厂商 |
|------|--------|----------|----------|----------|
| MESI | 4 | M 必须写回 | 无 | Intel 早期 |
| MOESI | 5 | O 可不写回 | O 可共享 | AMD |
| MESIF | 5 | M 必须写回 | F 独占转发 | Intel 现代 |

### 伪共享问题详解

```java
// 伪共享问题：同一缓存行的不同变量
class Counter {
    volatile long count1;  // 缓存行 1
    volatile long count2;  // 缓存行 1（伪共享！）
}

// CPU A 修改 count1 → 使 CPU B 的 count2 缓存失效
// CPU B 修改 count2 → 使 CPU A 的 count1 缓存失效
// 性能下降 10x+

// 解决方案 1：缓存行填充
class Counter {
    volatile long count1;
    long p1, p2, p3, p4, p5, p6, p7;  // 填充到不同缓存行
    volatile long count2;
}

// 解决方案 2：@Contended 注解（JDK 8+）
class Counter {
    @sun.misc.Contended
    volatile long count1;
    
    @sun.misc.Contended
    volatile long count2;
}

// 解决方案 3：分片计数
class Counter {
    // 每个 CPU 核心独立计数
    private final AtomicLong[] counts = new AtomicLong[Runtime.getRuntime().availableProcessors()];
    // 合并时求和
}
```

## cgroup v1 vs v2 资源隔离差异

### 架构差异

```
cgroup v1（经典版）：
  每个资源独立层级树：
    /sys/fs/cgroup/cpu/docker/<id>
    /sys/fs/cgroup/memory/docker/<id>
    /sys/fs/cgroup/blkio/docker/<id>

  优点：每个控制器独立管理
  缺点：
    - 层级复杂（多棵树）
    - 资源分配不一致（CPU/内存不同步）
    - 不支持统一资源控制

cgroup v2（统一层级）：
  单一层级树：
    /sys/fs/cgroup/system.slice/docker-<id>.scope/

  优点：
    - 统一资源分配
    - 更好的内存管理（memory.high, memory.low）
    - PSI（Pressure Stall Information）监控
    - io.max 替代 blkio

  缺点：
    - 需要内核 4.5+ 支持
    - 部分旧工具不兼容
```

| 维度 | cgroup v1 | cgroup v2 |
|------|-----------|-----------|
| 层级 | 多层级（每资源一棵树） | 单一层级（统一树） |
| CPU | cpu.cfs_quota_us | cpu.max |
| 内存 | memory.limit_in_bytes | memory.max |
| IO | blkio.throttle.* | io.max |
| PSI | 不支持 | 支持 |
| 内核版本 | 2.6+ | 4.5+ |
| Docker 版本 | 18.09 前 | 18.09+ |

### 资源限制实现

```bash
# cgroup v1 CPU 限制
echo 200000 > /sys/fs/cgroup/cpu/docker/<id>/cpu.cfs_quota_us  # 200ms/100ms
echo 100000 > /sys/fs/cgroup/cpu/docker/<id>/cpu.cfs_period_us  # 100ms 周期

# cgroup v2 CPU 限制
echo "200000 100000" > /sys/fs/cgroup/system.slice/docker-<id>.scope/cpu.max

# cgroup v1 内存限制
echo 1073741824 > /sys/fs/cgroup/memory/docker/<id>/memory.limit_in_bytes  # 1GB

# cgroup v2 内存限制
echo 1073741824 > /sys/fs/cgroup/system.slice/docker-<id>.scope/memory.max

# cgroup v2 IO 限制
echo "8:0 rbps=104857600 wbps=52428800" > /sys/fs/cgroup/system.slice/docker-<id>.scope/io.max
# 读 100MB/s，写 50MB/s
```

## NUMA 架构（本地内存/远程内存访问）

### NUMA 拓扑

```
NUMA（Non-Uniform Memory Access）= 非统一内存访问

双路服务器 NUMA 拓扑：
  Node 0：CPU 0-15 + 本地内存 64GB
  Node 1：CPU 16-31 + 本地内存 64GB

  CPU 访问本地内存：~100ns（快）
  CPU 访问远程内存：~150ns（慢 50%）

  互联带宽：QPI/UPI ~40GB/s
  本地内存带宽：~50GB/s
```

### NUMA 调度优化

```bash
# 查看 NUMA 拓扑
numactl --hardware
numactl --membind=0 ./myapp    # 绑定内存到 Node 0
numactl --cpubind=0 ./myapp    # 绑定 CPU 到 Node 0
numactl --interleave=all ./myapp  # 内存交织（适合大内存应用）

# Java NUMA 感知
# JDK 14+ 支持
java -XX:+UseNUMA -Xmx64g -jar app.jar

# MySQL/ES/Redis NUMA 优化
# 1. 绑定到单个 NUMA 节点
# 2. 关闭 NUMA balancing
echo 0 > /proc/sys/kernel/numa_balancing
```

## Linux 中断处理（硬中断/软中断/NAPI）

### 中断处理流程

```
硬中断（IRQ）：
  1. 网卡触发 IRQ
  2. 中断处理程序执行（时间敏感）
  3. 禁用网卡中断
  4. 调度 NAPI 或 softirq

软中断（softirq）：
  1. ksoftirqd 内核线程处理
  2. 协议栈处理（Ethernet/IP/TCP）
  3. 数据放入 socket 队列

NAPI（New API）：
  1. 硬中断禁用网卡中断
  2. NAPI poll 批量处理数据包
  3. 处理完重新启用中断
  4. 减少中断频率，提高吞吐
```

| 中断类型 | 处理函数 | 执行时间 | 上下文 |
|----------|----------|----------|--------|
| 硬中断 | IRQ handler | < 10μs | 中断上下文 |
| 软中断 | softirq handler | ~100μs | 软中断上下文 |
| tasklet | softirq 变体 | ~10μs | 软中断上下文 |
| 工作队列 | workqueue | 无限制 | 进程上下文 |

## 进程间通信（管道/共享内存/消息队列/信号量）

### IPC 方式对比

| IPC 方式 | 速度 | 复杂度 | 跨机 | 适用场景 |
|----------|------|--------|------|----------|
| 管道 | 中 | 低 | 否 | 父子进程通信 |
| 命名管道 | 中 | 低 | 否 | 无血缘进程 |
| 共享内存 | 最快 | 高 | 否 | 大数据量高频 |
| 消息队列 | 中 | 中 | 否 | 异步消息 |
| 信号量 | — | 中 | 否 | 资源同步 |
| 信号 | — | 低 | 否 | 异步通知 |
| Socket | 慢 | 高 | 是 | 跨主机通信 |

```
共享内存实现：
  1. shmget() 或 mmap() 创建共享内存段
  2. shmat() 或 mmap() 映射到进程地址空间
  3. 进程直接读写（无需系统调用）
  4. 配合信号量/锁做同步

消息队列（POSIX）：
  mq_open() → mq_send() → mq_receive() → mq_close()
  支持优先级
  支持通知（信号/线程）

管道（Pipe）：
  pipe() → 读端 fd[0] / 写端 fd[1]
  半双工字节流
  缓冲区大小：64KB（Linux 默认）
```

## 与其他板块的关系

- **计算机原理**：硬件组成/虚拟内存/Cache 的硬件底座，本文是其"内核视角"延伸。
- **Java虚拟机 / 并发编程**：JMM、synchronized 锁升级、AQS 都对应本文的 futex/内存屏障/原子操作。
- **Linux排查**：本文是"机制"，那里是"命令工具集"。
- **网络**：epoll/收包路径与 TCP 协议栈衔接（网络.md 有 PPC/prefork 演进）。
- **场景设计**：高并发 I/O 模型、线程模型选择依赖本文；生产问题定位依赖状态机/负载/IO。
- **源码系列**：Netty（epoll 封装）、Redis（单线程事件循环）、Kafka（零拷贝/mmap）、Tomcat（连接模型）都是本文机制的工程落地。
- **云原生**：namespace/cgroup/CFS 是容器调度的地基。

---

## 十二、速查表

| 主题 | 一句话 |
|------|--------|
| 系统调用 | 用户态→内核态唯一合法入口，缓冲减少切换 |
| 进程状态 | D 不可中断休眠=卡 IO；Z 僵尸=父未 wait |
| fork | COW 写时复制，父子共享页 |
| CFS | 按 vruntime 红黑树调度，nice 调权重 |
| 优先级反转 | 低优先级任务持锁 → 优先级继承解决 |
| futex | 用户态 CAS 快速路径 + 内核睡眠慢路径 |
| 死锁四条件 | 互斥/占有等待/不可剥夺/循环等待 |
| 页表/TLB | 多级页表省空间，TLB 缓存加速，大页提命中 |
| 碎片 | 外部碎片靠伙伴系统，内部碎片靠 slab |
| OOM | dmesg 查杀手，容器堆留余量 |
| VFS/inode | 一切皆文件；硬链接同 inode，软链接存路径 |
| 零拷贝 | sendfile 2 次拷贝，文件服务高吞吐关键 |
| epoll | 红黑树 + 就绪链表，O(就绪数)，支持 ET |
| 收包路径 | 网卡→ring→硬中断→softirq→socket 队列 |
| IPC | 共享内存最快；跨机只能 socket |
| 容器 | namespace 隔离视角 + cgroup 限制资源 |

---

## 面试高频问题（20+ 条）

1. **用户态和内核态的区别？** 权限不同；用户态不能碰硬件，系统调用陷入内核态执行特权指令。
2. **系统调用开销为什么大？** 上下文保存/恢复、权限切换、TLB/Cache 污染；用缓冲减少调用次数。
3. **进程和线程的区别？** 进程=资源分配单位（独立地址空间），线程=调度单位（共享地址空间）。
4. **进程有哪些状态？D 状态是什么？** R/S/D/T/Z；D 是不可中断休眠，多在等磁盘 IO，大量 D 是 IO 瓶颈。
5. **什么是僵尸进程？怎么处理？** 子进程退出父进程未 wait；父进程 wait/reap，否则由 init 收养。
6. **fork 后父子共享什么？** 共享物理页（COW），写入才复制；独立地址空间但内容相同。
7. **线程模型有哪些？Java 线程是哪种？** 1:1/N:1/M:N；Java 是 1:1 内核线程，虚拟线程是 M:N 思想。
8. **CFS 怎么调度？** vruntime 最小者优先，红黑树 O(log n)，nice 影响权重。
9. **什么是优先级反转？怎么解决？** 高优先级等低优先级持锁者，被中优先级插队；优先级继承。
10. **上下文切换成本？** 保存/恢复寄存器+页表，微秒级，核心代价是缓存/TLB 失效；线程多反而慢。
11. **自旋锁和互斥锁区别？** 自旋忙等不睡（临界区短/多核）；互斥睡眠（临界区长）。
12. **futex 是什么？** 用户态 CAS 快速路径，失败才系统调用睡眠；synchronized 重量级、AQS park 都基于它。
13. **死锁四条件？如何预防？** 互斥/占有并等待/不可剥夺/循环等待；破坏任一，如全局按序加锁。
14. **虚拟内存的作用？** 隔离、扩展（可超物理内存）、按需加载。
15. **TLB 是什么？大页有什么用？** 页表缓存；HugePage 减页表项、提命中，适合大内存服务。
16. **伙伴系统和 slab？** 伙伴按 2 幂管理物理页防外部碎片；slab 对象缓存防内部碎片。
17. **写时复制 COW？** fork/共享页写时才复制，省内存；注意 Redis BGSAVE 场景内存峰值。
18. **OOM 怎么排查？** dmesg | grep oom 看杀手与 oom_score；容器里注意 -Xmx 别顶到 cgroup 上限。
19. **硬链接和软链接区别？** 硬链接同 inode 是真身；软链接是路径引用，可跨 FS、目标删则失效。
20. **什么是零拷贝？** 免用户态参与（sendfile/splice），文件服务高吞吐关键。
21. **epoll 为什么快？** 红黑树管 fd + 就绪链表只返回就绪的；LT/ET 触发；O(就绪数)。
22. **select/poll/epoll 区别？** select 1024 上限位图 O(n)；poll 链表 O(n)；epoll 事件驱动 O(就绪数)。
23. **ET 和 LT 区别？** LT 不读完一直通知；ET 只在状态变化通知一次，需一次读空，防漏。
24. **一个网络包到应用的路径？** 网卡 DMA→ring→硬中断→软中断处理协议栈→socket 队列→应用 recv。
25. **Java 高并发为什么用 Netty 而不是一连接一线程？** 线程切换贵、内核栈占内存；epoll+事件循环线程少连接多。
26. **容器和虚拟机区别？** 容器共享内核（namespace+cgroup 隔离），虚拟化用独立内核（VM 隔离更强）。
27. **容器里 Java 内存的坑？** JVM 不感知 cgroup → UseContainerSupport/显式 -Xmx；CPU 配额影响 GC 并行度。
28. **IPC 有哪些？哪个最快？** 管道/消息队列/共享内存/信号量/信号/socket；共享内存最快，跨机用 socket。
