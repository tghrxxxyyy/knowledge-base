...
| 问题类型 | 症状 | 根因 | 解决方案 |
|----------|------|------|----------|
| 连接超时 | 客户端频繁超时 | LB 健康检查配置不当 | 调整健康检查间隔和阈值 |
| 502/504 错误 | 间歇性网关错误 | 后端实例异常或过载 | 增加后端实例数，调整超时 |
| DNS 解析慢 | 首次请求延迟高 | DNS 缓存 TTL 过长 | 降低 TTL，使用本地 DNS 缓存 |
| SSL 握手慢 | HTTPS 延迟增加 | 证书链过长或未启用 OCSP Stapling | 优化证书链，启用 Stapling |
| 跨域请求失败 | 浏览器 CORS 错误 | CORS 头配置缺失 | 在网关/CDN 配置 Access-Control 头 |
| 带宽突增 | 网络费用飙升 | DDoS 或爬虫 | 启用 WAF，配置限流 |

### 17.2 VPC 网络排障流程

```mermaid
flowchart TD
    A[客户端请求异常] --> B{DNS 解析正常?}
    B -->|否| C[检查 DNS 配置]
    B -->|是| D{能 ping 通 VIP?}
    D -->|否| E[检查 LB 健康状态]
    D -->|是| F{后端实例健康?}
...
| 检查项 | 推荐配置 | 说明 |
|--------|----------|------|
| 检查协议 | HTTP GET /health | 比 TCP 检查更精确 |
| 检查间隔 | 10s | 平衡灵敏度与开销 |
| 超时时间 | 5s | 避免误判 |
| 不健康阈值 | 3 次连续失败 | 避免偶发抖动误判 |
| 健康阈值 | 2 次连续成功 | 快速恢复流量 |
| 优雅下线 | draining 30s | 排空已有连接 |
...
流量治理能力矩阵：
  1. 流量染色
     - Header 标记（X-Traffic-Group: canary）
     - Cookie 标记
     - IP 段标记

  2. 流量分配
     - 按比例（95% 稳定 / 5% 灰度）
     - 按规则（Header/Cookie/IP）
     - 按用户（白名单/内测用户）
...
```mermaid
flowchart TD
    A[客户端] --> B[Global Server Balancing]
    B --> C[Region A]
    B --> D[Region B]
    C --> E[可用区 A-1]
    C --> F[可用区 A-2]
...
| 指标 | 采集方式 | 告警阈值 | 说明 |
|------|----------|----------|------|
| 连接数 | LB/网关 API | > 10000 | 连接数突增可能异常 |
| 请求延迟 P99 | Prometheus | > 2s | 延迟升高影响体验 |
| 5xx 错误率 | LB 日志 | > 1% | 服务端错误增多 |
| 带宽使用率 | 云监控 | > 80% | 带宽可能不足 |
| DNS 解析时间 | 本地探测 | > 500ms | DNS 问题影响全局 |
| SSL 证书过期 | 定期检查 | < 30 天 | 证书过期导致服务中断 |
...
Nginx 性能调优：
  ...
  proxy_buffer_size: 4k           # 代理缓冲
  proxy_buffers: 4 32k            # 代理缓冲区
  open_file_cache: max=10000      # 文件缓存
```
...
|----------|----------|------|------|
| 单 VPC | 小型业务 | 简单 | 扩展性差 |
| 多 VPC 对等 | 多环境隔离 | 灵活 | 管理复杂 |
| 中心辐射型 | 多区域 | 统一管控 | 中心带宽瓶颈 |
| 云企业网 | 跨区域 | 全互联 | 成本高 |
| Service Mesh | 微服务 | 细粒度 | 复杂度高 |
...
| 策略 | 说明 | 适用场景 |
|------|------|----------|
| 按功能划分 | 公有/私有/数据子网 | 通用架构 |
| 按环境划分 | Dev/Staging/Prod 独立子网 | 多环境隔离 |
| 按安全级别 | DMZ/应用/数据分层 | 金融合规 |
| 按可用区 | 每 AZ 独立子网 | 高可用 |

```yaml
# 子网 CIDR 规划
VPC CIDR: 10.0.0.0/16 (65536 IPs)
  公有子网:  10.0.1.0/24 (254 IPs) - ALB/NAT/堡垒机
  应用子网:  10.0.10.0/22 (1022 IPs) - 微服务 Pod
  数据子网:  10.0.100.0/24 (254 IPs) - RDS/Redis
  管理子网:  10.0.200.0/24 (254 IPs) - 运维/监控
```
...
aws ec2 create-route \
  ...
  --nat-gateway-id ngw-0abc123

# NAT Gateway 成本优化
# 每个 NAT Gateway $0.045/h + 数据处理费 $0.045/GB
# 方案1：每 AZ 一个 NAT（高可用）
# 方案2：共享 NAT（低成本，但单点）
```
...
| NAT 方案 | 成本/月 | 高可用 | 适用 |
|----------|---------|--------|------|
| 每 AZ 一个 NAT | $97+流量 | 高 | 生产环境 |
| 共享单 NAT | $32+流量 | 低 | 测试/非关键 |
| NAT 实例 | $10-50 | 中 | 低成本 |

### 19.3 VPC Endpoint 配置

```yaml
# VPC Endpoint 配置（避免公网流量）
Gateway Endpoint:
  - S3: 免费，高带宽
  - DynamoDB: 免费，低延迟
...
Interface Endpoint:
  - SSM: $0.01/h/ENI
  - Secrets Manager: $0.01/h/ENI
  - ECR: $0.01/h/ENI
  - CloudWatch: $0.01/h/ENI
...
### 20.1 ALB vs NLB 配置对比

| 参数 | ALB | NLB |
|------|-----|-----|
| 监听端口 | 80/443 | 80/443/任意 |
| 健康检查 | HTTP 200 | TCP 连接 |
| SSL 终止 | 支持 | 支持 |
| WebSocket | 支持 | 不支持 |
| gRPC | 支持 | 支持 |
| 静态 IP | 不支持 | 支持 |
...
| 算法 | 说明 | 配置 |
|------|------|------|
| Round Robin | 轮询（默认） | 无需配置 |
| Least Outstanding Requests | 最少未完成请求 | ALB 默认 |
| Consistent Hash | 一致性哈希 | 基于 Header/Cookie |
| 源 IP 哈希 | 按客户端 IP | NLB 默认 |
...
```text
用户 → 边缘节点(L1) → 区域节点(L2) → 源站(L3)
  缓存命中率: L1(80%) → L2(15%) → L3(5%)
  回源比例 = 1 - 命中率
```

### 21.2 缓存策略配置

| 资源类型 | TTL | Cache-Control | 缓存键 |
|----------|-----|---------------|--------|
| HTML | 5min | public, max-age=300 | URL |
| JS/CSS | 1年 | public, max-age=31536000, immutable | URL+Hash |
| 图片 | 30天 | public, max-age=2592000 | URL |
| API | 不缓存 | no-cache | — |
...
| 记录类型 | TTL 推荐 | 说明 |
|----------|----------|------|
| A 记录 | 300s (5min) | 便于故障切换 |
| CNAME | 600s (10min) | CDN 域名 |
| MX | 3600s (1h) | 邮件服务 |
| TXT | 300s (5min) | 验证记录 |
...
| 实践 | 说明 |
|------|------|
| 自动续期 | ACM/Let's Encrypt 自动续期 |
| 通配符证书 | `*.example.com` 覆盖子域名 |
| 证书透明度 | 监控 CT 日志防止伪造 |
...
熔断状态机：
  ...
  OPEN → 超时 → HALF-OPEN（试探）
  HALF-OPEN → 成功 → CLOSED
  HALF-OPEN → 失败 → OPEN

配置参数：
  failure_rate_threshold: 50%  # 错误率阈值
  slow_call_rate_threshold: 80% # 慢调用率
  wait_duration_in_open_state: 30s  # 熔断等待时间
...
| 层级 | 攻击类型 | 防护工具 |
|------|----------|----------|
| L3/L4 | SYN/UDP Flood | 云 DDoS 防护 |
| L7 | HTTP Flood | WAF 规则 |
| DNS | DNS 放大 | DNS 防护 |
...
SecurityGroups:
  ALB-SG:
    ...
    Outbound:
      - Protocol: TCP, Port: 8080, Destination: App-SG
  ...
  DB-SG:
    Inbound:
      - Protocol: TCP, Port: 3306, Source: App-SG
    Outbound: []  # 禁止出站
```
...
| 指标 | 采集方式 | 告警阈值 |
|------|----------|----------|
| 入站流量 | CloudWatch/VPC Flow Logs | >80% 带宽 |
| 延迟 P99 | ALB/NLB 指标 | >500ms |
| 5xx 错误率 | ALB 访问日志 | >1% |
| 连接数 | LB 指标 | >80% 上限 |
| 健康检查失败 | Route 53 | 连续 3 次 |
...
## 二十八、生产问题排查：延迟/丢包/连接超时

### 28.1 常见问题与排查

| 问题 | 可能原因 | 排查步骤 |
|------|----------|----------|
| 延迟高 | 跨 AZ/跨地域 | traceroute/CloudWatch |
| 丢包 | 安全组/ACL | VPC Flow Logs |
| 连接超时 | 健康检查失败 | LB 控制台 |
| 502 错误 | 后端实例异常 | 后端日志 |
...
| 组件 | 参数 | 推荐值 |
|------|------|--------|
| Nginx upstream | keepalive | 64-128 |
| ALB 连接保持 | idle_timeout | 60s |
| NLB 连接超时 | tcp.idle.timeout | 350s |
| 客户端连接池 | max_connections | 100-200 |
...
|------|----------|------|
| ALB | 自动扩展 | 无需手动 |
| NAT Gateway | 按流量计费 | 自动 |
| EC2/ECS | Auto Scaling | 按 CPU/内存 |
| EKS | HPA/KEDA | 按自定义指标 |

## 十七-1、云网络进阶与实战

### 17-1.1 VPC Flow Logs 高级分析
...
# 使用 Athena 查询 Flow Logs
CREATE EXTERNAL TABLE flow_logs (
  version int,
  account_id string,
  interface_id string,
  srcaddr string,
...
| 规则顺序 | 按编号优先级 | 所有规则评估 |
| 默认行为 | 拒绝所有 | 允许所有同组流量 |
| 关联数量 | 一个子网一个 NACL | 一个 ENI 多个安全组 |
| 有状态 | 无状态（需双向规则） | 有状态（自动允许回包） |
| 典型场景 | 子网级黑名单 | 实例级白名单 |
...
### 17-1.3 AWS Network Firewall 集成

```yaml
# AWS Network Firewall 资源组模板
Resources:
  NetworkFirewall:
          ...
          - ActionDefinition:
              PublishMetricAction:
                Dimensions:
                  - Value: blocked
                    Name: action
            ActionName: custom-metric

  StatelessRuleGroup:
    Type: AWS::NetworkFirewall::RuleGroup
    Properties:
      RuleGroupName: stateless-group
      Type: STATELESS
      Capacity: 100
      RuleGroup:
        RulesSource:
              ...
              - RuleDefinition:
                  MatchAttributes:
                    ...
                    Sources:
                      - AddressDefinition: 0.0.0.0/0
                RuleOptions:
                  - Keyword: "aws:destination_ports"
                    Settings:
                      - 443
```

### 17-1.4 Gateway Load Balancer (GLB) 使用场景

| 场景 | 说明 | 架构 |
|------|------|------|
| 第三方虚拟设备 | 防火墙、IDS/IPS、WAF | GLB + 第三方 VM |
| 安全服务链 | 多个安全设备串联 | GLB → FW → IDS → WAF |
| 混合云流量检查 | 本地/云端流量审计 | GLB + 专线 |
| 合规审计 | 满足 PCI-DSS/HIPAA | GLB + 审计设备 |
...
aws ec2 create-vpc \
  ...
  --tag-specifications 'ResourceType=vpc,Tags=[{Key=Name,Value=my-vpc-ipv6}]'
...
aws ec2 create-subnet \
  ...
  --ipv6-native \
  --ipv6-cidr-block 2600:1234:5678:1::/64
...
sysctl -w net.ipv4.tcp_min_mss=536
...
### 17-1.7 AWS Global Accelerator

| 特性 | Global Accelerator | CloudFront |
|------|-------------------|------------|
| 层级 | L3/L4 | L7 |
| 缓存 | 无缓存 | 边缘缓存 |
| 静态IP | 2 个 Anycast IP | 无 |
| 源站 | ALB/NLB/EIP | S3/ALB/EC2 |
| 适用 | TCP/UDP 全球加速 | HTTP/HTTPS 静态加速 |
...
### 17-1.8 私有终端节点（VPC Endpoint）类型对比

| 类型 | 服务 | 协议 | 适用 |
|------|------|------|------|
| Gateway | S3/DynamoDB | TCP | AWS 服务访问 |
| Interface | 其他 AWS 服务 | TCP | VPC 内访问 |
| Gateway Load Balancer | 第三方设备 | TCP/UDP | 安全设备集成 |
| PrivateLink | 自定义服务 | TCP | 跨 VPC 服务 |

```mermaid
flowchart TD
    A[VPC内实例] --> B{访问AWS服务}
    B -->|S3/DynamoDB| C[Gateway Endpoint]
    B -->|其他AWS服务| D[Interface Endpoint]
    B -->|第三方设备| E[GLB Endpoint]
    A -->|跨VPC服务| F[PrivateLink]
    C -->|免费| G[S3]
    D -->|按量付费| H[其他服务]
    E -->|按量付费| I[第三方设备]
```
...
aws fis create-experiment-template \
  ...
  --actions '{
    "action-1": {
      ...
      "targets": {
        "Target Arn(s)": "target-1"
      }
    }
  }' \
  --stopConditions '{
    "stop-1": {
      "source": "aws:fis:cloudwatch-alarm",
...
compliance_checklist:
  network_segmentation:
    - description: "生产/开发/测试环境 VPC 隔离"
      check: "VPC 数量 >= 环境数"
    - description: "数据库子网无公网出口"
      check: "NAT Gateway 不在数据库子网"
    - description: "管理子网与业务子网隔离"
      check: "堡垒机单独子网"

  access_control:
    - description: "安全组最小权限开放"
      check: "仅开放必要端口"
    - description: "NACL 双向规则完整"
      check: "入站出站规则匹配"
    - description: "VPC Flow Logs 开启"
      check: "Flow Logs 状态为 ACTIVE"

  encryption:
    - description: "传输加密 TLS 1.2+"
      check: "ALB/NLB TLS 策略 >= TLSv1.2"
    - description: "VPN 加密 AES-256"
      check: "IPSec 配置验证"
    - description: "DNS over HTTPS"
      check: "Route 53 DNSSEC 启用"

  monitoring:
    - description: "VPC Flow Logs 监控"
      check: "CloudWatch 告警配置"
    - description: "AWS Config 规则"
      check: "安全组合规检查"
    - description: "GuardDuty 启用"
      check: "威胁检测开启"
```
