...
| 重试策略 | 说明 | 配置 |
|----------|------|------|
| 固定间隔 | 固定时间重试 | retryInterval=30 |
| 指数退避 | 指数增长重试间隔 | retryBackoff=2 |
| 最大重试 | 限制重试次数 | retryMax=3 |

### 执行器弹性伸缩

| 伸缩模式 | 说明 | 配置 |
|----------|------|------|
| 手动伸缩 | 手动调整执行器数 | 管理界面配置 |
| 自动伸缩 | 根据负载自动调整 | 需集成K8s |
| 定时伸缩 | 按时间计划调整 | Cron表达式 |
...
| 问题 | 原因 | 解决方案 |
|------|------|----------|
| 任务不执行 | 执行器未注册 | 检查执行器状态 |
| 执行超时 | 任务执行时间过长 | 增加超时时间 |
| 任务丢失 | 调度器故障 | 检查调度器状态 |
| 资源耗尽 | 执行器资源不足 | 增加执行器资源 |
| 并发冲突 | 多实例执行同一任务 | 配置单机串行 |
...
### XXL-JOB vs DolphinScheduler vs Airflow对比

| 特性 | XXL-JOB | DolphinScheduler | Airflow |
|------|---------|------------------|---------|
| 架构 | 调度中心+执行器 | 主从架构 | Scheduler+Executor |
| 调度 | 时间触发 | 时间触发 | 时间/事件触发 |
| API | Java API | REST API | Python API |
| DAG | 不支持 | 支持 | 支持 |
| 社区 | 中文社区 | Apache社区 | Apache社区 |
