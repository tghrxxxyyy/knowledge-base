...
## EventBridge Pipes 详解

### Pipes 架构

```mermaid
flowchart LR
    SOURCE[事件源] -->|过滤/转换| PIPE[EventBridge Pipe]
    PIPE -->|目标| TARGET[目标服务]
```
...
```json
{
  ...
  "FilterCriteria": {
    "Filters": [
      {
        "Pattern": "{\"source\": [\"custom.orders\"]}"
      }
    ]
  },
  "EnrichmentParameters": {
    "InputTemplate": "{\"id\": \"<$.id>\", \"data\": \"<$.data>\"}"
  }
}
```
...
| Pipes 功能 | 说明 | 适用场景 |
|------------|------|----------|
| 事件过滤 | JSON Path 过滤 | 只处理特定事件 |
| 事件转换 | InputTemplate | 格式标准化 |
| 事件丰富 | 调用 Lambda | 补充元数据 |
| 流式处理 | 多目标 | 事件分发 |
...
| 类型 | 语法 | 示例 |
|------|------|------|
| Rate | rate(N minutes/hours/days) | rate(5 minutes) |
| Cron | cron(分 时 日 月 周) | cron(0 9 ? * MON *) |
| One-Time | at(YYYY-MM-DD) | at(2025-01-01T00:00:00) |
...
> 灵活时间窗口允许 AWS 在指定窗口内选择最佳执行时间，优化成本和成功率。
...
## EventBridge Pipes 高级用法

### Pipes 架构

```text
EventBridge Pipes 数据流：
  事件源（Source）
    ├── DynamoDB Stream
    ├── Kinesis Stream
    ├── SQS Queue
...
{
  "Name": "my-pipe",
  "Source": "arn:aws:sqs:us-east-1:123456789:my-queue",
  "Target": "arn:aws:lambda:us-east-1:123456789:my-function",
  "RoleArn": "arn:aws:iam::123456789:role/pipe-role",
  "FilterCriteria": {
    "Filters": [
      {
        "Pattern": "{\"source\": [\"my.app\"], \"type\": [\"order.created\"]}"
      }
    ]
  },
  "Transformations": [
    {
      "InputTemplate": "{\"orderId\": \"<$.detail.orderId>\", \"amount\": <$.detail.amount>}"
    }
  ],
  "SourceParameters": {
    "SqsQueueParameters": {
...
路由规则：
  1. 基于内容路由
     - 事件类型（order.created → OrderService）
     - 事件来源（aws.ec2 → InfraService）
     - 事件属性（region=us-east-1 → USEastService）
...
```json
{
  ...
  "Target": {
    "Arn": "arn:aws:lambda:us-east-1:123456789:my-function",
    "RoleArn": "arn:aws:iam::123456789:role/scheduler-role",
    "Input": "{\"reportType\": \"daily\", \"date\": \"<aws.eventbridge.date>\"}"
  },
  "ActionAfterCreation": "ENABLE",
  "State": "ENABLED"
}
```
...
| 特性 | 说明 | 使用场景 |
|------|------|----------|
| 灵活时间窗口 | 允许任务在指定窗口内执行 | 避免资源峰值 |
| 时区支持 | 指定时区执行 | 跨时区任务 |
| 重试策略 | 自动重试失败任务 | 可靠性保证 |
| 死信队列 | 存放失败事件 | 事后处理 |
| 跨账户调度 | 调度其他账户的目标 | 多账户管理 |
  ...
  "Statement": [
    {
      ...
      "Resource": [
        "arn:aws:events:us-east-1:123456789:event-bus/cross-account-bus"
      ]
    }
  ]
}
...
| 指标 | 说明 | 告警阈值 |
|------|------|----------|
| SuccessfulInvocations | 成功调用数 | <95% |
| FailedInvocations | 失败调用数 | >0 |
| ThrottledRules | 被限流规则数 | >0 |
| MatchedEvents | 匹配事件数 | 异常波动 |
| IngestedEvents | 接收事件数 | 异常波动 |
| DeliveryTime | 交付时间 | >5s |
| Invocation Age | 调用延迟 | >30s |
...
MetricAlarms:
  ...
  - AlarmName: EventBridge-ThrottledRules
...
| 方案 | 每百万事件成本 | 运维成本 | 延迟 | 适用场景 |
|------|---------------|----------|------|----------|
| EventBridge | $1.00 | 极低 | <1s | 云原生集成 |
| SNS + SQS | $2.00 | 低 | <1s | 消息队列 |
| Kafka | $0.10-0.50 | 高 | <10ms | 大数据流 |
| 自建MQ | $0.05-0.20 | 极高 | <10ms | 特殊需求 |
| RabbitMQ | $0.10-0.30 | 中 | <5ms | 企业消息 |
...
```text
Fan-Out 架构：
  事件源 → EventBridge → 规则1 → Lambda-1（处理用户）
                  ↓
              规则2 → Lambda-2（处理订单）
                  ↓
              规则3 → Lambda-3（发送通知）
...
Fan-In 架构：
  Lambda-1 ──┐
  Lambda-2 ──┼──→ SQS/Step Functions ──→ 聚合Lambda
  Lambda-3 ──┘

实现方式：
  1. SQS + 消费者
  2. Step Functions + 并行状态
  3. Kinesis + 聚合消费者
```
...
```text
Filter 架构：
  事件源 → EventBridge → 规则1（source=order.*）→ Lambda-1
                       → 规则2（source=payment.*）→ Lambda-2
                       → 规则3（source=user.*）→ Lambda-3
...
```json
{
  ...
  "registryId": "12345678-1234-1234-1234-123456789012",
  "schemaVersionNumber": {
    "latestVersion": true,
    "minorVersion": 0
  },
  "schema": "{\"type\":\"object\",\"properties\":{\"orderId\":{\"type\":\"string\"},\"amount\":{\"type\":\"number\"},\"userId\":{\"type\":\"string\"}},\"required\":[\"orderId\",\"amount\",\"userId\"]}",
...
validation:
  enabled: true
  schemaRegistry: "arn:aws:eventbridge:us-east-1:123456789:schema-registry"
...
rules:
  - name: order-created
    schema: "OrderCreated"
    validation: strict
    action: reject  # 验证失败时拒绝事件

  - name: user-updated
    schema: "UserUpdated"
    validation: lenient
    action: log  # 验证失败时记录日志
```
  ...
  "Targets": [
    {
      ...
      "Arn": "arn:aws:lambda:us-east-1:123456789:my-function",
      "DeadLetterConfig": {
        "Arn": "arn:aws:sqs:us-east-1:123456789:dlq"
      },
      "RetryPolicy": {
        "MaximumRetryAttempts": 3,
        "MaximumEventAgeInSeconds": 3600
...
| CQRS | 命令查询分离 | 读写分离 |
| Saga | 分布式事务 | 长事务 |
| Choreography | 编排模式 | 松耦合 |
| Orchestration | 协调模式 | 复杂流程 |
