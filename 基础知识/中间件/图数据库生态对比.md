...
| 问题类型 | 症状 | 根因 | 解决方案 |
|----------|------|------|----------|
| 查询超时 | 复杂遍历超时 | 深度过大或索引缺失 | 限制深度，添加索引 |
| 内存溢出 | 大图查询 OOM | 全图加载 | 分页遍历，流式处理 |
| 写入瓶颈 | 批量导入慢 | 事务冲突或索引更新 | 批量提交，异步索引 |
| 图分裂 | 连通分量过多 | 数据模型不合理 | 重新设计图模型 |
| 索引失效 | 查询全表扫描 | 索引未创建或失效 | 检查并重建索引 |
...
| 算法 | 时间复杂度 | 适用场景 | 优化策略 |
|------|-----------|----------|----------|
| 最短路径 | O(V+E) | 社交网络 | 双向搜索 |
| PageRank | O(k*(V+E)) | 推荐系统 | 近似算法 |
| 社区检测 | O(V*logV) | 聚类分析 | 并行化 |
| 连通分量 | O(V+E) | 数据清洗 | 增量计算 |
| 中心性分析 | O(V*E) | 关键节点 | 采样近似 |
...
groups:
  - name: graph-alerts
    rules:
      - alert: GraphQueryTimeout
        expr: graph_query_duration_seconds > 30
        for: 5m
        labels:
          severity: warning
        annotations:
      ...
      - alert: GraphMemoryHigh
        expr: graph_memory_usage_bytes / graph_memory_limit_bytes > 0.85
        for: 5m
        labels:
          severity: warning
        annotations:
      ...
      - alert: GraphClusterSyncDelay
        expr: graph_cluster_sync_delay_seconds > 60
        for: 5m
        labels:
          severity: warning
        annotations:
...
风控流程：
  1. 事件触发（转账/开户）
  2. 图遍历：从实体（手机号/设备/IP）出发，N 跳遍历
  3. 发现关联：共用设备、共用 IP、资金环路
  4. 风险评分：图特征 + 规则 + 模型
  5. 决策：拦截/人工审核/通过
...
| 节点类型 | 标签 | 关键属性 |
|----------|------|----------|
| 用户 | Person | 姓名、身份证、手机号 |
| 银行卡 | Card | 卡号、银行、类型 |
| 设备 | Device | IMEI、MAC、IP |
| 地址 | Address | 省市区、经纬度 |
| 商户 | Merchant | 名称、类型、MCC |

| 边类型 | 关系 | 属性 |
|--------|------|------|
| 持有 | Person → Card | 开卡时间、用途 |
| 使用 | Person → Device | 使用时间、频率 |
| 转账 | Card → Card | 金额、时间、备注 |
| 登录 | Person → Device | 登录时间、IP |
| 访问 | Device → Merchant | 交易时间、金额 |

### 17.3 Cypher 风控查询示例

```cypher
// 查询：找出与目标用户 2 跳内的所有关联账户
MATCH path = (target:Person {id: 'U1001'})-[*1..2]-(related:Person)
WHERE related.id <> 'U1001'
RETURN related.id, related.name, length(path) AS hops
ORDER BY hops;

// 查询：检测资金环路（A→B→C→A）
MATCH cycle = (a:Card)-[:TRANSFER*3..5]->(a)
WHERE a.cardNo = 'C001'
RETURN nodes(cycle) AS involved_cards, 
       [r IN relationships(cycle) | r.amount] AS amounts;

// 查询：找出高频共用设备的用户（团伙）
MATCH (d:Device)<-[:USES]-(p1:Person),
      (d)<-[:USES]-(p2:Person)
WHERE p1 <> p2
WITH p1, p2, COUNT(d) AS shared_devices
WHERE shared_devices >= 3
RETURN p1, p2, shared_devices
...
属性图模型（Neo4j）：
  Node: 张三 {name: "张三", age: 30}
  Relationship: 工作于 {since: 2020}
  Node: 阿里巴巴 {type: "公司"}
...
|------|------|-----|------|
| 好友推荐 | 用户 | 关注/好友 | 共同邻居 |
| 商品推荐 | 用户/商品 | 购买/浏览 | 协同过滤 |
| 内容推荐 | 用户/内容 | 点赞/分享 | 图嵌入 |
| 金融推荐 | 用户/产品 | 持有/查询 | 社群发现 |

### 19.2 图嵌入（Graph Embedding）
...
| 数据库 | SNB-IC (ops/s) | SNB-IC (latency) | 适用规模 |
|--------|----------------|-------------------|----------|
| Neo4j (单机) | 10,000~50,000 | 1~10ms | 中小规模 |
| NebulaGraph | 50,000~200,000 | 1~5ms | 大规模 |
| JanusGraph | 5,000~30,000 | 5~50ms | 大规模 |
| TigerGraph | 100,000~500,000 | <1ms | 超大规模 |
| ArangoDB | 10,000~50,000 | 1~10ms | 中小规模 |
...
| 数据库 | 节点遍历（ms） | 最短路径（ms） | 社区检测（ms） | 写入吞吐（ops/s） |
|--------|----------------|----------------|----------------|-------------------|
| Neo4j | 0.5 | 2.1 | 15.3 | 12,000 |
| JanusGraph | 1.2 | 5.8 | 42.1 | 8,500 |
| TigerGraph | 0.3 | 1.5 | 10.2 | 15,000 |
| NebulaGraph | 0.4 | 1.8 | 12.5 | 18,000 |
| ArangoDB | 0.8 | 3.2 | 25.6 | 10,000 |

### G.2 图算法执行效率对比

| 算法 | Neo4j GDS | JanusGraph | TigerGraph | 说明 |
|------|-----------|------------|------------|------|
| PageRank | 120ms | 450ms | 80ms | 迭代收敛 |
| Louvain | 350ms | 1200ms | 200ms | 社区检测 |
| Shortest Path | 2ms | 8ms | 1ms | 单源最短路径 |
| Connected Components | 80ms | 300ms | 50ms | 连通分量 |
| GraphSAGE | 500ms | - | 300ms | 图神经网络 |

### G.3 RDBMS 模拟图查询性能

| 查询深度 | RDBMS JOIN（ms） | 图数据库（ms） | 性能差 |
|----------|------------------|----------------|--------|
| 2 层 | 5 | 0.5 | 10x |
| 3 层 | 50 | 1.2 | 42x |
| 4 层 | 500 | 3.5 | 143x |
...
| 安全措施 | 说明 | 配置 |
|----------|------|------|
| 认证 | 用户名密码/证书 | 启用 LDAP/AD |
| 授权 | 角色权限（READ/WRITE/ADMIN） | 最小权限原则 |
| 加密 | 传输 TLS + 存储加密 | TLS 1.2+ |
| 审计 | 操作日志记录 | 启用审计日志 |
| 网络 | VPC 隔离 + 安全组 | 仅允许必要端口 |

### G.6 图数据库应用场景速查

| 场景 | 推荐数据库 | 理由 |
|------|-----------|------|
| 知识图谱 | Neo4j | Cypher 生态丰富 |
| 实时推荐 | TigerGraph | 内置图分析 |
| 社交网络 | NebulaGraph | 高性能遍历 |
| 风控反欺诈 | JanusGraph | 可扩展性好 |
| 元数据管理 | ArangoDB | 多模型支持 |
| 网络安全 | Neo4j | 路径分析强 |
...
| 引擎 | 存储方式 | 适用场景 | 特点 |
|------|----------|----------|------|
| Neo4j Native | 原生存储 | 关系遍历 | 高性能 |
| JanusGraph+Cassandra | 分布式 KV | 大规模 | 可扩展 |
| TigerGraph DFS | 分布式文件 | 分析 | 高吞吐 |
| NebulaGraph RocksDB | KV 存储 | 混合负载 | 均衡 |
| ArangoDB RocksDB | KV 存储 | 多模型 | 灵活 |

### G.9 图查询语言对比

| 语言 | 数据库 | 特点 | 学习曲线 |
|------|--------|------|----------|
| Cypher | Neo4j | 声明式，模式匹配 | 低 |
| Gremlin | JanusGraph | 遍历式，Apache 标准 | 中 |
| GSQL | TigerGraph | 过程式，内置算法 | 高 |
| nGQL | NebulaGraph | 类 Cypher | 低 |
| AQL | ArangoDB | 类 SQL | 低 |
...
neo4j-admin database restore neo4j \
  --from-path=/backup/neo4j-20240101

# JanusGraph 备份（基于 Cassandra）
nodetool snapshot keyspace
nodetool compact keyspace

# TigerGraph 备份
gadmin backup create /backup/tg-$(date +%Y%m%d)
```
...
| 优化策略 | 说明 | 效果 |
|----------|------|------|
| 索引优化 | 为常用属性创建索引 | 查询加速 10-100x |
| 查询重写 | 优化 Cypher/Gremlin | 减少扫描 |
| 缓存调优 | 页面缓存/查询缓存 | 减少 IO |
| 并行查询 | 多线程遍历 | 提升吞吐 |
| 数据分区 | 按图分区 | 减少跨节点 |
...
| 症状 | 可能原因 | 排查步骤 |
|------|----------|----------|
| 查询超时 | 缺少索引/全图扫描 | EXPLAIN 分析 |
| 内存溢出 | 大查询/缓存不足 | 调整内存配置 |
| 写入慢 | 索引过多/锁竞争 | 检查索引和事务 |
| 集群不同步 | 网络/磁盘 | 检查集群状态 |
| 数据不一致 | 事务未提交 | 检查事务隔离 |
...
| 集成方式 | 说明 | 工具 |
|----------|------|------|
| 图嵌入 | 节点/边向量化 | GraphSAGE/Node2Vec |
| 图神经网络 | 端到端训练 | PyG/DGL |
| 知识图谱推理 | 规则+嵌入 | TransE/R-GCN |
| 推荐系统 | 协同过滤+图 | Neo4j GDS |
...
groups:
  - name: graphdb_alerts
    rules:
      - alert: GraphDBQueryTimeout
        expr: graphdb_query_duration_seconds > 10
        for: 5m
        labels:
          severity: warning
      - alert: GraphDBStorageHigh
        expr: graphdb_storage_used_bytes / graphdb_storage_total_bytes > 0.8
        for: 10m
        labels:
          severity: warning
      - alert: GraphDBClusterDown
        expr: graphdb_cluster_nodes_up == 0
        for: 1m
        labels:
          severity: critical
```
...
| 迁移场景 | 源数据库 | 目标数据库 | 工具 |
|----------|----------|-----------|------|
| RDBMS → 图 | MySQL | Neo4j | ETL + Cypher |
| 图 → 图 | Neo4j | NebulaGraph | 导出/导入工具 |
| 图 → RDBMS | Neo4j | PostgreSQL | CSV 导出 |
| 跨云迁移 | 任意 | 任意 | 备份/恢复 |
