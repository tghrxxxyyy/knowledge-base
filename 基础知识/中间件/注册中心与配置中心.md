...
public class HealthChecker {
    ...
    public void checkHealth() {
        ...
        for (ServiceInstance instance : instances) {
            try {
                ...
                serviceRegistry.updateInstance(instance);
                ...
                log.error("Health check failed for instance {}", instance.getId(), e);
                
                // 标记实例为不健康
                instance.setStatus(HealthStatus.unhealthy("Health check failed"));
                serviceRegistry.updateInstance(instance);
            }
        }
    }
    ...
    private HealthStatus performHealthCheck(ServiceInstance instance) {
        ...
        try {
            ...
            // TCP 健康检查
            try (Socket socket = new Socket(instance.getHost(), instance.getPort())) {
...
public class ServiceMetadataManager {
    ...
    public void registerWithMetadata(ServiceInstance instance, Map<String, String> metadata) {
        // 添加元数据
        ...
        log.info("Registered service {} with metadata: {}", instance.getId(), metadata);
    }
    
    public Map<String, String> getServiceMetadata(String serviceId) {
        ServiceInstance instance = serviceRegistry.getInstance(serviceId);
        return instance != null ? instance.getMetadata() : Collections.emptyMap();
    }
    ...
    public ServiceInstance selectInstanceByMetadata(String serviceName, 
        ...
        return instances.stream()
                .filter(instance -> matchesMetadata(instance.getMetadata(), criteria))
                .findFirst()
                .orElse(null);
    }
...
public class ConfigVersionManager {
    ...
    public ConfigVersion createVersion(String configId, String content, String comment) {
        // 获取当前版本
        ConfigVersion currentVersion = configRepository.getCurrentVersion(configId);
        ...
        // 保存版本
        configRepository.saveVersion(newVersion);
        
        // 更新当前版本指针
        configRepository.updateCurrentVersion(configId, newVersion.getVersion());
        
        return newVersion;
    }
    
    public List<ConfigVersion> getVersionHistory(String configId) {
        return configRepository.getVersionHistory(configId);
    }
    
    public ConfigVersion rollback(String configId, int targetVersion) {
        ConfigVersion targetVersionObj = configRepository.getVersion(configId, targetVersion);
        ...
        // 创建回滚版本
        return createVersion(configId, targetVersionObj.getContent(), 
                "Rollback to version " + targetVersion);
    }
    
    public void compareVersions(String configId, int version1, int version2) {
        ConfigVersion v1 = configRepository.getVersion(configId, version1);
        ConfigVersion v2 = configRepository.getVersion(configId, version2);
        
        // 比较差异
        DiffResult diff = DiffUtils.diff(v1.getContent(), v2.getContent());
...
public class ConfigEncryption {
    ...
    public ConfigEncryption() throws Exception {
        // 生成密钥
...
encryption:
  ...
  key-management:
    type: file
    key-file: /etc/config/encryption.key
...
// 客户端缓存优化
@Component
public class ClientCacheOptimizer {
    
    private final Cache<String, ServiceInstance> serviceCache;
    private final Cache<String, String> configCache;
    
    public ClientCacheOptimizer() {
        // 服务实例缓存
    ...
    public ServiceInstance getServiceInstance(String serviceName) {
        return serviceCache.get(serviceName, this::loadServiceInstance);
    }
    
    public String getConfig(String configKey) {
        return configCache.get(configKey, this::loadConfig);
    }
    
    private ServiceInstance loadServiceInstance(String serviceName) {
        // 从注册中心加载
    ...
    private String loadConfig(String configKey) {
        // 从配置中心加载
        return configCenter.getConfig(configKey);
    }
    
    // 缓存统计
    public CacheStats getCacheStats() {
        return serviceCache.stats();
    }
}
```
...
public class ConnectionPoolConfig {
    ...
    @Bean
    public RegistryService registryService(ConnectionPool connectionPool) {
        return new RegistryService(connectionPool);
    }
    
    @Bean
    public ConfigService configService(ConnectionPool connectionPool) {
        return new ConfigService(connectionPool);
    }
}
```
...
FROM service_discovery_metrics
  ...
  - name: registry_alerts
    rules:
      - alert: ServiceInstanceDown
        expr: service_healthy_count == 0
        for: 1m
        labels:
          severity: critical
        annotations:
          summary: "服务实例全部下线"
          description: "服务 {{ $labels.service_name }} 没有健康实例"
      
      - alert: ServiceInstanceUnhealthy
        expr: service_unhealthy_count > 0
        for: 5m
        labels:
          severity: warning
        annotations:
          summary: "存在不健康服务实例"
          description: "服务 {{ $labels.service_name }} 有 {{ $labels.unhealthy_count }} 个不健康实例"
      
      - alert: ConfigUpdateFailed
        expr: rate(config_update_failed_total[5m]) > 0
        for: 5m
        labels:
          severity: warning
        annotations:
          summary: "配置更新失败"
          description: "配置组 {{ $labels.config_group }} 更新失败"
```
...
alert_rules:
  # 服务实例告警
  - name: service_instance_down
    condition: service_healthy_count == 0
    duration: 1m
    severity: critical
    message: "服务实例全部下线: {{ .Labels.service_name }}"
  
  # 不健康实例告警
  - name: service_instance_unhealthy
    condition: service_unhealthy_count > 0
    duration: 5m
    severity: warning
    message: "存在不健康服务实例: {{ .Labels.service_name }}"
  
  # 配置更新告警
  - name: config_update_failed
    condition: config_update_failed_rate > 0
    duration: 5m
    severity: warning
    message: "配置更新失败: {{ .Labels.config_group }}"
...
public class UserService {
    ...
    public User getUserById(String userId) {
        ...
        ServiceInstance instance = discoveryClient.getInstances("user-service")
                .stream()
                .findFirst()
                .orElseThrow(() -> new RuntimeException("No available instance"));
...
public class KubernetesIntegration {
    ...
    public void discoverServices() {
        // 从 Kubernetes 发现服务
        ServiceList services = kubernetesClient.services().inNamespace("default").list();
        
        for (Service service : services.getItems()) {
            log.info("Discovered service: {}", service.getMetadata().getName());
        }
    }
                ...
                .watch(new Watcher<ConfigMap>() {
                    ...
                    public void eventReceived(Action action, ConfigMap configMap) {
                        log.info("ConfigMap {} changed: {}", 
                                configMap.getMetadata().getName(), action);
                        
                        // 刷新配置
                        refreshConfiguration(configMap);
                    }
                    
                    @Override
                    public void onClose(KubernetesClientException cause) {
                        log.error("Watch closed", cause);
                    }
                });
    }
}
