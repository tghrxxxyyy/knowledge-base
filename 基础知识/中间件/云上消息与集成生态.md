...
| 场景 | 推荐服务 | 选型理由 |
|------|---------|----------|
| 异步解耦 | SQS/Pub/Sub | 简单可靠 |
| 事件驱动 | EventBridge | 规则丰富 |
| 实时流处理 | Kinesis/Kafka | 高吞吐 |
| 推送通知 | SNS/FCM | 多端推送 |
| 物联网 | IoT Core/MQTT | 协议支持 |

```mermaid
graph TB
    subgraph 云消息架构选型
        APP[应用] --> EVENT{事件类型?}
        EVENT -->|队列| SQS[SQS/Pub/Sub]
        EVENT -->|事件| EB[EventBridge]
        EVENT -->|流| KINESIS[Kinesis]
        EVENT -->|推送| SNS[SNS/FCM]
...
COST=$(echo "$MESSAGES_SENT * 0.0000004" | bc)
echo "SQS成本: \$${COST}"
```
...
|----------|---------|------|--------|
| 消息数据 | SQS内置 | 14天 | 14天 |
| 事件规则 | CloudFormation | 每次变更 | 永久 |
| 监控数据 | CloudWatch | 15天 | 15天 |
| 配置文件 | S3版本控制 | 每次变更 | 永久 |
...
| 演练场景 | 演练步骤 | 预期结果 | RTO |
|----------|---------|----------|-----|
| 队列不可用 | 模拟队列故障 | 消息重试+死信队列 | <5min |
| Lambda故障 | 停止Lambda | 事件重试 | <1min |
| 跨区域故障 | 模拟区域故障 | 灾备区域接管 | <10min |
| 限流触发 | 模拟高并发 | 自动扩容 | <5min |
...
```json
{
  "FilterPolicy": {
    "store": ["example_corp"],
    "event": ["order_placed", "order_delivered"],
    "customer_interests": ["automotive", "music"],
...
| 路由条件 | 说明 | 示例 |
|----------|------|------|
| source | 事件来源 | myapp.orders |
| detail-type | 事件类型 | OrderCreated |
| detail | 事件详情 | 金额>=100 |
| 时间窗口 | 事件时间 | 过去1小时 |
| 多条件 | 组合条件 | AND/OR 逻辑 |

### 10.4 Step Functions 工作流
...
| 优化项 | 优化前 | 优化后 | 节省比例 |
|--------|--------|--------|----------|
| 批量发送 | 1000次API | 10次API | 99% |
| 消息压缩 | 100KB | 30KB | 70% |
| 生命周期 | 永久保留 | 7天保留 | 90% |
| 预留容量 | 按需付费 | 预留折扣 | 30-50% |
...
response = sqs.send_message(
    ...
    MessageGroupId='orders',  # 消息组ID
    MessageDeduplicationId='order-123-20240101',  # 去重ID
    MessageAttributes={
        'Priority': {
            'DataType': 'String',
            'StringValue': 'high'
...
response = sqs.receive_message(
    ...
    ReceiveRequestAttemptId='attempt-1'  # 用于精确一次语义
)
```
...
main_response = sqs.create_queue(
    ...
    Attributes={
        ...
        'RedrivePolicy': json.dumps({
            'deadLetterTargetArn': dlq_response['QueueArn'],
            'maxReceiveCount': '3'  # 最大接收次数
        })
    }
)
```
...
response = sns.subscribe(
    ...
    Protocol='sqs',
    Endpoint='arn:aws:sqs:us-east-1:123456789012:orders-queue.fifo',
    Attributes={
        'FilterPolicy': json.dumps({
            'orderType': ['VIP', 'PREMIUM'],
            'priority': [{'numeric': ['>=', 5]}],
            'region': ['us-east-1', 'us-west-2']
        })
    }
...
# SQS 订阅过滤策略
filter_policy = {
    'orderType': ['VIP', 'PREMIUM'],  # 精确匹配
    'priority': [{'numeric': ['>=', 5]}],  # 数值范围
    'region': ['us-east-1', 'us-west-2'],  # 列表匹配
    'status': [{'prefix': 'pending-'}]  # 前缀匹配
}
```
...
```python
# 主区域 SNS
primary_topic = sns.create_topic(
    Name='primary-orders',
    Attributes={
...
# 副区域 SNS
secondary_topic = sns.create_topic(
    Name='secondary-orders',
    Attributes={
        'DisplayName': 'Secondary Orders'
    }
)

# 配置跨区域订阅
sqs_client = boto3.client('sqs', region_name='us-west-2')
dlq_arn = 'arn:aws:sqs:us-west-2:123456789012:orders-dlq'

# 从主区域订阅到副区域
sns.subscribe(
    TopicArn=primary_topic['TopicArn'],
    Protocol='sqs',
    Endpoint=dlq_arn,
    Attributes={
        'RedrivePolicy': json.dumps({
...
event_pattern = json.dumps({
    ...
    'detail': {
        'amount': [
            {'numeric': ['>=', 1000]},  # 大于等于1000
            {'numeric': ['<=', 10000]}  # 小于等于10000
        ],
        'status': ['pending', 'processing'],
        'metadata': {
...
# 使用通配符
event_pattern_wildcard = json.dumps({
    'source': ['myapp.*'],  # 通配符匹配
    'detail-type': ['Order*']  # 前缀匹配
})

# 使用否定匹配
event_pattern_negation = json.dumps({
    'source': ['myapp.orders'],
    'detail': {
...
graph TB
    subgraph 事件源
        A[订单服务] -->|OrderCreated| B[事件总线]
        C[支付服务] -->|PaymentProcessed| B
        E[库存服务] -->|InventoryUpdated| B
    end
...
```json
{
  ...
  "States": {
    "ProcessOrder": {
      "Type": "Task",
      "Resource": "arn:aws:lambda:us-east-1:123456789012:process-order",
      "Next": "CheckInventory"
    },
    "CheckInventory": {
      "Type": "Choice",
    ...
    "CheckPaymentStatus": {
      "Type": "Task",
      "Resource": "arn:aws:lambda:us-east-1:123456789012:check-payment",
      "Next": "PaymentStatus"
    },
    "PaymentStatus": {
      "Type": "Choice",
    ...
    "ShipOrder": {
      ...
      "Parameters": {
        "TopicArn": "arn:aws:sns:us-east-1:123456789012:shipping",
        "Message.$": "$"
      },
      "End": true
    },
    "Backorder": {
      "Type": "Task",
      "Resource": "arn:aws:lambda:us-east-1:123456789012:handle-backorder",
      "Next": "WaitForBackorder"
    },
    "WaitForBackorder": {
      "Type": "Wait",
      "Seconds": 3600,
    ...
    "CancelOrder": {
      "Type": "Task",
      "Resource": "arn:aws:lambda:us-east-1:123456789012:cancel-order",
      "End": true
    },
    "Error": {
      "Type": "Fail",
...
```json
{
  ...
  "States": {
      ...
      "Branches": [
        {
          ...
          "States": {
            "ProcessInventory": {
              "Type": "Task",
              "Resource": "arn:aws:lambda:us-east-1:123456789012:process-inventory",
              "End": true
            }
          }
        },
        {
          ...
          "States": {
            "ProcessPayment": {
              "Type": "Task",
              "Resource": "arn:aws:lambda:us-east-1:123456789012:process-payment",
              "End": true
            }
          }
        },
        {
          ...
          "States": {
            "ProcessShipping": {
              "Type": "Task",
              "Resource": "arn:aws:lambda:us-east-1:123456789012:process-shipping",
              "End": true
            }
          }
        }
    ...
    "MergeResults": {
      "Type": "Task",
      "Resource": "arn:aws:lambda:us-east-1:123456789012:merge-results",
      "End": true
    }
  }
}
...
```python
# SQS 成本计算
def calculate_sqs_cost(messages_per_month, message_size_kb):
    # 免费额度：100万次请求/月
    free_requests = 1_000_000
    
    # 标准队列价格
    standard_price_per_request = 0.40 / 1_000_000  # $0.40 per million
    fifo_price_per_request = 0.50 / 1_000_000  # $0.50 per million
    
    # 数据传输成本
    data_transfer_price = 0.09 / 1000  # $0.09 per GB
    
    # 计算
    paid_requests = max(0, messages_per_month - free_requests)
    standard_cost = paid_requests * standard_price_per_request
    fifo_cost = paid_requests * fifo_price_per_request
    
    # 数据传输
    data_gb = (messages_per_month * message_size_kb) / (1024 * 1024)
    transfer_cost = data_gb * data_transfer_price
    
    return {
        'standard_cost': standard_cost,
        'fifo_cost': fifo_cost,
        'transfer_cost': transfer_cost,
        'total_standard': standard_cost + transfer_cost,
        'total_fifo': fifo_cost + transfer_cost
    }

# 示例计算
result = calculate_sqs_cost(messages_per_month=10_000_000, message_size_kb=5)
print(f"标准队列成本: ${result['total_standard']:.2f}")
print(f"FIFO队列成本: ${result['total_fifo']:.2f}")
```
...
cloudwatch.put_metric_alarm(
    AlarmName='SQS-Cost-Alert',
    MetricName='NumberOfMessagesSent',
    Namespace='AWS/SQS',
    Statistic='Sum',
    Period=86400,  # 1天
    EvaluationPeriods=1,
    Threshold=10_000_000,  # 1000万消息
    ComparisonOperator='GreaterThanThreshold',
    AlarmActions=['arn:aws:sns:us-east-1:123456789012:cost-alerts'],
    Dimensions=[
        {'Name': 'QueueName', 'Value': 'orders-queue.fifo'}
    ]
)
```
...
graph TB
    subgraph 事件生产者
        A[订单服务] -->|OrderCreated| B[SNS]
        C[支付服务] -->|PaymentCompleted| B
        E[库存服务] -->|InventoryUpdated| B
    end
...
class EventStore:
    ...
    def get_events(self, aggregate_id, from_version=0):
        ...
        response = self.table.query(
            ...
            FilterExpression=Key('version').gt(from_version),
