...
架构：
  图像 → CLIP Image Encoder → 图像向量 ─┐
  文本 → CLIP Text Encoder  → 文本向量 ─┤→ 统一向量空间 → 向量数据库
  音频 → CLAP Encoder       → 音频向量 ─┘
...
class MultimodalSearch:
    ...
    def index_image_text_pairs(self, pairs: list[dict]):
        """索引图文对"""
        images = [p["image_path"] for p in pairs]
        texts = [p["text"] for p in pairs]

        # CLIP 编码
        img_embeddings = self.clip_model.encode(images, batch_size=32)
        txt_embeddings = self.clip_model.encode(texts, batch_size=32)

        # 写入向量库（图像和文本分别存储，共享标签）
        entities = []
        for i, pair in enumerate(pairs):
            entities.append({
                ...
                "modality": "image",
                "content": pair["image_path"],
                "pair_id": pair["id"]
            })
            entities.append({
                "id": i * 2 + 1,
                "embedding": txt_embeddings[i].tolist(),
                "modality": "text",
                "content": pair["text"],
                "pair_id": pair["id"]
            })
        self.client.insert("multimodal", entities)

    def search(self, query: str, modality: str = "text", top_k: int = 10):
        """跨模态搜索"""
        ...
        results = self.client.search(
            ...
            data=[query_embedding.tolist()],
            limit=top_k,
            output_fields=["modality", "content", "pair_id"]
        )
        return results[0]
```
...
```
异常检测架构：
  正常数据 → Embedding 模型 → 正常向量分布（参考集）
  实时数据 → Embedding 模型 → 查询向量 → 相似度搜索
  → 相似度 < 阈值 → 异常告警
...
class VectorAnomalyDetector:
    ...
    def build_baseline(self, normal_vectors: np.ndarray, collection: str):
        """构建正常向量基线"""
        entities = [
            {"id": i, "embedding": v.tolist()}
            for i, v in enumerate(normal_vectors)
        ]
        self.client.insert(collection, entities)

    def detect(self, query_vector: np.ndarray, collection: str,
               k: int = 10) -> dict:
        """检测单个向量是否异常"""
        results = self.client.search(
...
推荐系统向量召回策略：
  ...
  2. 向量召回细化：
     ├── 双塔模型：User Tower + Item Tower → 对比学习
     ├── 多目标：CTR/CVR/GMV 多目标排序
     ├── 实时特征：Flink 实时更新用户向量
     └── 冷启动：新物品用内容特征生成向量
...
| 策略 | 向量来源 | 更新频率 | 适用 |
|------|----------|----------|------|
| 离线向量 | 离线训练模型 | T+1 | 冷启动/基础召回 |
| 近实时向量 | Flink 实时特征 | 分钟级 | 兴趣变化追踪 |
| 实时向量 | 在线推理 | 毫秒级 | 高精度个性化 |

## 向量数据库容量规划

```
容量估算公式：
  存储量 = 向量数 × 向量维度 × 4字节/维度 × (1 + 索引开销)
  内存量 = 向量数 × 向量维度 × 4字节/维度 × 索引内存系数

  HNSW 索引内存系数 ≈ 1.2~1.5（每向量额外开销）
  IVF-PQ 索引内存系数 ≈ 0.3~0.5（量化压缩）

示例（1000万向量，1536维）：
  原始向量：10M × 1536 × 4B = 61.4GB
  HNSW 索引：61.4GB × 1.3 ≈ 80GB 内存
  IVF-PQ 索引：61.4GB × 0.4 ≈ 25GB 内存

  查询 QPS 估算：
    HNSW：单节点 5000~10000 QPS（取决于维度/数据量）
    IVF-PQ：单节点 10000~20000 QPS（量化加速）
    需要 10000 QPS → 2~3 节点（HNSW）或 1 节点（IVF-PQ）
```
...
| 检查项 | 频率 | 做法 |
|--------|------|------|
| 索引健康 | 每日 | 检查索引大小/碎片率/构建状态 |
| 查询延迟 | 实时 | 监控 P50/P95/P99 延迟 |
| 内存使用 | 实时 | 监控向量内存/系统内存 |
| 数据一致性 | 每周 | 校验向量数量与源数据一致 |
| 备份验证 | 每月 | 恢复测试验证备份可用性 |
| 索引重建 | 按需 | 数据大量变更后重建索引 |
| 版本升级 | 按需 | 测试环境验证后升级 |
| 安全审计 | 每月 | 检查认证/授权/加密配置 |
...
生产级 RAG 架构：
  1. 数据处理层
     ├── 文档解析：PDF/Word/HTML → 纯文本
     ├── 文本分块：按语义/固定长度分块
     ├── 向量化：Embedding 模型（OpenAI/Cohere/自训练）
     └── 元数据提取：标题/作者/时间/标签
...
| 策略 | 方法 | 效果 |
|------|------|------|
| 查询扩展 | HyDE（假设文档嵌入） | 提升检索召回率 |
| 分块优化 | 语义分块（按段落/主题） | 提升相关性 |
| 混合检索 | 向量 + BM25 融合 | 提升召回率 |
| 重排序 | Cross-Encoder 重排 | 提升准确率 |
| 上下文压缩 | 压缩无关上下文 | 降低 token 消耗 |
| 多轮检索 | 迭代检索 + 生成 | 复杂问题效果好 |
...
# 混合搜索：向量 + 关键词 + 元数据
class HybridSearch:
    def __init__(self, vector_db, keyword_index, metadata_db):
...
# 方案二：Partition 级隔离
def create_tenant_partition(collection, tenant_id):
    partition_name = f"tenant_{tenant_id}"
    collection.create_partition(partition_name)
    return partition_name

# 方案三：字段级隔离
def search_with_tenant(collection, query, tenant_id):
...
```bash
# Milvus 备份
etcdctl snapshot save /backup/milvus-etcd-$(date +%Y%m%d).db
aws s3 sync /var/lib/milvus/data/ s3://milvus-backup/data/
...
curl -X POST http://localhost:8080/v1/backups \
  ...
  -d '{"id":"backup-001","includes":["class1","class2"]}'
```
...
恢复流程：
  1. 停止写入（暂停业务）
  2. 恢复元数据（etcd/Qdrant/Weaviate 配置）
  3. 恢复数据（MinIO/S3 数据）
  4. 验证数据完整性
  5. 重新建立索引
  6. 恢复业务
...
```
向量数据库 Benchmark：
  数据集：100 万向量，维度 1536（OpenAI Ada-002）
  
  测试指标：
    1. 写入性能：QPS（Queries Per Second）
    2. 查询性能：延迟（Latency）P50/P95/P99
    3. 召回率：Recall@10（Top-10 准确率）
    4. 资源消耗：CPU/内存/磁盘

  Benchmark 工具：
    ann-benchmarks（标准工具）
    vector-db-bench（专用工具）
    自定义脚本（Python）
```

### 20.2 性能对比

| 数据库 | 写入 QPS | 查询延迟 P95 | Recall@10 | 内存占用 |
|--------|----------|-------------|-----------|----------|
| Milvus | 5000 | 5ms | 0.95 | 8GB |
| Qdrant | 4000 | 8ms | 0.93 | 10GB |
| Weaviate | 3000 | 10ms | 0.92 | 12GB |
| Pinecone | 2000 | 15ms | 0.90 | N/A |
| pgvector | 1000 | 50ms | 0.85 | 6GB |
...
class ImageDedup:
    ...
    def add_image(self, image_path, metadata):
        ...
        # 3. 检查哈希相似度（汉明距离 < 10 = 相似）
        similar_hash = self.find_similar_hash(phash, threshold=10)
        if similar_hash:
            return {"status": "duplicate", "similar_id": similar_hash}
        
        # 4. 向量检索（余弦相似度 > 0.95 = 重复）
        similar_vectors = self.vector_db.search(embedding, top_k=5)
        if similar_vectors and similar_vectors[0]['score'] > 0.95:
            return {"status": "duplicate", "similar_id": similar_vectors[0]['id']}
        
        # 5. 存储
        doc_id = self.vector_db.insert(embedding, metadata)
        self.hash_db[phash] = doc_id
        return {"status": "new", "id": doc_id}
```
...
| 场景 | 说明 | 相似度阈值 |
|------|------|------------|
| 电商图片去重 | 去除重复商品图片 | 余弦 > 0.95 |
| 社交媒体去重 | 去除转发重复图片 | 余弦 > 0.90 |
| 版权检测 | 检测盗版图片 | 汉明距离 < 10 |
| 医疗影像去重 | 去除重复检查影像 | 余弦 > 0.98 |
| 安防监控 | 去除重复告警图片 | 余弦 > 0.85 |
...
|------|----------|----------|------|
| 朴素检索 | 直接向量检索 | 简单场景 | 基线 |
| 混合检索 | 向量+关键词融合 | 通用场景 | +15% |
| 重排序 | Cross-encoder重排 | 精度要求高 | +20% |
| 多路召回 | 多策略并行 | 复杂场景 | +25% |
| 查询改写 | LLM改写查询 | 模糊查询 | +10% |
...
def choose_vector_db(requirements):
    ...
    requirements: dict
      ...
      - budget: 预算
    """
    if requirements['data_scale'] == 'small' and requirements['budget'] == 'low':
        return 'pgvector'
    elif requirements['data_scale'] == 'medium' and requirements['latency'] < 10:
        return 'Qdrant'
    elif requirements['data_scale'] == 'large' and requirements['consistency'] == 'strong':
        return 'Milvus'
    else:
        return 'Pinecone'
```
...
| 维度 | 向量数据库 | MySQL | ES |
|------|-----------|-------|-----|
| 查询类型 | 相似度搜索 | 精确匹配 | 全文搜索 |
| 索引算法 | HNSW/IVF | B+Tree | 倒排索引 |
| 数据模型 | 高维向量 | 结构化表 | JSON文档 |
| 典型延迟 | 1-10ms | 1-5ms | 10-100ms |
| 扩展方式 | 分片+副本 | 主从+分片 | 分片+副本 |
| 适用场景 | AI/推荐/检索 | 事务/分析 | 搜索/日志 |
...
| 参数 | 默认值 | 说明 | 调优建议 |
|------|--------|------|----------|
| M | 16 | 每层连接数 | 增大提高召回率，增加内存 |
| efConstruction | 256 | 构建时搜索范围 | 增大提高索引质量 |
| ef | 64 | 查询时搜索范围 | 增大提高召回率，增加延迟 |

### IVF 索引参数

| 参数 | 默认值 | 说明 | 调优建议 |
|------|--------|------|----------|
| nlist | 1024 | 聚类中心数 | 增大提高精度，增加构建时间 |
| nprobe | 8 | 搜索聚类数 | 增大提高召回率，增加延迟 |

### 索引调优实践

```python
# 1. 数据量 vs 索引选择
数据量 < 100万：HNSW（召回率高，构建快）
数据量 100万-1亿：IVF_FLAT（平衡性能和召回率）
数据量 > 1亿：IVF_PQ（压缩向量，节省内存）
...
架构组件：
  ├── Embedding 模型：text-embedding-ada-002/bge-large-zh
  ├── 向量数据库：Milvus/Qdrant/Weaviate
  ├── 重排序模型：bge-reranker-large
  └── LLM：GPT-4/ChatGLM/Qwen
```
...
| 策略 | 描述 | 效果 |
|------|------|------|
| 分块优化 | 调整 chunk_size 和 overlap | 提高检索精度 |
| 混合检索 | 向量+关键词混合检索 | 提高召回率 |
| 重排序 | 使用 Cross-Encoder 重排序 | 提高相关性 |
| 多路召回 | 多种检索策略并行 | 提高覆盖率 |
| 查询扩展 | 扩展用户查询 | 提高召回率 |
...
| query_latency | 查询延迟 | > 100ms |
| index_size | 索引大小 | 接近内存上限 |
| memory_usage | 内存使用 | > 80% |
| disk_usage | 磁盘使用 | > 85% |
| insert_count | 插入数量 | 基线对比 |
| insert_latency | 插入延迟 | > 50ms |
  ...
  - name: vector_db
    rules:
      - alert: VectorDBHighLatency
        expr: histogram_quantile(0.99, rate(vector_db_query_duration_seconds_bucket[5m])) > 0.1
        for: 5m
        labels:
          severity: warning
        annotations:
      ...
      - alert: VectorDBHighMemory
        expr: vector_db_memory_usage / vector_db_max_memory > 0.8
        for: 5m
        labels:
          severity: warning
        annotations:
...
| 模型 | 维度 | 中文支持 | 性能 | 适用场景 |
|------|------|----------|------|----------|
| text-embedding-ada-002 | 1536 | 一般 | 高 | 英文为主 |
| bge-large-zh | 1024 | 优秀 | 高 | 中文场景 |
| bge-m3 | 1024 | 优秀 | 极高 | 多语言 |
| m3e-base | 768 | 良好 | 中 | 中文轻量 |
| e5-large | 1024 | 一般 | 高 | 英文为主 |
...
模型选择决策：
  1. 语言支持
     ├── 纯中文：bge-large-zh/m3e-base
     ├── 纯英文：text-embedding-ada-002/e5-large
     └── 多语言：bge-m3

  2. 性能要求
     ├── 高性能：bge-m3/bge-large-zh
     └── 轻量级：m3e-base

  3. 成本考虑
     ├── 开源免费：bge/m3e 系列
     └── 商业API：OpenAI text-embedding
```
...
| 数据库 | 1M数据 | 10M数据 | 100M数据 | 延迟(ms) | 召回率 |
|--------|--------|---------|----------|----------|--------|
| Milvus | 1M | 10M | 100M+ | 5-20 | 95%+ |
| Qdrant | 1M | 10M | 100M+ | 3-15 | 95%+ |
| Weaviate | 1M | 10M | 100M+ | 5-25 | 90%+ |
| pgvector | 1M | 10M | 10M | 10-50 | 90%+ |
| Chroma | 100K | 1M | 1M | 5-20 | 90%+ |
...
# 2. 插入测试
def test_insert(collection, vectors, batch_size=1000):
    start = time.time()
    for i in range(0, len(vectors), batch_size):
        batch = vectors[i:i+batch_size]
        collection.insert([list(range(i, i+len(batch))), batch.tolist()])
    collection.flush()
    return time.time() - start

# 3. 查询测试
def test_query(collection, query_vectors, top_k=10):
    start = time.time()
    for vector in query_vectors:
        collection.search([vector.tolist()], "embedding", {"metric_type": "L2", "params": {"ef": 64}}, top_k)
    return time.time() - start
...
# 授权用户
utility.grant_role("reader", "user1")

# 查看权限
utility.get_role("reader")
```
...
# Milvus 运维
# 查看集合状态
milvusctl show collection --name my_collection

# 查看索引状态
milvusctl show index --collection my_collection --name my_index

# 手动压缩
milvusctl compact --collection my_collection

# 查看副本状态
milvusctl show replica --collection my_collection
```
...
```bash
# Milvus 备份
milvus-backup create --name my_backup --collection my_collection

# Milvus 恢复
milvus-backup restore --name my_backup --collection my_collection_restore
```
...
数据建模原则：
  ...
  2. 字段设计
     ├── 必须字段：id, embedding
     ├── 可选字段：metadata, content
     └── 支持过滤：category, timestamp

  3. 索引选择
     ├── 小数据量：HNSW
     ├── 大数据量：IVF_FLAT/IVF_PQ
     └── 高召回率：HNSW + 大 ef
