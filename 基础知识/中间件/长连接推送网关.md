...
| Token 参数 | URL参数传递 | 低 |
| Header 鉴权 | 请求头传递 | 中 |
| Cookie 鉴权 | Cookie传递 | 中 |
| JWT | JSON Web Token | 高 |
| 双向鉴权 | 服务端也需验证 | 极高 |
...
```text
Token 刷新策略：
  1. 连接时验证：WebSocket握手时验证Token
  2. 定期刷新：每30分钟刷新Token
  3. 过期处理：Token过期时断开连接
  4. 双向验证：服务端证书验证

  最佳实践：
    - 使用短生命周期Token（15-30分钟）
    - 支持无感刷新（在Token过期前主动刷新）
    - 断线重连时重新验证Token
```
...
public class TokenBucket {
    ...
    private void refill() {
        ...
        long refill = elapsed * refillRate / 1_000_000_000L;
...
public class PushGatewayMetrics {
    private final Counter messageCounter = Counter.build()
        ...
        .help("Total push messages")
        .labelNames("type", "status")
        .register();

    private final Gauge connectionGauge = Gauge.build()
        .name("push_connections_active")
    ...
    public void recordMessage(String type, String status) {
        messageCounter.labels(type, status).inc();
    }
}
```
...
public class DeviceStatusManager {
    public DeviceStatus getStatus(String deviceId) {
        // 1. 检查连接状态
        boolean connected = connectionManager.isConnected(deviceId);
        
        // 2. 检查最后活跃时间
        long lastActive = redis.get(deviceId + ":last_active");
        
        // 3. 判断状态
        if (connected) {
            if (System.currentTimeMillis() - lastActive > 300000) {
...
| 协议 | 传输层 | 压缩 | 二进制 | 适用场景 |
|------|--------|------|--------|---------|
| WebSocket | TCP | 可选 | 可选 | Web应用 |
| MQTT | TCP | 支持 | 是 | IoT设备 |
| STOMP | TCP | 可选 | 否 | 消息代理 |
| SSE | HTTP | 否 | 否 | 服务器推送 |
| gRPC | HTTP/2 | 支持 | 是 | 微服务 |
...
| 推送延迟 | 消息积压 | 检查消息队列 | 扩容消费者 |
| 认证失败 | Token过期 | 检查Token | 刷新Token |
| 内存溢出 | 连接数过多 | 检查连接数 | 限制连接数 |
| 消息丢失 | 持久化失败 | 检查存储 | 修复存储 |
