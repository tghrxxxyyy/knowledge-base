...
| 日志类型 | 说明 | 保留策略 |
|----------|------|---------|
| CloudTrail | API调用日志 | 90天 |
| Access Log | 访问日志 | 365天 |
| 配置变更 | IAM配置变更 | 永久 |
| 登录日志 | 用户登录日志 | 365天 |
...
identity_federation:
  ...
  cross_cloud:
    aws:
      role_arn: "arn:aws:iam::account:role/federation-role"
...
| 审计维度 | 说明 | 合规要求 |
|----------|------|----------|
| 访问控制 | 身份与权限 | SOC2/ISO27001 |
| 日志记录 | 操作审计 | SOC2/GDPR |
| 密钥管理 | 密钥安全 | SOC2/PCI DSS |
| 安全监控 | 异常检测 | ISO27001 |
...
| 多云维度 | 说明 | 工具 |
|----------|------|------|
| 统一身份 | 身份管理 | Okta/Azure AD |
| 统一权限 | 权限管理 | Terraform/Pulumi |
| 统一审计 | 审计管理 | Datadog/Splunk |
| 统一监控 | 监控管理 | Prometheus/Grafana |
...
| 认证失败 | 凭证错误 | 检查凭证 | 修正凭证 |
| 授权失败 | 权限不足 | 检查策略 | 调整策略 |
| 联邦认证失败 | 配置错误 | 检查配置 | 修正配置 |
| 临时凭证过期 | 过期时间短 | 检查过期时间 | 调整时间 |
| MFA验证失败 | MFA设备问题 | 检查MFA设备 | 重置MFA |
| 审计日志缺失 | 日志配置 | 检查日志配置 | 启用日志 |
...
iam_monitoring:
  ...
  alerts:
    - name: "high_auth_failures"
      condition: "iam_auth_failures > 100"
      severity: "critical"
    
    - name: "low_mfa_usage"
      condition: "iam_mfa_usage < 0.8"
      severity: "warning"
    
    - name: "unusual_key_usage"
      condition: "iam_key_usage > 10000"
      severity: "warning"
```

| 监控指标 | 说明 | 告警阈值 |
|----------|------|----------|
| auth_failures | 认证失败次数 | >100 |
| authz_failures | 授权失败次数 | >50 |
| mfa_usage | MFA使用率 | <80% |
| key_usage | 密钥使用次数 | >10000 |

> 核心原则：**最小权限，临时凭证，多因素认证，身份联邦，统一审计，多云治理**。
...
| 治理维度 | 检查项 | 工具 |
|----------|--------|------|
| 权限管理 | 最小权限、权限边界 | IAM Access Analyzer |
| 凭证管理 | 密钥轮换、临时凭证 | AWS STS/Azure AD |
| 认证管理 | MFA、SSO | Okta/Azure AD |
| 审计管理 | 日志记录、合规检查 | CloudTrail/Config |
| 多云治理 | 统一策略、跨云审计 | Terraform/Pulumi |
...
| 维度 | AWS IAM | Azure AD | GCP IAM | 自建IAM |
|------|---------|----------|---------|---------|
| 可用性 | 99.99% | 99.99% | 99.99% | 自行保证 |
| 扩展性 | 自动扩展 | 自动扩展 | 自动扩展 | 需要设计 |
| 安全性 | 云厂商保证 | 云厂商保证 | 云厂商保证 | 需要专业团队 |
| 成本 | 按使用付费 | 按使用付费 | 按使用付费 | 硬件+人力 |
| 功能 | 丰富 | 丰富 | 丰富 | 需要开发 |
| 合规 | 云厂商认证 | 云厂商认证 | 云厂商认证 | 需要自行认证 |
...
| 维度 | 说明 | 优化方向 |
|------|------|----------|
| 按用户计费 | 每月活跃用户数 | 控制活跃用户数 |
| 按请求计费 | API调用次数 | 减少重复调用 |
| 按存储计费 | 策略/角色存储 | 清理无用策略 |
| 按功能计费 | 高级功能使用 | 按需启用 |
