...
| 参数 | 推荐值 | 说明 |
|------|--------|------|
| TaskManager内存 | 4-8GB | 根据状态大小调整 |
| TaskManager核数 | 2-4核 | 并行度÷TM核数 |
| Checkpoint间隔 | 1-5分钟 | 根据延迟要求调整 |
| 并行度 | CPU核数×2-3 | 最大化资源利用 |

### 常见问题排查

| 问题 | 排查步骤 | 解决方案 |
|------|----------|----------|
| 反压 | 检查Web UI反压标签 | 增加并行度/优化算子 |
| Checkpoint超时 | 检查Checkpoint日志 | 增加超时时间/减少状态 |
| 内存溢出 | 检查TaskManager日志 | 增加内存/优化状态 |
| 数据倾斜 | 检查分区分布 | 加盐/预聚合 |
