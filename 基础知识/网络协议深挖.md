...
HTTP/2 Stream 状态转换：
  ...
  open（打开）
    ↓ 发送/接收 DATA
    ↓ half-closed (remote)：对方发了 END_STREAM
    ↓ half-closed (local)：自己发了 END_STREAM
  closed（关闭）

  状态转换规则：
    idle → open：发送/接收 HEADERS
    open → half-closed：发送 END_STREAM
    half-closed → closed：收到 END_STREAM + 响应完成
    open → closed：发送 RST_STREAM（异常终止）
```
...
```
HTTP/2 帧优先级机制：
  Stream Dependency（流依赖）：形成依赖树
  Weight（权重）：1-256，默认 16
  Exclusive Flag：独占依赖

  示例依赖树：
    root (id=0)
      ├── A (weight=200, exclusive)
      │     └── C (weight=100)
      └── B (weight=100)
...
HTTP/2 流控窗口：
  连接级窗口：所有 stream 共享
  Stream 级窗口：每个 stream 独立

  窗口更新：
    接收方处理数据后发送 WINDOW_UPDATE 帧
    增加窗口大小（字节数）
  ...
  默认窗口：65535 字节（64KB）
  调优：SETTINGS_INITIAL_WINDOW_SIZE（大文件传输可调大）

  问题：
    小窗口 → 频繁 WINDOW_UPDATE → 带宽浪费
    大窗口 → 内存压力 → 需平衡
```
...
Http2FrameCodecBuilder.forClient()
    .initialWindowSize(1048576)  // 1MB 窗口
    .build();
```
...
HPACK 动态表维护：
  ...
  效果：
    重复头部（Cookie/Accept）：索引编码 → 1-2 字节
    新头部：完整编码 + 可选加入动态表

  注意：
    动态表大小限制：SETTINGS_HEADER_TABLE_SIZE（默认 4096）
    超出时强制淘汰旧条目
```
...
### 23.1 Connection ID 管理

```
QUIC Connection ID 管理：

  生成：
    服务端生成随机 CID（64-128 位）
    客户端首次连接获取

  轮换：
    服务端可发送 NEW_CONNECTION_ID 帧
    客户端使用新 CID（防追踪）

  验证：
    客户端发送 PATH_CHALLENGE 帧
    服务端响应 PATH_RESPONSE
    验证路径可达性

  退役：
    客户端发送 RETIRE_CONNECTION_ID
    服务端释放旧 CID 资源
```
...
TLS 1.3 0-RTT 重放攻击：
  ...
  风险等级：
    幂等请求（GET）：低风险
    非幂等请求（POST/DELETE）：高风险

  防护措施：
    ① 0-RTT 只传幂等请求
    ② 服务端检测重放（Nonce/时间戳）
    ③ 限制 0-RTT 数据大小
    ④ 使用 PSK 而非 0-RTT 早期数据
...
### 24.2 Session Ticket 安全性

```
Session Ticket 安全性：

  加密密钥：
    服务端使用 Ticket Key 加密会话数据
    密钥定期轮换（推荐 24 小时）
  ...
  最佳实践：
    多密钥轮换（key1, key2, key3）
    当前密钥签发新票据
    旧密钥验证旧票据
```

### 24.3 TLS 1.3 与 1.2 安全对比

| 安全特性 | TLS 1.2 | TLS 1.3 |
|----------|---------|---------|
| 前向保密 | 可选（需配置 ECDHE） | 强制（所有密码套件） |
| 弱密码 | 支持（RC4, 3DES, CBC） | 移除（仅 AES-GCM/ChaCha20） |
| 握手加密 | 部分加密 | 全部加密（包括证书） |
| 0-RTT | 不支持 | 支持（有重放风险） |
| 密钥交换 | RSA/ECDHE/DHE | 仅 (EC)DHE |
| 证书验证 | RSA 签名 | 支持 ECDSA/EdDSA |
...
BBR 调优参数：
  ...
  BBR 参数：
    net.ipv4.tcp_rto_min=200ms      # 最小 RTO
    net.ipv4.tcp_slow_start_after_idle=0  # 空闲后不慢启动

  BBR vs CUBIC：
    高延迟网络：BBR 吞吐提升 2-10x
    低延迟网络：差异不大
    丢包环境：BBR 更稳定

  监控 BBR：
    ss -ti | grep bbr
    # 显示 BBR 状态：bw, rtt, pacing_gain
```
...
| 场景 | 推荐协议 | 理由 |
|------|----------|------|
| 微服务内部调用 | gRPC/Dubbo | 高性能+治理 |
| 对外 REST API | HTTP/JSON | 通用性+调试 |
| 实时双向通信 | WebSocket | 全双工 |
| 流式推送 | gRPC Streaming/SSE | 流式支持 |
| 弱网移动端 | HTTP/3 QUIC | 0-RTT+连接迁移 |
| 大数据传输 | gRPC Streaming | 流式+压缩 |
| 服务网格 | gRPC | xDS 原生支持 |
...
CUBIC（Linux 默认拥塞控制算法）：
  ...
  参数：
    C = 0.4（增长系数）
    Wmax = 上次丢包时的窗口大小
    k = 从 Wmax 恢复到 Wmax 所需时间

  特点：
    高带宽长距离网络表现好
    RTT 不敏感（公平性好）
    适合数据中心间传输
    Linux 内核默认（net.ipv4.tcp_congestion_control=cubic）
```
...
| 场景 | 推荐算法 | 理由 |
|------|----------|------|
| 数据中心内网 | Cubic | 低延迟，丢包少 |
| 跨国/跨云 | BBR | 高延迟，吞吐提升明显 |
| 视频直播 | BBR | 丢包敏感，吞吐优先 |
| 移动网络 | BBR | 丢包率高，RTT 波动大 |
| 混合场景 | BBR | 兼顾各种网络条件 |
...
# [  5] 0.00-30.00 sec  2.78 GBytes  796 Mbits/sec
```
...
# 客户端（UDP 测试）
nuttcp -u -b 1G -t 30 server_ip

# 输出结果分析
#  2688.00 MB /  30.00 sec =  91.82 MB/sec
```

### mtr 网络诊断

```bash
# 综合诊断（ping + traceroute）
mtr -r -c 100 target.com

# 输出解读
# Host                 Loss%   Snt   Last   Avg  Best  Wrst StDev
# 1. gateway           0.0%   100    0.5   0.8   0.3   1.2   0.2
# 2. isp-router        0.0%   100    5.2   6.1   4.8   8.3   0.8
# 3. target            0.5%   100   15.3  16.2  14.5  22.1   1.5
# Loss% = 丢包率，Avg = 平均延迟，StDev = 延迟抖动
```

## 二十九、网络安全（TLS 1.3/证书管理/WAF）

### TLS 1.3 安全特性

| 特性 | 说明 | 安全提升 |
|------|------|----------|
| 前向保密 | 所有密码套件强制支持 | 防历史流量泄露 |
| 弱密码移除 | 移除 RC4/3DES/CBC | 防已知攻击 |
| 握手加密 | Certificate 也加密 | 隐私保护 |
| 0-RTT | 会话恢复 | 性能+安全平衡 |
...
# 常见攻击防护
# SQL 注入：检测 UNION SELECT 等
# XSS：检测 <script> 等
# 路径遍历：检测 ../ 等
# 命令注入：检测 | ; 等
```

## 三十、网络协议调优（sysctl参数/TCP缓冲区/窗口缩放）
...
# 窗口缩放因子（0-14）
# 因子=7 → 最大窗口 = 65535 × 2^7 = 8MB
# 因子=14 → 最大窗口 = 65535 × 2^14 = 1GB

# 查看当前窗口大小
ss -tni | grep ESTAB
# cwnd:10 ssthresh:20 rto:200 rtt:1.5/0.5
# recv_buf:131072 send_buf:131072
```
...
| net.ipv4.tcp_max_syn_backlog | 128 | 65535 | 半连接队列上限 |
| net.ipv4.tcp_tw_reuse | 0 | 1 | TIME_WAIT 复用 |
| net.ipv4.ip_local_port_range | 32768 60999 | 1024 65535 | 端口范围 |
| net.ipv4.tcp_syncookies | 0 | 1 | SYN 洪泛防护 |
| net.ipv4.tcp_keepalive_time | 7200 | 600 | keepalive 超时 |
...
## 三十一、网络监控（Prometheus网络指标/延迟/丢包/抖动）
...
- name: network_alerts
  rules:
  - alert: HighLatency
    expr: node_network_receive_errs_total > 100
    for: 5m
...
# 延迟监控脚本
while true; do
    latency=$(ping -c 10 target.com | tail -1 | awk -F '/' '{print $5}')
...
```text
丢包监控指标：
  TcpRetransSegs: TCP 重传段数
  TcpExtListenOverflows: 全连接队列溢出
  TcpExtTCPAbortOnMemory: 内存不足导致的连接中止

抖动监控：
  RTT 标准差 = sqrt(mean(RTT²) - mean(RTT)²)
  抖动 > 50ms → 网络不稳定
...
| 队列 | 现象 | 排查 | 解决 |
|------|------|------|------|
| 半连接队列 | SYN_RECV 堆积 | ss -tn state syn-recv | 调 tcp_max_syn_backlog |
| 全连接队列 | Recv-Q 堆积 | ss -lnt Recv-Q > 0 | 调 somaxconn + 加快 accept |
...
Nagle 算法：
  ...
  问题：与延迟确认叠加 → 等 40ms 延迟
  
  解决：
    TCP_NODELAY=1（禁用 Nagle）
    TCP_QUICKACK=1（禁用延迟确认）
...
时间线：
  T0: 客户端发小包（Nagle 合并）
  T1: 服务端收包 → 延迟确认（等 40ms）
  T2: 客户端等 ACK → Nagle 不发新包
  T3: 40ms 后 → 服务端发 ACK
  T4: 客户端收到 ACK → 发新包

  死锁风险：两端都等 → 延迟翻倍
  解决：TCP_NODELAY 禁用 Nagle
```

## 十一、TLS 会话恢复

### Session ID vs Session Ticket

| 方式 | 原理 | 优点 | 缺点 |
|------|------|------|------|
| Session ID | 服务端缓存会话 | 简单 | 有状态，负载均衡不友好 |
| Session Ticket | 客户端携带加密票据 | 无状态 | 密钥泄露风险 |

```text
TLS 会话恢复流程：
  首次握手：
    Client Hello → Server Hello + 会话票据
    密钥协商 → 加密通信

  恢复握手：
    Client Hello（带 Session Ticket）→ Server 解密票据
    跳过完整密钥协商 → 1-RTT 恢复
...
```text
QUIC 连接迁移：
  TCP 连接 = 四元组（src IP/port + dst IP/port）
  → IP 变化 → 连接断开

  QUIC 连接 = Connection ID（随机标识）
  → IP 变化 → 用 Connection ID 继续
  → 无缝切换（WiFi → 4G）
...
心跳参数推导公式：
  ...
  心跳间隔计算：
    NAT 超时 300s → 心跳间隔 ≤ 100s
    NAT 超时 60s → 心跳间隔 ≤ 20s
    
  超时计算：
    连接超时 = 3 × 心跳间隔（3次心跳未收到则判定断开）
...
```text
半连接队列（SYN Queue）：
  存放：SYN_RECEIVED状态的连接
  大小：tcp_max_syn_backlog（默认128）
  溢出：SYN被丢弃，重传SYN-ACK

全连接队列（Accept Queue）：
  存放：ESTABLISHED状态等待accept的连接
  大小：min(somaxconn, backlog)
  溢出：新连接被丢弃

排查命令：
  ss -lnt | grep -E 'SYN|ACK'
  netstat -s | grep -i listen

溢出处理：
  1. 增大队列：sysctl -w net.core.somaxconn=65535
  2. 加快accept：使用epoll事件驱动
  3. 监控：watch -n 1 'ss -lnt'
```
...
| 问题 | 描述 | 解决方案 |
|------|------|----------|
| Nagle算法 | 小包合并，等待ACK | TCP_NODELAY |
| 延迟确认 | 等待数据一起ACK | TCP_QUICKACK |
| 40ms死锁 | 两者叠加导致延迟 | 关闭Nagle或延迟确认 |
...
# 设置TCP_QUICKACK
setsockopt(fd, IPPROTO_TCP, TCP_QUICKACK, &val, sizeof(int));
```
...
| 方式 | 机制 | 握手RTT | 安全性 |
|------|------|---------|--------|
| Session ID | 服务端缓存会话 | 1-RTT | 高 |
| Session Ticket | 客户端携带Ticket | 1-RTT | 中 |
| 0-RTT | 携带PSK数据 | 0-RTT | 有重放风险 |

### QUIC连接迁移

```text
QUIC特点：
  Connection ID：标识连接，非IP:Port
  多路复用：多Stream独立，无队头阻塞
  0-RTT：携带前次参数直接发数据
  连接迁移：网络切换无需重建连接
...
tcpdump 'tcp and port 80 and (((ip[2:2] - ((ip[0]&0xf)<<2)) - ((tcp[12]&0xf0)>>2)) != 0)'
...
| 特性 | 说明 | 优化点 |
|------|------|--------|
| HEADERS帧 | 头部压缩HPACK | 合并头部 |
| DATA帧 | 数据传输 | 流控窗口 |
| 流控 | 每个流独立窗口 | 调整窗口大小 |
| PRIORITY | 流优先级 | 合理设置优先级 |
...
| 协议 | 优势 | 劣势 | 适用场景 |
|------|------|------|----------|
| TCP | 可靠有序 | 队头阻塞 | HTTP/数据库 |
| UDP | 低延迟 | 不可靠 | 视频/游戏 |
| QUIC | 可靠+低延迟 | 生态较新 | 实时应用 |
