...
| 故障类型 | 排查步骤 | 解决方案 |
|----------|----------|----------|
| 连接拒绝 | 检查maxclients/连接池 | 调整参数 |
| 内存溢出 | 检查maxmemory/大key | 清理/扩容 |
| 延迟高 | 检查慢查询/网络 | 优化命令 |
| 主从延迟 | 检查复制缓冲区 | 调整参数 |
...
| 维度 | Redis | Memcached | Caffeine |
|------|-------|-----------|----------|
| 数据结构 | 丰富 | 简单 | 简单 |
| 持久化 | 支持 | 不支持 | 不支持 |
| 分布式 | 集群 | 客户端分片 | 本地 |
| 适用场景 | 缓存/消息 | 缓存 | 本地缓存 |
| 许可证 | BSD | BSD | Apache 2.0 |

## Redis 版本对比

| 版本 | 功能 | 适用场景 | 许可证 |
|------|------|----------|--------|
| Redis 7.x | 最新特性 | 新项目 | BSD |
| Redis 6.x | 稳定 | 生产环境 | BSD |
| Redis 5.x | 旧版本 | 已有项目 | BSD |
