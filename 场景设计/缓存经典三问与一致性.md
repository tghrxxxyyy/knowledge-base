...
```
请求 → CDN → Nginx 本地缓存 → 应用本地缓存(Caffeine) → Redis 集群 → DB
```

- 越往前命中，延迟越低、对后端压力越小。
- 本地缓存容量小、需控制 TTL 与淘汰策略（LFU/LRU），防止"缓存污染"。
- 一致性挑战：本地缓存失效慢，可用「Redis 发布订阅主动失效」或「短 TTL + 主动删除」。
...
Cache<String, Product> cache = Caffeine.newBuilder()
    ...
    .build();
Product p = cache.get(pid, id -> loadFromRedis(id));
```
...
- **大 Key**（如 1MB 的 Hash）：删除/迁移阻塞 Redis、网络拥塞 → 拆分、压缩、定期清理。
- **热 Key**（某 key 访问集中）：单分片 CPU 打满 → 本地缓存副本、key 打散（拼接随机后缀）、多副本读。

## 七、监控与指标

| 指标 | 含义 | 预警 |
|------|------|------|
| 命中率 | 命中/总请求 | 低于阈值告警（可能穿透） |
| 内存使用率 | used_memory/maxmemory | 接近上限告警 |
| 逐出数 | evicted_keys | 突增说明容量不足 |
| 慢查询 | slowlog | 大 key/复杂命令 |
| 连接数 | connected_clients | 异常增长 |
