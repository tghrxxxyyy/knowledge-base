# MySQL redolog与binlog源码要点

> redolog 保证崩溃恢复（物理），binlog 用于复制与归档（逻辑）。核心是两阶段提交。
...
MySQL redolog与binlog源码要点 的核心是"结构化拆解、明确边界、可观测兜底"：在正确性与扩展性之间取平衡，用工程化手段把不确定性降到最低，并随规模持续演进。
