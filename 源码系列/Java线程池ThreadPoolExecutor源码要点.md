# Java线程池ThreadPoolExecutor源码要点

> 线程池用核心/最大线程+队列+拒绝策略复用线程。核心是资源与饱和。
...
```python
# 伪代码：提交任务
if running < core: new_thread()
elif queue.offer(t): pass
elif running < max: new_thread()
else: reject(t)
```
...
Java线程池ThreadPoolExecutor源码要点 的核心是"结构化拆解、明确边界、可观测兜底"：在正确性与扩展性之间取平衡，用工程化手段把不确定性降到最低，并随规模持续演进。
